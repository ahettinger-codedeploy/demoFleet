...
MemberEventCode(0)      = 846794    'Requred event to purchase
MemberEventCode(1)      = 846795    'Requred event to purchase
MemberEventCode(2)      = 846793    'Requred event to purchase


Dim AllowedSeatType(2)
...
SQLMemberCurrent = "SELECT COUNT(OrderLine.LineNumber) AS TicketCount FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber WHERE EventCode = 846795 AND OrderLine.OrderNumber = " & CleanNumeric(Session("OrderNumber"))& ""
...
ErrorLog("MemberPastCount: " &  MemberPastCount & "")

MemberCount = (MemberCurrentCount + MemberPastCount)

ErrorLog("MemberCount: " &  MemberCount & "")
...
If TicketQuantity > 0 Then
    ...
    SQLLineNo = "SELECT TOP(" & TicketQuantity & ") LineNumber FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber INNER JOIN EVENT (NOLOCK) ON Seat.EventCode = Event.EventCode WHERE OrderLine.OrderNumber = " & Session("OrderNumber") & " AND Seat.EventCode <> " & CleanNumeric(846795)& " ORDER BY Price DESC"
