...
'2015 Supporter
'------------
'All Events: 	General Admission tickets discounted down to the same price as the membership ticket for the event


'2015 Friend
'------------
'Regular: 		2 General Admission tickets discounted  to 100% 
'All Events: 	General Admission tickets discounted down to the same price as the membership ticket for the event


'2015 Filmmaker
'------------
'Regular: 		4 General Admission tickets discounted  to 100% 
'All Events: 	General Admission tickets discounted down to the same price as the membership ticket for the event


'2015 Sponsor
'------------
'Regular: 		6 General Admission tickets discounted  to 100%
'Opening:  		2 General Admission tickets discounted  to 100% for this event.
'All Events: 	General Admission tickets discounted down to the same price as the membership ticket for the event


'2015 Patron
'------------
'Regular: 		8 General Admission tickets discounted  to 100%
'Opening:  		2 General Admission tickets discounted  to 100% for this event.
'Closing:  		2 General Admission tickets discounted  to 100% for this event.
'All Events: 	General Admission tickets discounted down to the same price as the membership ticket for the event


'2015 Benefactor
'------------
'Regular:    	10 General Admission tickets discounted  to 100%
'VIP: 			2 General  Admission tickets discounted  to 100% for this event.
'Closing:    	2 General  Admission tickets discounted  to 100% for this event.
'All Events: 	General Admission tickets discounted down to the same price as the membership ticket for the event
...
'------------
'Regular:		1 General Admission ticket discounted  to 100% for each event. Max 1 ticket per event.
'VIP: 			1 General Admission ticket discounted  to 100% for this event.
'Closing:  		1 General Admission ticket discounted  to 100% for this event.
'All Events: 	General Admission tickets discounted down to the same price as the membership ticket for the event


'2015 Festival Guest
'------------
'Regular:		1  General Admission ticket discounted to 100% for each event. Max 1 ticket per event. Max of 15 tickets total.
'Opening:  		1  General Admission ticket discounted to 100% for this event.
'Closing:  		1  General Admission ticket discounted to 100% for this event.
'All Events: 	General Admission tickets discounted down to the same price as the membership ticket for the event
...
DIM WeekEvent
WeekEvent = "745608" 'Weekend Event
...
If AllowDiscount = "Y" Then 'It's okay to apply to this order.
	...
	'Process Free Regular Event Tickets (unlimited discounted tickets per event)
	...
	If SingleCount > 0 Then
		...
		If EventCode NOT IN (VIPEvent,OpenEvent,CloseEvent,WeekEvent)
					
			'ErrorLog("ActCode: " & ActCode & " - " & Act & "")
						
			'Count the number of discounted tickets for this event code
			SQLApplied = "SELECT COUNT(OrderLine.LineNumber) AS TicketCount FROM OrderLine (NOLOCK) INNER JOIN OrderHeader (NOLOCK) ON OrderLine.OrderNumber = OrderHeader.OrderNumber INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber INNER JOIN Event (NOLOCK) ON Seat.EventCode = Event.EventCode WHERE (CustomerNumber = " & CustomerNumber & " AND EventCode = " & EventCode & " AND OrderLine.DiscountTypeNumber = " & DiscountTypeNumber & " AND OrderLine.StatusCode = 'S') OR (OrderLine.OrderNumber = " & OrderNumber & " AND EventCode = " & EventCode & " AND DiscountTypeNumber = " & DiscountTypeNumber & " AND OrderLine.ItemType IN ('Seat', 'SubSeat', 'SubFixedEvent'))"
			Set rsApplied = OBJdbConnection.Execute(SQLApplied)	
				AppliedCount = rsApplied("TicketCount")		
			rsApplied.Close
			Set rsApplied = nothing
			
			'Determine number of allowed discounts for this act code
			'ErrorLog("FullSub: " & ActCode & "-" & Act & "-" & AppliedCount &"")
			
			'Count the total number of discounted tickets
			SQLDiscCount = "SELECT COUNT(OrderLine.LineNumber) AS LineCount FROM OrderLine (NOLOCK) INNER JOIN OrderHeader WITH (NOLOCK) ON OrderLine.OrderNumber = OrderHeader.OrderNumber INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber AND OrderLine.ItemType = 'Seat' INNER JOIN Event(NOLOCK) ON Seat.EventCode = Event.EventCode WHERE OrderHeader.CustomerNumber = " & CustomerNumber & " AND Seat.EventCode NOT IN (" & VIPEvent & "," & OpenEvent & "," & CloseEvent &"," & WeekEvent &") AND OrderLine.DiscountTypeNumber = " & DiscountTypeNumber & " AND OrderLine.Price=OrderLine.Discount"
			Set rsDiscCount = OBJdbConnection.Execute(SQLDiscCount)
			...
			If MaxDisc > 0 Then
				sqlWhere = "AND TotalDisc < MaxDisc"
			Else
				sqlWhere = ""
			End If
						
			If AppliedCount < SingleCount & sqlWhere Then	
					'ErrorLog("RegularCount less then 0?: " & RegularCount & "")
					NewPrice = round(CDbl(Clean(Request("Price"))) * (1-CDbl(Clean(Request("Percentage")))/100), 2)
			Else
				If AddDisc Then
	...
	'Process Free and Discounted Open VIP Tickets
	...
	'Process Discounted Weekend Tickets

		If EventCode = WeekEvent Then
