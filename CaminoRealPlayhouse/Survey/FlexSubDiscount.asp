...
'check OrderHeader OrderDate for Re-Open Orders or new ones
SQLOrderDate = "SELECT OrderDate FROM OrderHeader (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber")
Set rsOrderDate = OBJdbConnection.Execute(SQLOrderDate)
If Not rsOrderDate.EOF Then
    If DateDiff("d",FormatDateTime(Now(),2),DateAdd("d",1,FormatDateTime(rsOrderDate("OrderDate"),2))) >= 0 Then  'OrderDate is today
        rsOrderDate.Close
        Set rsOrderDate = Nothing
        Response.Redirect("FlexSubDiscount6Act.asp")
    End If
End If
rsOrderDate.Close
Set rsOrderDate = Nothing
...
If CountAct < 7 Then 'Get PREVIEW nights
