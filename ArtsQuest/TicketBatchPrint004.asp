...
<TABLE WIDTH="600">
  <TR>
    <TD><FONT SIZE="1" COLOR="#FFFFFF">
		...
		SQLTickets = "SELECT OrderLine.OrderNumber, OrderLine.ShipCode, OrderLine.LineNumber, Event.EventCode, PrintFormat.Format FROM OrderLine WITH (NOLOCK) INNER JOIN Seat WITH (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber INNER JOIN Event WITH (NOLOCK) ON Seat.EventCode = Event.EventCode INNER JOIN OrganizationVenue ON Event.VenueCode = OrganizationVenue.VenueCode INNER JOIN OrganizationAct ON Event.ActCode = OrganizationAct.ActCode INNER JOIN EventOptions ON Event.EventCode = EventOptions.EventCode INNER JOIN PrintFormat ON EventOptions.OptionValue = PrintFormat.FormatNumber"
		...
		'REE 4/6/2 - Modified to use OrganizationAct in selection criteria
		SQLTickets = SQLTickets & SQLWhere & ") AND OrderLine.StatusCode = 'S' AND OrganizationVenue.OrganizationNumber = " & Session("OrganizationNumber") & " AND OrganizationAct.OrganizationNumber = " & Session("OrganizationNumber") & " AND OrderLine.ItemType = 'Seat'  AND EventOptions.OptionName = 'TicketFormatNumber' AND OrderLine.OrderNumber IN (95786, 95788, 95832, 95850, 95943, 95952, 95956, 95958, 95964, 95982, 95989, 96025, 96040, 96049, 96055, 96059, 96062, 96096, 96112, 96122, 96133, 96136, 96156, 96159, 96168, 96179, 96181, 96182, 96185, 96199, 96204, 96217, 96238, 96246, 96253, 96259) ORDER BY OrderLine.OrderNumber, OrderLine.ShipCode, Seat.EventCode, Seat.SectionCode, RowSort, SeatSort DESC"
		Set rsTickets = OBJdbConnection.Execute(SQLTickets)
