...
<TABLE WIDTH="600">
  <TR>
    <TD><FONT SIZE="1" COLOR="#FFFFFF">
		...
		'REE 4/6/2 - Modified to use OrganizationAct in selection criteria
		SQLTickets = SQLTickets & SQLWhere & ") AND OrderLine.StatusCode = 'S' AND OrderLine.PrintDate IS NULL AND OrganizationVenue.OrganizationNumber = " & Session("OrganizationNumber") & " AND OrganizationAct.OrganizationNumber = " & Session("OrganizationNumber") & " AND OrderLine.ItemType = 'Seat'  AND EventOptions.OptionName = 'TicketFormatNumber' AND OrderLine.OrderNumber >= 77361 AND OrderLine.OrderNumber <= 83050 ORDER BY OrderLine.OrderNumber, OrderLine.ShipCode, Seat.EventCode, Seat.SectionCode, RowSort, SeatSort DESC"
		Set rsTickets = OBJdbConnection.Execute(SQLTickets)
