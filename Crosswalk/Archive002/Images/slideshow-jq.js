...
function instantiateHeaderSlideshow(options) {
	var slideshowElement = $('.wsite-header').css('background','#000')[0];
	if (slideshowElement) {
...
function Slideshow(element, options) {
	...
	function renderContent() {
		...
		if (photos.length > 1) {
			...
			if (!options.links && options.arrows !== false) {
	...
	function renderLink(photo, i) {
		...
		if (isTouch) {
			...
			var decodedCaption = $('<textarea />').html(photo.caption || '').val();
			a.attr('href', largeURL(photo))
			 .attr('title', decodedCaption);
		}
		else {
			a.on('click', function() {
				pause();
	...
	function go(newIndex, forceDirection, noTransition) { // forceDirection = 1 (forward) or -1 (backward)
		...
		if (newIndex != slideIndex) {
			...
			if (noTransition || oldIndex === undefined) {
				...
				// IE8 and below has a bug if we change opacity of entire slide.
				// so, let's just fade the image + caption individually
				var oldOpacityElements = slides[oldIndex].find('.wslide-slide-inner2').add(slides[oldIndex].find('.wslide-caption'));
