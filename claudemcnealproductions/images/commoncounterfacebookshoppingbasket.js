...
apsinth.ApsinthModule=Modul.extend({CONFIG_MIN_WIDTH:550,proxy:null,static_proxy:null,handler_proxy:null,view_proxy:null,configSaveHandler:"config",mainView:"main",data:null,data_admin:null,configIsOpen:false,appendError:false,errorTarget:"auto",fieldErrorsTarget:"auto",loadMainViewAfterSave:true,ajaxUtil:null,initialize:function(e,d,b,g){try{apsinth.util.EventingMixin.mixin(this);
this.instance_id=d;this.module_name=b;this.mode=g;this.internalContainerId="NGH"+e;this.idPrefix=this.internalContainerId;
var f=window["__NGHModuleInstanceData"+e];this.moduleServer=f.server;this.data=f.data_web;this.data_admin=f.data_admin;
...
if(mm[this.moduleId] instanceof Modul){a._diyNewlyAdded=mm[this.moduleId].getIsNew()?1:0}}this.ajaxUtil.rpcRequest(this.proxy+"/rpc/",d,a,e,b)
},sendData:function(b,c,d,a){this.ajaxUtil.postRequest(this.getHandlerUrl(b),c,d,a)},onError:function(b,a){apsinth.util.ErrorUtil.onError(b,a)
},save:apsinth.util.ErrorUtil.wrap(function(){jQuery(".error-msg").hide();this._removeDefaultInputValues();
if(!this.validateConfig()){this._initDefaultInputValues();return}var a={data:this.getConfigData(),mod_saveEdit:1};
...
},validateConfig:function(){var b=true;var c=this.getConfigJQ();var a={badge:c.find('input[name="badge"]').is(":checked")?1:0,badge_show_photo:c.find('input[name="badge_show_photo"]').is(":checked")?1:0,badge_show_name:c.find('input[name="badge_show_name"]').is(":checked")?1:0,badge_show_fans:c.find('input[name="badge_show_fans"]').is(":checked")?1:0};
...
b=false}return b},getConfigData:function(){var b=this.getConfigJQ();var a={like_box:b.find('input[name="like_box"]').is(":checked")?1:0,page_url:b.find('input[name="page_url"]').val(),show_faces:b.find('input[name="show_faces"]').is(":checked")?1:0,show_stream:b.find('input[name="show_stream"]').is(":checked")?1:0,like_button:b.find('input[name="like_button"]').is(":checked")?1:0,color:b.find('select[name="color"]').val(),badge:b.find('input[name="badge"]').is(":checked")?1:0,badge_url:b.find('input[name="badge_url"]').val(),badge_show_photo:b.find('input[name="badge_show_photo"]').is(":checked")?1:0,badge_show_name:b.find('input[name="badge_show_name"]').is(":checked")?1:0,badge_show_fans:b.find('input[name="badge_show_fans"]').is(":checked")?1:0};
return a},_injectFacebookScript:function(e,a,f){var c,b=e.getElementsByTagName(a)[0];if(e.getElementById(f)){return
}c=e.createElement(a);c.id=f;c.src="//connect.facebook.net/"+(FacebookModule.msg.all&&FacebookModule.msg.all.facebookLocale?FacebookModule.msg.all.facebookLocale:apsinth.lang)+"/sdk.js#xfbml=1&version=v2.3";
b.parentNode.insertBefore(c,b)},_initFacebookSDK:function(){if(false==FacebookModule.jsInjected){jQuery(document.body).append('<div id="fb-root"></div>');
...
var b=g.width();var e=g.find(".fb-page");var a=g.find(".fb-like");var d=g.find(".facebook-badge");this.initMainViewBadges(d,b);
if(e.length>0||a.length>0){this._initFacebookSDK()}if(window.FB){this.initMainViewWidgets(e,a,b)}else{var h=document.getElementById("facebook-jssdk");
var f=this;var c=function(){if(!this.readyState||this.readyState=="loaded"||this.readyState=="complete"){f.toHandler(function(){f.initMainViewWidgets(e,a,b)
...
c.css("overflow","hidden")}if(b){b.attr("data-width",a);var d=35;if(a<225){d=20;b.attr("data-layout","button_count")
}else{if(a<450){d=60}}FB.Event.subscribe("xfbml.render",function(){var e={width:a+"px",height:d+"px"};
b.find("iframe").add(b).css(e);c.find("iframe").add(c).css("width",a+"px")})}if(this.mode=="admin"&&window.FB){window.FB.init({version:"v2.3"});
window.FB.XFBML.parse(this.getMainJQ().get(0))}},_initConfigView:function(){var a=this.getConfigJQ();
this.disableToggleInit("like_box","likeBox");this.disableToggleInit("like_button","likeButton");this.disableToggleInit("badge","badge");
jQuery(".wrapper-color",a).diy_combobox({editable:false});a.find(".form-item input.checkbox").diy_checkbox();
...
if(!d.is(":checked")){c.prop("disabled",true);c.filter('input[type="checkbox"]').diy_checkbox({disabled:true})
}d.change(function(f){if(d.is(":checked")){c.prop("disabled",false);c.filter('input[type="checkbox"]').diy_checkbox({disabled:false})
}else{c.prop("disabled",true);c.filter('input[type="checkbox"]').diy_checkbox({disabled:true})}})},loadBadge:function(b,c){var d="https://graph.facebook.com/"+c;
var a=jQuery("#"+b);if(!a){return}jQuery.jsonp({url:d,callbackParameter:"callback",callback:b,success:this.toHandler(function(h){if(h){var k=h.id;
var e=h.name;var g="//graph.facebook.com/"+k+"/picture?type=normal";var f=h.likes;var j=a.find(".profile-image");
...
var a=/(\d+)(\d{3})/;while(a.test(x1)){x1=x1.replace(a,"$1,$2")}return x1+x2}});
