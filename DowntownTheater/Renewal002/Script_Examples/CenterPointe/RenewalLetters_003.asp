		...
		<DIV class="panel-body" id="panel-options-outer">
			...
			<DIV class="container-fluid effect1" id="panel-options-inner"> 
				...
				If EventList <> "" Then
					...
					<FORM ACTION="RenewalLetters_003.asp" METHOD="post">
...
TemplateFilePath = Server.MapPath("/Clients/CenterPointTheatre/Renewal/Templates/Template_Letter.asp")
...
Do Until rsOrderLine.EOF ' Or LetterCount >= 1
	...
	'This is not the last order.
		...
		If Not rsCustInfo.EOF Then
			...
			Response.Write InsertTemplate(TemplateFilePath,"CoverPage") 
			
			'-----------------------------------------
						
			'Insert HTML Template - Masthead Content
			Response.Write InsertTemplate(TemplateFilePath,"Header") 
...
If TicketCount > 0 Then	'Print the last footer
	...
	'Footer Info
	'Insert HTML Template - Footer Content
	 Response.Write InsertTemplate(TemplateFilePath,"Footer") 
...
PUBLIC FUNCTION replaceVar(strText)
		...
		strNewText = Replace(strText, 	 "[CustomerFName]", CustomerFName)
		strNewText = Replace(strText, 	 "[CustomerLName]", CustomerLName)
