...
DIM ETicketURL
ETicketURL = "http://www.testtix.com/ETicketPrint.asp?"
...
Do Until rsOrderLine.EOF
	...
	'This is not the last order.
		...
		If Not rsCustInfo.EOF Then
			...
			ReturnAddress = ReturnAddress & "<FONT FACE=" & MsgFontFace & ">Dear " & PCase(rsCustInfo("ShipFirstName")) & " " & PCase(rsCustInfo("ShipLastName")) & "" & vbCrLf
			...
			SQLETicket = "SELECT Ticket.OrderNumber, TicketNumber FROM OrderLine (NOLOCK) INNER JOIN Ticket (NOLOCK) ON OrderLine.ItemNumber = Ticket.ItemNumber AND OrderLine.OrderNumber = Ticket.OrderNumber WHERE OrderLine.OrderNumber = " & rsOrderLine("OrderNumber") & " AND OrderLine.ShipCode = 13 AND OrderLine.StatusCode = 'S' AND Ticket.StatusCode = 'A'"
			Set rsETicket = OBJdbConnection.Execute(SQLETicket)
				If Not rsETicket.EOF Then
					OrderNumber = rsOrderLine("OrderNumber")
					TicketNumber = rsETicket("TicketNumber")
					ETicketURL = ETicketURL & "OrderNumber=" & OrderNumber & "&TicketNumber=" & TicketNumber & ""
				End If
			rsETicket.Close
