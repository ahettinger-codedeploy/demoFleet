...
'REE 2/11/6 - Created UserType variable.
...
ClientDirectory = "/Clients/OmegaEvents/Renewal"

TemplateFileName = "Template_ETicketReminder.asp"

DIM RenewalURL
RenewalURL = "http://omegaevents.tix.com/renew.aspx?"
...
DIM MsgFontFace
MsgFontFace = "verdana, arial,helvetica,sans-serif"
...
DIM EMailReplyTo
EMailReplyTo = "tickets@hyattconcerts.com"

DIM EMailSubject
EMailSubject = "ETicket Reminder"
...
'TestEMailAddress = "sara@omegaevents.com"
...
<title>Tix - Ticket Sales Menu</title>

<link rel="stylesheet" type="text/css" href="https://oss.maxcdn.com/icheck/1.0.2/skins/all.min.css" >

<link rel="stylesheet" type="text/css" href="https://www.testtix.com/assets/app.css">
...
.table tbody tr:hover.odd td,
.table tbody tr:hover.even td,

.table tbody tr.active:hover.odd td,
.table tbody tr.active:hover.even td,
		...
		<div class="panel panel-default" id="panel-parent">
						...
						<div class="pull-left">
							<ul class="nav nav-tabs">
								...
								<li class="active">
								  ...
								  <span class="icon-label">Reminder Email</span>
								  </a>
								</li>
			...
			If EventList <> "" Then
				...
				If Not rsTickets.EOF Then 'There are some events.  List them
					Response.Write "<FORM ACTION=""" & fnScriptName & """ METHOD=""post""><INPUT TYPE=""hidden"" NAME=""FormName"" VALUE=""PrintMenu"">" & vbCrLf
					Response.Write "<TABLE class=""table table-striped"">" & vbCrLf
					Response.Write "<THEAD>" & vbCrLf
					Response.Write "<TR><TH>Print</TH><TH>Event<BR>Code</TH><TH>Date<BR>Time</TH><TH>Production<BR>Name</TH><TH>Venue<BR>Name</TH><TH>Order<BR>Quantity</TH></TR>" & vbCrLf
					Response.Write "</THEAD>" & vbCrLf
					...
					Do Until rsTickets.EOF
						...
						Response.Write "<TD><INPUT TYPE=""checkbox"" NAME=""EventCode"" VALUE=" & rsTickets("EventCode") & "></TD>" & vbCrLf
						Response.Write "<TD>" & rsTickets("EventCode") & "</TD>" & vbCrLf
						Response.Write "<TD>" & WeekDayName(Weekday(rsTickets("EventDate")),TRUE) & "&nbsp;" & FormatDateTime(rsTickets("EventDate"), vbShortDate) & "<BR>" & Left(FormatDateTime(rsTickets("EventDate"),vbLongTime),Len(FormatDateTime(rsTickets("EventDate"),vbLongTime))-6) & Right(FormatDateTime(rsTickets("EventDate"),vbLongTime),3) & "</TD>" & vbCrLf
						Response.Write "<TD>" & rsTickets("Act") & "</TD>" & vbCrLf
						
						Response.Write "<TD>" & rsTickets("Venue") & "</TD>" & vbCrLf
						Response.Write "<TD ALIGN=""center"">" & rsTickets("TicketCount") & "</TD>" & vbCrLf
						Response.Write "</TR>" & vbCrLf
					...
					Response.Write "Event Count: " & EventCount & "<BR><BR><BR>" & vbCrLf
...
Do Until rsOrderLine.EOF
	...
	'This is not the last order.
		...
		If TicketCount > 0 Then
			...
			TenderInfo = TenderInfo & "<TR><TD COLSPAN=""" & NumColumns - 1 & """>&nbsp;</TD><TD><HR></TD></TR>" & vbCrLf			
			If LastShipFee <> 0 And LastOrderTypeNumber <> 5 Then
				TenderInfo = TenderInfo & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Processing Fee</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastShipFee,2) & "</TD></TR>" & vbCrLf
				TenderInfo = TenderInfo & "<TR><TD COLSPAN=""" & NumColumns - 1 & """>&nbsp;</TD><TD><HR></TD></TR>" & vbCrLf
			End If
			...
			Else
				...
				TenderInfo = TenderInfo & SQLTender & "<BR>"
			End If
		
			rsTender.Close
			Set rsTender = nothing	

			TenderInfo = TenderInfo & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Order Total</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTotal,2) & "</TD></TR>" & vbCrLf
			If LastTender <> 0 Then
				TenderInfo = TenderInfo & "<TR><TD WIDTH=""50"">&nbsp;</TD><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=C" & MsgFontFace & ">Less Payments</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTender,2) & "</TD></TR>" & vbCrLf
				TenderInfo = TenderInfo & "<TR><TD WIDTH=""50"">&nbsp;</TD><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Balance Due</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTotal - LastTender,2) & "</TD></TR>" & vbCrLf
			End If
			
			TenderInfo = TenderInfo & "</TABLE>" & vbCrLf
			TenderInfo = TenderInfo & "</CENTER><BR>" & vbCrLf
			...
			Response.Write OrderCount & " | " & LastOrderNumber & " | " & LastEmailAddress & "<BR>" & vbCrLf
		...
		If Not rsCustInfo.EOF Then
			...
			ReturnAddress = ReturnAddress & "<tr style=""display:none !important; visibility:hidden; mso-hide:all; font-size:1px; color:#ffffff; line-height:1px; max-height:0px; max-width:0px; opacity:0; overflow:hidden;"">" & vbCrLf
			ReturnAddress = ReturnAddress & "<td style=""width:100%;"">" & vbCrLf
			ReturnAddress = ReturnAddress & "Your 2016 Summer Concert Series Ticket Order.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" & vbCrLf    
			ReturnAddress = ReturnAddress & "</td>" & vbCrLf
			ReturnAddress = ReturnAddress & "</tr>" & vbCrLf
			
			ReturnAddress = ReturnAddress & "<TR><TD>" & vbCrLf
			
			ReturnAddress = ReturnAddress & "<IMG SRC=""https://www.testtix.com/Clients/OmegaEvents/Renewal/Images/2016_SummerConcertSeries_Outlined.png""><BR><BR>" & vbCrLf
			ReturnAddress = ReturnAddress & "</TD></TR>" & vbCrLf
			
			ReturnAddress = ReturnAddress & "<TR><TD>" & vbCrLf
			
			ReturnAddress = ReturnAddress & "<BR><BR><BR><FONT FACE=" & MsgFontFace & ">Dear " & PCase(rsCustInfo("ShipFirstName")) & " " & PCase(rsCustInfo("ShipLastName")) & "" & vbCrLf
						
			ReturnAddress = ReturnAddress & "</TD></TR>" & vbCrLf
			ReturnAddress = ReturnAddress & "</TABLE>" & vbCrLf
			
			FirstName = PCase(rsCustInfo("ShipFirstName"))
			LastName = PCase(rsCustInfo("ShipLastName"))
			...
			SQLItemNumber = "SELECT OrderLine.ItemNumber, Ticket.TicketNumber FROM OrderLine (NOLOCK) INNER JOIN Ticket (NOLOCK) ON OrderLine.ItemNumber = Ticket.ItemNumber AND OrderLine.OrderNumber = Ticket.OrderNumber WHERE OrderLine.OrderNumber = " & rsOrderLine("OrderNumber") & " AND OrderLine.StatusCode = 'S' AND Ticket.StatusCode = 'A' ORDER BY ItemNumber"
			Set rsItemNumber = OBJdbConnection.Execute(SQLItemNumber)
				TicketNumber = rsItemNumber("TicketNumber")
			rsItemNumber.Close
			Set rsItemNumber = nothing
			
			ETicketURL = "https://www.tix.com/ETicketFormat.asp?OrderNumber=" & rsOrderLine("OrderNumber") & "&TicketNumber=" & TicketNumber & ""
			
			'OrderRenewal = ""
			'OrderRenewal = OrderRenewal & "<center>" & vbCrLf
			'OrderRenewal = OrderRenewal & "    <table border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbCrLf
			'OrderRenewal = OrderRenewal & "        <tr>" & vbCrLf
			'OrderRenewal = OrderRenewal & "            <td align="center" style=""-webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px;"" bgcolor=""#C01F18"">" & vbCrLf
			'OrderRenewal = OrderRenewal & "<a href=""" & ETicketURL & """ target=""_blank"" style=""font-size: 13px; font-family: Helvetica, Arial, sans-serif; color: #ffffff; text-decoration: none; color: #ffffff; text-decoration: none; -webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px; padding: 12px 18px; border: 1px solid #C01F18; display: inline-block;"">" & vbCrLf
			'OrderRenewal = OrderRenewal & "ETickets" & vbCrLf
			'OrderRenewal = OrderRenewal & "</a>" & vbCrLf
			...
			'OrderRenewal = OrderRenewal & "<FONT FACE=""" & MsgFontFace & """><B>Order Number:</B>&nbsp;" & rsOrderLine("OrderNumber") & "" & vbCrLf
			'OrderRenewal = OrderRenewal & "</TD>" & vbCrLf
			'OrderRenewal = OrderRenewal & "<TD ALIGN=""right"">" & vbCrLf		
			'OrderRenewal = OrderRenewal & "<FONT FACE=""" & MsgFontFace & """><B>Renewal Code:</B>&nbsp;" & rsCustInfo("CustomerNumber") & ItemNumber & "" & vbCrLf
			'OrderRenewal = OrderRenewal & "</TD>" & vbCrLf
			'OrderRenewal = OrderRenewal & "</TR>" & vbCrLf
			'OrderRenewal = OrderRenewal & "<TR>" & vbCrLf
			'OrderRenewal = OrderRenewal & "<TD ALIGN=""left"">" & vbCrLf
			'OrderRenewal = OrderRenewal & "<FONT FACE=""" & MsgFontFace & """><B>Customer Number:</B>&nbsp;" & rsCustInfo("CustomerNumber") & "" & vbCrLf
			'OrderRenewal = OrderRenewal & "</TD>" & vbCrLf
			'OrderRenewal = OrderRenewal & "<TD ALIGN=""right"">" & vbCrLf		
			'OrderRenewal = OrderRenewal & "<FONT FACE=""" & MsgFontFace & """><B>Item Number :</B>&nbsp;" & ItemNumber & "" & vbCrLf
			'OrderRenewal = OrderRenewal & "</TD>" & vbCrLf
			'OrderRenewal = OrderRenewal & "</TR>" & vbCrLf
			'OrderRenewal = OrderRenewal & "</TABLE>" & vbCrLf
	...
	If LastEventCode <> rsOrderLine("EventCode") Or LastOrderNumber <> rsOrderLine("OrderNumber") Then
		...
		ProductionInfo = ProductionInfo & "<TABLE WIDTH=""650"" cellpadding=""0"" cellspacing=""0""  border=""0"">" & vbCrLf
		ProductionInfo = ProductionInfo & "<TR><TD>" & vbCrLf
		ProductionInfo = ProductionInfo & "<FONT FACE=""" & MsgFontFace & """><B>" & rsOrderLine("Act") & "<br>" & vbCrLf
		ProductionInfo = ProductionInfo & "at " & rsOrderLine("Venue") & "<br>" & vbCrLf
		...
		If rsDateSuppress.EOF Then
			ProductionInfo = ProductionInfo & " on " & FormatDateTime(rsOrderLine("EventDate"),vbShortDate)
		End If
		...
		If rsTimeSuppress.EOF Then
			ProductionInfo = ProductionInfo & " at " & Left(FormatDateTime(rsOrderLine("EventDate"),vbLongTime),Len(FormatDateTime(rsOrderLine("EventDate"),vbLongTime))-6) & Right(FormatDateTime(rsOrderLine("EventDate"),vbLongTime),3)
		End If
		rsTimeSuppress.Close
		Set rsTimeSuppress = nothing

		ProductionInfo = ProductionInfo & "</B><BR>Delivery Method: " & rsOrderLine("ShipType") & "</TD></TR>" & vbCrLf
		ProductionInfo = ProductionInfo & "</TABLE>" & vbCrLf
		
		ProductionInfo = ProductionInfo & "<BR>" & vbCrLf
		
		'EMailMessage = EMailMessage & ProductionInfo
		...
		'-----------------------------------------
			...
			If SHOWCHILD Then
				...
				If Not rsEventChild.EOF Then
					...
					ChildEvents = ChildEvents & "<FONT FACE=" & MsgFontFace & "><B>This subscription includes:</B>" & vbCrLf
					...
					ChildEvents = ChildEvents & "</tr>" & vbCrLf
						...
						Do While Not rsEventChild.EOF
							...
							ChildEvents = ChildEvents & "<FONT FACE=" & MsgFontFace & ">" & rsEventChild("Act") & "" & vbCrLf	
							ChildEvents = ChildEvents & "</td>" & vbCrLf	
							ChildEvents = ChildEvents & "<td WIDTH=""40%"" NOWRAP>" & vbCrLf
							ChildEvents = ChildEvents & "<FONT FACE=" & MsgFontFace & ">" & FormatDateTime(rsEventChild("EventDate"), vbLongDate) & " at " & Left(FormatDateTime(rsEventChild("EventDate"),vbLongTime),Len(FormatDateTime(rsEventChild("EventDate"),vbLongTime))-6) & Right(FormatDateTime(rsEventChild("EventDate"),vbLongTime),3) & "" & vbCrLf
							ChildEvents = ChildEvents & "</td>" & vbCrLf
							ChildEvents = ChildEvents & "</tr>" & vbCrLf
...
If TicketCount > 0 Then	'Print the last footer
	...
	If TestEMailAddress = "" Then
			...
			If LastTender <> 0 Then
				TenderInfo = TenderInfo & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Less Payments</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTender,2) & "</TD></TR>" & vbCrLf
				TenderInfo = TenderInfo & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Balance Due</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTotal - LastTender,2) & "</TD></TR>" & vbCrLf
			End If
		...
		objCDOSYSMail.ReplyTo = EMailReplyTo
		objCDOSYSMail.To = LastEMailAddress
		objCDOSYSMail.BCC = "sergio.rodriguez@tix.com"
		objCDOSYSMail.Subject = EMailSubject
		objCDOSYSMail.HTMLBody = EMailMessage
		objCDOSYSMail.Send
...
PUBLIC FUNCTION replaceVar(strText)
		...
		strNewText = Replace(strText,    "[FirstName]", FirstName)
		strNewText = Replace(strText,    "[LastName]", LastName)
		...
		strNewText = Replace(strNewText, "[ETicketURL]", ETicketURL)
