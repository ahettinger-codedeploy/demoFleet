...
'SSR 11/22/13 - Added producer to E-Ticket.  If producer found in query then producer will display otherwise "org name presents" will be used
...
'SSR 04/12/13 - If no image links in event options, then check local directory for images: ETicketBackground, ETicketLogo, and ETicketBottomBar, if not found, use tix default images.
'SSR 04/08/14 - Added E-Ticket General Information
'REE 06/13/14 - Parameterized first query.
'SSR 06/17/14 - Hard Coded Default options for ETicketBackground, ETicketLogo, ETicketBottomBar, General Info, Driving Directions and Ad
'SSR 07/16/14 - Ad Image: if found, use Ticket Text 1 for Ad Image, otherwise use image in client folder, otherwise, check for link in event options, otherwise use tix default image
%>
...
DIM Default_ETicketAdPath
Default_ETicketAdPath = ""

DIM Default_ETicketProducer
Default_ETicketProducer = ""
...
If Not rsOrderNum.EOF Then
	...
	If Not rsTix.EOF Then
		...
		'Begin Ticket
			...
			'Check Client ETicket Images Folder
			If GetLocalImage(Server.MapPath("Images/ETicketBackground.gif")) Then
				ETicketBackground = Server.MapPath("Images/ETicketBackground.gif")
			'Check Client ETicket Folder
			ElseIf GetLocalImage(Server.MapPath("ETicketBackground.gif")) Then
				ETicketBackground = Server.MapPath("Images/ETicketBackground.gif")
			'Check if hard coded
            ElseIf Default_ETicketBackground <> "" Then
			     ETicketBackground = Default_ETicketBackground
			'Check Event Options
            ElseIf Not IsNull(rsTix("ETicketBackgroundPath")) Then
                ETicketBackground = rsTix("ETicketBackgroundPath")
			Else
			'Check TIX Images Folder
			...
			'Check Client ETicket Folder
			If GetLocalImage(Server.MapPath("ETicketLogo.gif")) Then
				ETicketLogoPath = Server.MapPath("Images/ETicketLogo.gif")
			'Check Client ETicket Images Folder
			ElseIf GetLocalImage(Server.MapPath("Images/ETicketLogo.gif")) Then
				ETicketLogoPath = Server.MapPath("Images/ETicketLogo.gif")
			'Check if hard coded
            ElseIf Default_ETicketLogoPath <> "" Then
			     ETicketLogoPath = Default_ETicketLogoPath
			'Check Event Options
            ElseIf Not IsNull(rsTix("ETicketLogoPath")) Then
                ETicketLogoPath = rsTix("ETicketLogoPath")
			Else
			'Check TIX Images Folder
				ETicketLogoPath = "/Images/ETicketLogo.gif"
            End If
			
			Response.Write "<IMG SRC="" " & ETicketLogoPath & " "">" & vbCrLf
			...
			If rsTix("Producer") <> "" Then 
				ETicketProducer = rsTix("Producer")
			ElseIf Default_ETicketProducer <> "" Then
				ETicketProducer = Default_ETicketProducer
			Else
			...
			'Check Client ETicket Images Folder
			If GetLocalImage(Server.MapPath("Images/ETicketBottomBar.gif")) Then
				ETicketBottomBarPath = Server.MapPath("Images/ETicketBottomBar.gif")
			'Check Client ETicket Folder
			ElseIf GetLocalImage(Server.MapPath("ETicketBottomBar.gif")) Then
				ETicketBottomBarPath = Server.MapPath("ETicketBottomBar.gif")
			'Check Event Options
            ElseIf Not IsNull(rsTix("ETicketBottomBarPath")) Then
				ETicketBottomBarPath = rsTix("ETicketBottomBarPath")
			'Check if hard coded
            ElseIf Default_ETicketBottomBarPath <> "" Then
			     ETicketBottomBarPath = Default_ETicketBottomBarPath
			Else
			'Check TIX Images Folder
				ETicketBottomBarPath = "/Images/ETicketBottomBar.gif"
            End If
			
			Response.Write "<IMG SRC="" " & ETicketBottomBarPath & " "">" & vbCrLf
		...
		'Begin Ad Space
			...
			'Check Ticket Text 1
			If GetEventOption("TicketText1") <> "" Then
				Response.Write "<FONT FACE=""Arial, Helvetica"" SIZE=""2"">" & vbCrLf
				Response.Write GetEventOption("TicketText1")
				Response.Write "</FONT>" & vbCrLf
			'Check hard coded Default
            ElseIf Default_ETicketAdPath <> "" Then
			   Response.Write "<IMG SRC=""" & rsTix("ETicketAdPath") & """>" & vbCrLf
			'Check Client local images folder
			ElseIf GetLocalImage(Server.MapPath("Images/ETicketAd.gif")) Then
				ETicketAdPath = Server.MapPath("Images/ETicketAd.gif")
				Response.Write "<IMG SRC=""" & ETicketAdPath & """>" & vbCrLf
			'Check Client local folder
			ElseIf GetLocalImage(Server.MapPath("ETicketAd.gif")) Then
				ETicketAdPath = Server.MapPath("ETicketAd.gif")
				Response.Write "<IMG SRC=""" & ETicketAdPath & """>" & vbCrLf
			'Check Event Options
			ElseIf IsNull(rsTix("ETicketAdPath")) Then
               Response.Write "<IMG SRC=""" & rsTix("ETicketAdPath") & """>" & vbCrLf
			Else
			'Check TIX Images Folder
		...
		'End Ad Space
			...
			'Insert Vouchers
			 Response.Write InsertTemplate("ETicketInclude.asp","Header") 
...
PUBLIC FUNCTION GetLocalImage(byVal pathname)
	...
	GetLocalImage = fileFind
	
END FUNCTION

'-----------------------------------------

PUBLIC FUNCTION GetEventOption(strOptionName)

	DIM strTemp

	SQLEventOptionChk = "SELECT OptionValue FROM EventOptions WITH (NOLOCK) WHERE EventCode = " & rsTix("EventCode") & " AND (OptionName = '" & strOptionName & "')"
	Set rsEventOptionChk = OBJdbConnection.Execute(SQLEventOptionChk)
		strTemp = rsEventOptionChk("OptionValue")
	rsEventOptionChk.Close
	Set rsEventOptionChk = nothing
	
	GetEventOption = strTemp
