...
If Not rsOrderNum.EOF Then
	...
	If Not rsTix.EOF Then
		...
		'-----------------------------------------
			...
			DIM VoucherDiscountNumber
			VoucherDiscountNumber = "89526"
			
			DIM VoucherEventList
			VoucherEventList = "479444,632602,653550"
			
			errorlog("OrderNumber " & rsTix("OrderNumber") & "")
							
			'Check to see if this order has required discount code
			SQLDiscountCheck = "SELECT COUNT(OrderLine.ItemNumber) AS Count FROM OrderLine (NOLOCK) INNER JOIN OrderHeader (NOLOCK) ON OrderLine.OrderNumber = OrderHeader.OrderNumber WHERE OrderLine.OrderNumber = " & rsTix("OrderNumber") & " AND OrderLine.DiscountTypeNumber = " & VoucherDiscountNumber & ""
			Set rsDiscountCheck = OBJdbConnection.Execute(SQLDiscountCheck)
				If rsDiscountCheck("Count") > 0 Then 'Other discount found
					PrintVoucher = True
				End If
			rsDiscountCheck.Close
			Set rsDiscountCheck = nothing
			
			If PrintVoucher Then
					
				SQLEventList = "SELECT DISTINCT event.eventcode FROM   orderheader (nolock) INNER JOIN orderline (nolock) ON orderheader.ordernumber = orderline.ordernumber INNER JOIN seat (nolock) ON orderline.itemnumber = seat.itemnumber INNER JOIN event (nolock) ON seat.eventcode = event.eventcode WHERE  ( Orderheader.ordernumber = " & rsTix("OrderNumber") & " ) and seat.eventcode in (" & VoucherEventList & ")"	
				Set rsEventList = OBJdbConnection.Execute(SQLEventList)
					
					Do While Not rsEventList.EOF
					
						VoucherAct = rsTix("Act")
						VoucherDate = rsTix("EventDate")
						VoucherOrder = rsTix("OrderNumber") 
						VoucherName = rsTix("FirstName") & " " & rsTix("LastName")
					
						Voucher = InsertTemplate("VoucherInclude.asp","VoucherTemplate") 
...
PUBLIC FUNCTION replaceVar(strText)
		...
		strNewText = Replace(strText, 	 "[VoucherAct]", VoucherAct)
		strNewText = Replace(strNewText, "[VoucherDate]", VoucherDate)
		strNewText = Replace(strNewText, "[VoucherOrder ]", VoucherOrder)
		strNewText = Replace(strNewText, "[VoucherName]", VoucherName)
