...
'SSR 06/18/14 - Updated to allow colorful backgrounds
%>
...
If Not rsOrderNum.EOF Then
	...
	If Not rsTix.EOF Then
		...
		'Begin Ticket
			...
			Response.Write "<TABLE WIDTH=""615"" HEIGHT=""240"" BORDER=""1"" class=""ETicketBackground"" BGCOLOR="" " & GetETicketBGColor() & " "" CELLPADDING=""0"" CELLSPACING=""0"">" & vbCrLf
			Response.Write "<TR>" & vbCrLf
...
PUBLIC FUNCTION GetETicketBGColor()

	SQLBGCOLOR = "SELECT OptionValue FROM EventOptions (NOLOCK) WHERE EventCode = " & EventCode & " AND OptionName = 'TicketText1'"
	Set rsBGCOLOR = OBJdbConnection.Execute(SQLBGCOLOR)

		If Not rsBGCOLOR.EOF Then 
			BGCOLOR = rsBGCOLOR("OptionValue")
		Else
			BGCOLOR = "#FFD51D"
		End If

	rsBGCOLOR.Close
	Set rsBGCOLOR = nothing
	
	GetETicketBGColor = BGCOLOR
