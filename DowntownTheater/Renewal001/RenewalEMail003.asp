	...
	<div id="wrapper" class="row">
		...
		<div class="col-lg-12">
			...
			<div class="tab-content" style="padding:10px;">
				...
				<BR>			
				<% If TestEMailAddress <> "" Then %>
					Test Mode.  A test email will be sent to: <%=TestEMailAddress%><BR><BR>
				<% End If %>
				<BR>
				<%
					...
					If EventList <> "" Then
						...
						Else 'There aren't any matching events
							Response.Write "<BR><BR><FONT FACE=""verdana,arial,helvetica"" COLOR=""#000000"" SIZE=""2"">There are no events to be emailed.</FONT><BR><BR>" & vbCrLf
						End If
					Else 'There aren't any matching events
						Response.Write "<BR><BR><FONT FACE=""verdana,arial,helvetica"" COLOR=""#000000"" SIZE=""2"">There are no events to be emailed.</FONT><BR><BR>" & vbCrLf
					End If
...
Do Until rsOrderLine.EOF ' Or LetterCount >= 1
	...
	'This is not the last order.
		...
		If TicketCount > 0 Then 
			...
			If LastShipFee = 0 And LastOrderTypeNumber <> 5 Then
				EMailMessage = EMailMessage & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=""" & MsgFontFace & """>[&nbsp;&nbsp;] ADD $1 for mailing</TD><TD ALIGN=""right""><FONT FACE=""" & MsgFontFace & """>&nbsp;&nbsp;</TD></TR>" & vbCrLf
				EMailMessage = EMailMessage & "<TR><TD COLSPAN=""" & NumColumns - 1 & """>&nbsp;</TD><TD><HR></TD></TR>" & vbCrLf
			End If
			...
			EMailMessage = EMailMessage & InsertTemplate("LetterFooter.html") 
			...
			Response.Write " " & OrderCount & " | Order Number: " & LastOrderNumber & " | Email: " & LastEmailAddress & "<BR>" & vbCrLf

            If OrderCount Mod 100 = 0 Then
                Response.Flush
            End If
		...
		If Not rsCustInfo.EOF Then
			...
			EMailMessage = EMailMessage & InsertTemplate("LetterHeader.html") 
			
			'-----------------------------------------
						
			'Insert HTML Template - Masthead Content
			EMailMessage = EMailMessage & InsertTemplate("LetterMastHead.html") 
			...
			EMailMessage = EMailMessage & "<FONT FACE=""" & MsgFontFace & """><B>Order Number:</B>&nbsp;" & rsOrderLine("OrderNumber") & "" & vbCrLf
			EMailMessage = EMailMessage & "</TD>" & vbCrLf
			...
			EMailMessage = EMailMessage & "<B>Renewal Code:</B>&nbsp;" & RenewalCode & "" & vbCrLf
			EMailMessage = EMailMessage & "</TD>" & vbCrLf
			EMailMessage = EMailMessage & "</TR>" & vbCrLf
			EMailMessage = EMailMessage & "</TABLE>" & vbCrLf
			EMailMessage = EMailMessage & "<BR>" & vbCrLf
...
If TicketCount > 0 Then	'Print the last footer
	...
	EMailMessage = EMailMessage & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=""" & MsgFontFace & """>Total Order</TD><TD ALIGN=""right""><FONT FACE=""" & MsgFontFace & """>" & FormatCurrency(LastTotal,2) & "</TD></TR>" & vbCrLf
...
Response.Write "Total Count: " & OrderCount & "<BR>"
...
strNewText = ""
	...
	templatePath = Server.MapPath(templateName)
