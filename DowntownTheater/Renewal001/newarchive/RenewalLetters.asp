...
if request.form("FormName") <> "" then
	strForm =  request.form("FormName")
end if

DIM strMedia

If request.queryString("media") <> "" Then
	strMedia = request.queryString("media")
End If

If strMedia = "emails" then
	lettertab = ""
	emailtab = "active"
	strTitle = "Renewal " & PCase(strMedia) 
Else
	lettertab = "active"
	emailtab = ""
	strTitle = "Renewal " & PCase(strMedia) 
	...
	<div class="col-lg-12">
		...
		<ul class="nav nav-tabs">
			...
			<li id="Letters" class="<%=lettertab%>" >
				<a href="RenewalLetters.asp?media=letters">
			...
			<li id="Emails" class="<%=emailtab%>">		
				<a href="RenewalLetters.asp?media=emails">
		...
		<div class="tab-content" style="padding:10px;">
			...
			<FONT FACE="verdana,arial,helvetica" COLOR="#008400"><%=strTitle%></H4></FONT>
			<BR>			
			<%=TestModeStatus%><BR>
			<BR>
...
SQLOrgUsers = "Select UserNumber FROM Users (NOLOCK) WHERE OrganizationNumber = 1 AND UserNumber = 1212"
...
PUBLIC FUNCTION cleanBOM(strText)
	...
	'remove BOM if present https://unicode.org/faq/utf_bom.html
