...
ErrorLog MsgOrgName
ErrorLog PrivateLabel
ErrorLog MsgRenewalURL
ErrorLog MsgLogoPath
ErrorLog MsgFontFace
...
If TestEMailAddress <> "" Then
	...
	strTestMode = "Test Mode.  A test email will be sent to: " & TestEMailAddress & ""
	ErrorLog strTestMode
End If
...
response.write Request("FormName")
	...
	Case Else
		...
		Select Case strMode
			Case "letters"
				lettertab = "active"
				strFormName = "PrintLetters"
				strTestMode = "<BR>"			
			Case "emails"
				emailtab = "active"	
				strFormName = "SendEmails"				
		End Select
	...
	<div class="col-lg-12">
		...
		<ul class="nav nav-tabs">
			...
			<li id="Letters" class="<%=lettertab%>">	
				<a href="RenewalLetters.asp?mode=letters">
			...
			<li id="Emails" class="<%=emailtab%>">		
				<a href="RenewalLetters.asp?mode=emails">
		...
		<div class="tab-content" style="padding:10px;">
			...
			<H4><FONT FACE="verdana,arial,helvetica" COLOR="#008400"><%=MsgOrgName%></FONT></H4>
			...
			<BR>
				...
				If EventList <> "" Then
					...
					If Not rsTickets.EOF Then 'There are some events.  List them
						...
						Response.Write "<FONT FACE=""verdana,arial,helvetica"" COLOR=""#000000"" SIZE=""2"">Select events and click below.</FONT><BR><BR>" & vbCrLf
					...
					Else 'There aren't any matching events
						Response.Write "<BR><BR><FONT FACE=""verdana,arial,helvetica"" COLOR=""#000000"" SIZE=""2"">There are no events.</FONT><BR><BR>" & vbCrLf
					End If
				Else 'There aren't any matching events
					Response.Write "<BR><BR><FONT FACE=""verdana,arial,helvetica"" COLOR=""#000000"" SIZE=""2"">There are no events.</FONT><BR><BR>" & vbCrLf
				End If
...
Do Until rsOrderLine.EOF
	...
	'This is not the last order.
		...
		If TicketCount > 0 Then
			...
			LogFile = LogFile & LastOrderNumber & " | " & LastEmailAddress & "<BR>" & vbCrLf
	...
	End If
		...
		LogFile = LogFile & "Order Count: " & OrderCount & "<BR>"
		LogFile = "-----------------------------------------<BR>"
...
If TicketCount > 0 Then	'Print the last footer
	...
	If TestEMailAddress = "" Then
		...
		'ErrorLog "LastOrderNumber " & LastOrderNumber & " | LastEmailAddress " & LastEmailAddress & ""
		
		OrderCount = OrderCount + 1
		
		ErrorLog "OrderCount " & OrderCount & " LastOrderNumber " & LastOrderNumber & " | LastEmailAddress " & LastEmailAddress & ""
...
End If
		 ErrorLog "Final Order Count: " & OrderCount & "" 
		'LogFile = LogFile & "Final Order Count: " & OrderCount & "<BR>"
		'LogFile = "-----------------------------------------<BR>"
...
Set rsOrderLine = nothing
		...
		Response.Redirect("/RenewalLetters.asp?mode=letters")
