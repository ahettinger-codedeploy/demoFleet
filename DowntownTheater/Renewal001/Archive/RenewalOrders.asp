<%
'CHANGE LOG
'SLM 2/26/14 2014 Cerrito Renewal
%>
...
OrderExpirationDate = "3/26/14"
RenewDescription = Left("RIALTO CER 03/14", 15)
...
'E.G., If there were two events previous year, the first number would be set to 1. 
 'JAI 2/26/13 - Changed array to allow creating renewals on a pair of events.

RenewalEventCount = 2
Dim EventArray(1, 2)

EventArray(0,0) = 593155
EventArray(0,1) = 637503
EventArray(0,2) = 637508

EventArray(1,0) = 593156
EventArray(1,1) = 637504
EventArray(1,2) = 637507

For i = 0 To UBound(EventArray, 1)
    OldEventList = OldEventList & EventArray(i, 0) & ","
    NewEventList1 = NewEventList1 & EventArray(i, 1) & ","
    NewEventList2 = NewEventList2 & EventArray(i, 2) & ","
Next
OldEventList = Left(OldEventList, Len(OldEventList) - 1)
NewEventList1 = Left(NewEventList1, Len(NewEventList1) - 1)
NewEventList2 = Left(NewEventList2, Len(NewEventList2) - 1)
...
'----------------------------------------------------------------
Function LookupNewEventCode(OldEventCode, RenewalCount)

For i = 0 To UBound(EventArray)
    If OldEventCode = EventArray(i, 0) Then
        LookupNewEventCode = EventArray(i, RenewalCount)
    End If
Next
...
For i = 0 To UBound(EventArray)
    For RenewalCount = 1 to RenewalEventCount

        LYEventCode = EventArray(i,0)
        TYEventCode = EventArray(i,RenewalCount)
...
Do Until rsTickets.EOF
    ...
    For RenewalCount = 1 to RenewalEventCount
	    ...
	    NewEventCode = LookupNewEventCode(rsTickets("EventCode"), RenewalCount)
