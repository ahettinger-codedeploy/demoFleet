...
<% If SurveyTitle <> "" Then %>
<FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><b><%=SurveyTitle%></b></FONT>
<% Else %>
<FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="4">Survey</FONT>
<% End If %>
<br /><br />
<table cellpadding="0" cellspacing="0" border="0" width="80%">
    <tr><td><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=SurveyHeader%></FONT></td></tr>
<% 
For j = 1 To NumTickets
    i = LineNumbers(j)
    Response.Write "<tr><td><BR><BR><FONT FACE=""" & FontFace & """ COLOR=""" & FontColor & """ SIZE=""2""><b><u>Attendee " & j & "</u></b><BR>" & EventDetail(j) & "<BR>" & LineDetail(j) & "</FONT><BR></td></tr>" & vbCrLf
...
%>
    <tr>
        <td align="left"><BR><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=rsSurveyQues("QuestionText") & IsRequired%></FONT>
        <% If j = 1 Then %>        
...
<%        
        ...
        Do While Not rsSurveyAns.EOF
            If rsSurveyQues("QuestionType") = "checkbox" Or rsSurveyQues("QuestionType") = "radio" Then 
                ...
                End If
%>
            <INPUT TYPE="<%=rsSurveyQues("QuestionType")%>" NAME="Answer<%=rsSurveyQues("QuestionNumber")%>_<%=i%>" VALUE="<%=rsSurveyAns("AnswerText")%>"<% If StrChecked <> "" Then Response.Write StrChecked End If %>><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=rsSurveyAns("AnswerText")%></FONT>
          <% If rsSurveyAns("AnswerTextFieldIncluded") = "Y" Then %><INPUT type="text" name="<%=rsSurveyAns("AnswerText")%>_<%=rsSurveyQues("QuestionNumber")%>_<%=i%>" value="<%=StoredAnswer%>" size="50" /><% End If %><br />
<%          ElseIf rsSurveyQues("QuestionType") = "text" Then 
                ...
                End If
%>
          <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=rsSurveyAns("AnswerText")%></FONT>&nbsp;<INPUT TYPE="<%=rsSurveyQues("QuestionType")%>" NAME="Answer<%=rsSurveyQues("QuestionNumber")%>_<%=i%>" ID="Answer<%=rsSurveyQues("QuestionNumber")%>_<%=i%>" value="<%=StoredAnswer%>" size="50"><br />
<%          Else 'dropdown list 
                If GetLineAnswer(i, rsSurveyQues("QuestionText")) <> "" Then
...
<tr><td><br /></td></tr>
<% If answer_name_list <> "" Then %>
<tr><td><br /><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2">* - Answer Required</FONT></td></tr>
<% End If %>

<tr><td><br /><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=SurveyFooter%></FONT></td></tr>
</table><br />
