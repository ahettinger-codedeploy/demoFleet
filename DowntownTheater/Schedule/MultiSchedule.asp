...
'SSR (9/07/2011)
'Created the Design Your Own Schedule Page
'Rogue Theater
...
'When this option is active the subscription events are grouped together.
'They will not be displayed in "Event Listing" or "Act Selection Listing"
'Default is False
...
'Display events by their specific eventcode
'When this option is active, these events will not display in "Event Listing" or "Act Selection Listing"
...
'Display events grouped by their sub-category number
'When this option is active, these events will not display in "Event Listing" or "Act Selection Listing"
'Listed in date order
'Default is False
'SubcategoryTitle is auto populated with the name of the sub category

SubCategoryDisplay = True
SubCategoryVenueLocation = True
SubCategoryDateDisplay = True

SubCategorySubTitle = ""
SubCategoryCodeList = 83

'-----------------------------------

'ACT SELECTION LISTING

'Display events grouped by act
'These events can be listed in act date order or act name
'Default is False
...
'Display events listed in event date order
'Default is True

EventDisplay = True
EventVenueLocation = True
EventDateDisplay = True
...
'Display the link to the donation page
'Default is False
...
<div class="schedule">
<BR />
<div style="width: 90%;" class="title"><%=ScheduleTitle%></div>
<BR />
<BR />
<%
...
%>
        ...
        <table class="rounded-corner" summary="presale" style="width: 90%;" >
        <thead>
        <tr>
...
%>
        <table class="rounded-corner" class="schedule-table" style="width: 90%;" summary="presale">
        <thead>
        <tr>
            <th scope="col" class="category-left">&nbsp;</th>
...
If SubscriptionsDisplay Then
    ...
    If Not rsEvents.EOF Then
	    ...
	    If SubscriptionVenueLocation  Then
            ...
            <table class="rounded-corner" class="schedule-table" style="width: 90%;" summary="subscription header">
            <thead>
            <tr>
                <th scope="col" class="category-left">&nbsp;</th>
	    ...
	    Else
            %>                
            <div class="title"><%= SubscriptionEventsTitle %></div>  
            <table class="rounded-corner" style="width: 90%;" summary="subscription header">
            <thead>
            <tr>
                <th scope="col" class="category-left">&nbsp;</th>
	...
	Do While Not rsEvents.EOF
	    ...
	    'Generate list of subscription eventcodes to exclude in the event listing
	    '-----------------------------------------------------------------------
	...
	If Not rsEvents.EOF Then
	 ...
	 If SpecialEventVenueLocation Then	
	        ...
	        If SpecialEventDateDisplay Then
                ...
                <table class="rounded-corner" class="schedule-table" style="width: 90%;" summary="special events header">
                <thead>
                <tr>
                    <% ColNbr = 6 %>
                    <% FootColNbr = 4 %>
            ...
            Else 
                ...
                %>                
                <div class="title"><%= SpecialEventTitle %></div> 
               <table class="rounded-corner" style="width: 90%;" summary="special events header">
                <thead>
                <tr>
                    <% ColNbr = 5 %>
                    <% FootColNbr = 3 %>
	 ...
	 Else
            ...
            Else 
                ...
                %>            
                <div class="title"><%= SpecialEventTitle %></div> 
                <table class="rounded-corner" style="width: 90%;" class="schedule-table" summary="special events header">
                <thead>
                <tr>
                    <% ColNbr = 4 %>
                    <% FootColNbr = 2 %>
...
'SUB CATEGORY EVENT LISTING

SubCategoryEventList = ""

If SubCategoryDisplay Then

	SubCategoryList = Split(SubCategoryCodeList,",")

	For SubCd = lbound(SubCategoryList) to ubound(SubCategoryList)

	SQLSubCatName = "SELECT SubCategory.SubCategory FROM SubCategory (NOLOCK) WHERE SubCategory.SubCategoryCode = " & SubCategoryList(SubCd) & ""
	Set rsSubCatName = OBJdbConnection.Execute(SQLSubCatName)
		SubCategoryTitle = rsSubCatName("SubCategory")
		SubCategoryTitle = SubCategoryTitle & "&nbsp;Tickets"
	rsSubCatName.Close
	Set rsSubCatName = Nothing
    ...
    'Display only the Special Events from this list
    '-----------------------------------------------
    SQLWhere = SQLWhere & " AND Act.SubCategoryCode = 83"       
	...
	If Not rsEvents.EOF Then
	 ...
	 If SubCategoryVenueLocation Then	
	    
	        If SubCategoryDateDisplay Then

	            'Show Venue / Show Date 
                %>            
				<div style="width: 90%;" class="title"><%= SubCategoryTitle %></div> 
				<table class="rounded-corner" style="width: 90%;" class="schedule-table" summary="special events header">
				<thead>
				<tr>
					<% ColNbr = 6 %>
            ...
            Else 
                ...
                'Show Venue / No Date 
                %>                
                <div class="title"><%= SubCategoryTitle %></div> 
				<table class="rounded-corner" summary="events header">
				<thead>
				<tr>
					<% ColNbr = 5 %>
					<% FootColNbr = 3 %>
	 ...
	 Else
            ...
            Else 
                ...
                %>            
                <div class="title"><%= SubCategoryTitle %></div> 
                <table class="rounded-corner" summary="special events header">
                <thead>
                <tr>
                    <% ColNbr = 4 %>
                    <% FootColNbr = 2 %>
	...
	Do While Not rsEvents.EOF
		...
		'Generate list of eventcodes to exclude in the event listing
	    '-----------------------------------------------------------------------
	    SubCategoryEventList = SubCategoryEventList &  rsEvents("EventCode") & "," 
    ...
    'Finalize the SubCategory Event List
    '-------------------------------------
    If Right(SubCategoryEventList,1) = "," Then
        X = Len(SubCategoryEventList)
        SubCategoryEventList = Left(SubCategoryEventList,(X-1))
    End If
	

	ErrorLog("SubCategoryEventList " & SubCategoryEventList & "")
...
'Exclude the Sub Category Events from this list
'-----------------------------------------------
If SubCategoryDisplay Then
    If SubCategoryEventList <> "" Then
        SQLWhere = SQLWhere & " AND Event.EventCode NOT IN (" & SubCategoryEventList & ")"
    End If
End If 
...
%>
        <div style="width: 90%;" class="title"><%= ActSelectionTitle %></div> 

        <table class="rounded-corner" class="schedule-table" style="width: 90%;" summary="act selection header">
        <thead>
        <tr>
            <th scope="col" class="category-left">&nbsp;</th>
...
If EventDisplay Then
	...
	'Show Venue / Show Date 
	%>                           
        <div style="width: 90%;" class="title"><%= EventTitle %></div> 
        <table class="rounded-corner" class="schedule-table" style="width: 90%;" summary="special events header">
        <thead>
        <tr>
            <% ColNbr = 6 %>
...
%>
            ...
            <Br />
            <div style="width: 90%;" class="title"><%=DonationEventsTitle%></div> 
            <table class="rounded-corner" class="schedule-table" style="width: 90%;" summary="donation events header">
            <thead>
            <tr>
                <th scope="col" class="category-left">&nbsp;</th>
