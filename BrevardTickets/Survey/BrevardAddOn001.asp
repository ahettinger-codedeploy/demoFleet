...
<BR>
<BR>
<FONT FACE="<%= FontFace %>" COLOR="<%= HeadingFontColor %>" SIZE="2"><H3>Please select the performance of Amahl you wish to attend:</H3></FONT>
...
<%
    ...
    If EventCodeSelected = "194322" Then
        EventCodeList = "194325"
    'ElseIf EventCodeSelected = "194322" Then
        'EventCodeList = "194325"
	...
	Select Case ReturnCode
		Case 0	'Stored Procedure executed properly.  Continue.
			...
			For i = 1 To OfferQuantity
				...
				SQLUpdateSeatType = "UPDATE OrderLine WITH (ROWLOCK) SET SeatTypeCode = " & Clean(Request("SeatTypeCode")) & ", Price = 0, Surcharge = 0, Discount = 25 WHERE OrderNumber = " & Session("OrderNumber") & " AND ItemNumber IN (SELECT TOP 1 ItemNumber FROM OrderLine (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber") & " AND SeatTypeCode IS NULL AND ItemType IN ('Seat', 'SubFixedEvent') ORDER BY LineNumber DESC)"
