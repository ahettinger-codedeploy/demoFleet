...
<BODY onLoad="PrintTickets()">
		...
		'Create Batch
		Set rsInsertTicketBatch = Server.CreateObject("ADODB.Recordset")
		rsInsertTicketBatch.ActiveConnection = OBJdbConnection
		rsInsertTicketBatch.CursorLocation = 3 'adUseClient
		rsInsertTicketBatch.CursorType = 1 'adOpenKeyset
		rsInsertTicketBatch.LockType = 3 'adLockOptimistic
		rsInsertTicketBatch.Source = "TicketPrintBatch"
		rsInsertTicketBatch.Open

		rsInsertTicketBatch.AddNew
			rsInsertTicketBatch("BatchDate") = Now()
			rsInsertTicketBatch("UserNumber") = Session("UserNumber")
			rsInsertTicketBatch("BatchType") = "Ticket Batch Print"
		rsInsertTicketBatch.Update
				
		BatchNumber = rsInsertTicketBatch.Fields("BatchNumber").Value
