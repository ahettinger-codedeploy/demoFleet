...
'CHANGE LOG
'SSR 2/11/2014 - Modified existing group discount to allow ticket count across multiple events - to allow add  MultiEvent = "Y" to discount script
...
'SSR 9/30/2013 - Count tickets from multiple events towards group quantity
'Add  MultiEvent = "Y" to discount script
MultiEvent = "N"
If Clean(Request("MultiEvent")) = "Y" Then
	SQLMultiEvent = ""
Else
...
If OrderTypeFlag = "Y" And AllowedSeatTypeFlag = "Y" And MaxDiscountsPerOrderOK = "Y" And MaxDiscountsPerEventOK = "Y" And MaxDiscountsPerOrderPerEventOK = "Y" And MaxDiscountSystemUsageOK = "Y" And AllowedSectionOK = "Y" And Now() > StartDate And Now() < ExpirationDate Then 'It's okay to apply to this order type and seat type.
	'SSR 9/30/13 - Added SQLMultiEvent for discounts across multiple events
	'REE 6/20/7 - Removed SQLList from query criteria since SeatTypeCode is being checked above.
	SQLEventCount = "SELECT COUNT(Seat.EventCode) AS SeatCount FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber WHERE OrderLine.OrderNumber = "&Clean(Request("OrderNumber"))&" "&SQLMultiEvent&" "&SQLAllowedSectionList&" "
