...
SQLReport = "SELECT (Select Left(Organization, Len(Organization)-8)) as Organization, Users.FirstName, Users.LastName, Users.UserID, Users.UserNumber FROM Users INNER JOIN Organization ON Organization.OrganizationNumber = Users.OrganizationNumber WHERE (Users.OrganizationNumber IN (SELECT SubOrganizationNumber FROM SubOrganization WHERE (OrganizationNumber = 700))) ORDER BY Organization"
Set rsReport = OBJdbConnection.Execute(SQLReport)

Do Until rsReport.EOF
		
		Response.Write "<TR BGCOLOR=""#DDDDDD""><TD><FONT FACE=""verdana,arial,helvetica"" SIZE=""2"">" & rsReport("Organization") & "</FONT></TD></TD><TD ALIGN=""left""><FONT FACE='verdana,arial,helvetica' SIZE=2>" & rsReport("FirstName") & " " & rsReport("LastName") & "</FONT></TD><TD ALIGN=""left""><FONT FACE='verdana,arial,helvetica' SIZE=2>" & rsReport("UserID") & "</FONT></TD><TD ALIGN=""center""><FONT FACE='verdana,arial,helvetica' SIZE=2><a href=""https://www.tix.com/Management/PasswordReset.asp"">Reset</a></FONT></TD></TR>" & vbCrLf
...
SELECT LEFT(Organization.Organization, 8) AS Organization.Organization