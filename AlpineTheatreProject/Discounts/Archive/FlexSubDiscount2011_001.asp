...
' 58993,58994,58995,58996,58997,58998,58999,59000

' Tickets discounted on the following productions only: 58993; 58994; 58995; 58997; 58998; 58999
' * Not Discounted: 58996,59000
...
' Tickets discounted on the following productions only: 58993; 58994; 58996; 58997; 58998; 58999
' Not Discount: N/A
...
' Tickets discounted on the following productions only: 58996; 58997; 58998; 58999
' Not Discount: N/A
...
DIM ActCodeList(3)
ActCodeList1 = "58993,58994,58995,58996,58997,58998,58999,59000"
ActCodeList2 = "58993,58994,58996,58997,58998,58999"
ActCodeList3 = "58996,58997,58998,58999"
...
If Now() < CDate(Date(1)) Then  
	    ...
	    'Count # of Allowed Acts on the order
	    SQLActCount1 = "SELECT COUNT(ActCode) AS ActCount1 FROM (SELECT Event.ActCode FROM Event (NOLOCK) INNER JOIN Seat (NOLOCK) ON Event.EventCode = Seat.EventCode WHERE Seat.OrderNumber = " & OrderNumber & " AND Event.ActCode IN (" & ActCodeList1 & ") GROUP BY Event.ActCode) AS ActCount1"
	    Set rsActCount1 = OBJdbConnection.Execute(SQLActCount1)	
	    ActCount1 = rsActCount1("ActCount1")	
	    rsActCount1.Close
	    Set rsActCount1 = nothing
	    
	    SQLActCount2 = "SELECT COUNT(ActCode) AS ActCount2 FROM (SELECT Event.ActCode FROM Event (NOLOCK) INNER JOIN Seat (NOLOCK) ON Event.EventCode = Seat.EventCode WHERE Seat.OrderNumber = " & OrderNumber & " AND Event.ActCode IN (" & ActCodeList2 & ") GROUP BY Event.ActCode) AS ActCount2"
	    Set rsActCount2 = OBJdbConnection.Execute(SQLActCount2)	
	    ActCount2 = rsActCount2("ActCount2")	
	    rsActCount2.Close
	    Set rsActCount2 = nothing
	    
	     SQLActCount3 = "SELECT COUNT(ActCode) AS ActCount3 FROM (SELECT Event.ActCode FROM Event (NOLOCK) INNER JOIN Seat (NOLOCK) ON Event.EventCode = Seat.EventCode WHERE Seat.OrderNumber = " & OrderNumber & " AND Event.ActCode IN (" & ActCodeList3 & ") GROUP BY Event.ActCode) AS ActCount3"
		...
		SQLMinTicketCount = "SELECT MIN(TicketCount) AS NumSubs FROM (SELECT ActCode, COUNT(Seat.ItemNumber) AS TicketCount FROM Event (NOLOCK) INNER JOIN Seat (NOLOCK) ON Event.EventCode = Seat.EventCode WHERE Seat.OrderNumber = " & OrderNumber & " AND Event.ActCode IN (" & ActCodeList1 & ") GROUP BY Event.ActCode) AS TicketCount"
		...
		If AppliedCount < NbrSubscriptions Then
		    ...
		    If SubscriptionType <> "NOSHOW" Then	
		            ...
		            If SubscriptionType = "6SHOW" Then
                        ...
                        Select Case ActCode 
                            Case 58993,58994,58996,58997,58998,58999 
                                NewPrice = round(CDbl(Clean(Request("Price"))) * (1-CDbl(Percentage)/100), 2)                
                                DiscountAmount = Clean(Request("Price")) - NewPrice
                                AppliedFlag = "Y" 	
                        End Select  
		            ...
		            If SubscriptionType = "8SHOW" Then 
                        ...
                        Select Case ActCode 
                            Case 58993,58994,58995,58997,58998,58999 
                                NewPrice = round(CDbl(Clean(Request("Price"))) * (1-CDbl(Percentage)/100), 2)                
                                DiscountAmount = Clean(Request("Price")) - NewPrice
                                AppliedFlag = "Y" 	
                        End Select  
