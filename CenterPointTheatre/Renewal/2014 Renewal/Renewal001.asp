<%
'CHANGE LOG
'JAI 7/11/13 -  Renewals for Centerpoint Legacy Theatre 2014.  Copied from Rogers Memorial (previous name)
%>
...
'Run Renewal steps
Response.Write "Begin " & RenewDescription & "<BR><BR>"
Dim RenewalMismatch
RenewalMismatch = "N"
Call RenewalCheck()
If Request("RenewalCheck") <> "Y" Then  'Do not run if only want renewal check
...
Response.Write "End " & RenewDescription & "<BR>"
...
Case 1  'Adult
	LookupNewSeatTypeCode = 1 'Adult
Case 307 'Senior
	LookupNewSeatTypeCode = 3 'Senior
Case Else
...
Response.Write "<B><U>Renewal</U></B><BR><BR>" & vbCrLf

SQLTickets = "SELECT Seat.EventCode, Seat.OrderNumber, Seat.SectionCode, Seat.Row, Seat.Seat, OrderLine.SeatTypeCode, OrderLine.ShipFirstName, OrderLine.ShipLastName, OrderLine.ShipAddress1, OrderLine.ShipAddress2, OrderLine.ShipCity, OrderLine.ShipState, OrderLine.ShipCountry, OrderLine.ShipPostalCode, OrderHeader.CustomerNumber FROM Seat (NOLOCK) INNER JOIN OrderLine (NOLOCK) ON Seat.ItemNumber = OrderLine.ItemNumber INNER JOIN OrderHeader (NOLOCK) ON Seat.OrderNumber = OrderHeader.OrderNumber WHERE Seat.EventCode IN (" & OldEventList & ") AND Seat.StatusCode = 'S' AND OrderLine.ItemType IN ('Seat', 'SubFixedEvent') AND OrderHeader.OrderTypeNumber <> 5 ORDER BY Seat.OrderNumber"
...
Do Until rsTickets.EOF
	...
	SQLSeat = "SELECT ItemNumber, Event.EventCode, EventType, Map FROM Seat (NOLOCK) INNER JOIN Event (NOLOCK) ON Seat.EventCode = Event.EventCode WHERE Event.EventCode = " & NewEventCode & " AND SectionCode = '" & rsTickets("SectionCode") & "' AND Row = '" & rsTickets("Row") & "' AND Seat = '" & rsTickets("Seat") & "' AND StatusCode IN ('A', 'H')"
	'ErrorLog( SQLSeat)
	Set rsSeat = OBJdbConnection.Execute(SQLSeat)
	
	'JAI 4/26/11 - Added SubFixedEvent for subscription tickets.
	If UCASE(rsSeat("EventType")) = "SUBFIXEDEVENT" Then  'Subscription Event
		If UCase(rsSeat("Map")) = "GENERAL" Then 'General Admission Subscription
		...
		Else 'Reserved Seating Subscription
			...
			spReservedSub.Parameters.Append spReservedSub.CreateParameter("ItemNumber", 200, 1, 5000, rsSeat("ItemNumber")) 'As Varchar and Input
	...
	Else  'Single event
		...
		Else 'Reserved Seating single event
			...
			spReservedEvent.Parameters.Append spReservedEvent.CreateParameter("ItemNumber", 200, 1, 5000, rsSeat("ItemNumber")) 'As Varchar and Input
	...
	Response.Write "EventCode = " & NewEventCode & " | OldOrderNumber = " & OldOrderNumber & " | NewOrderNumber = " & OrderNumber & " | ItemNumber = " & rsSeat("ItemNumber") & " | ReturnCode = " & ReturnCode & "<BR>"
	...
	Else 'It failed
		...
		Response.Write "Import Failed - CustomerNumber = " & rsTickets("CustomerNumber") & " | Section = " & rsTickets("SectionCode") & " | Row = " & rsTickets("Row") & " | Seat = " & rsTickets("Seat") & "<BR>" & vbCrLf
