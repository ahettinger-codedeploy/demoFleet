...
SQLCount = "SELECT COUNT(OrderLine.LineNumber) AS TicketCount FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber AND OrderLine.ItemType = 'Seat' WHERE OrderLine.OrderNumber = " & OrderNumber & " AND ItemType = 'Seat' AND EventCode IN (83142,83143,83144,83145,83146,83147)"
...
If NbrTickets > 1 Then 'Need to have more than one ticket on order
	...
	If rsDiscCount("LineCount") < Fix(NbrTickets / 5) Then 'Apply the discount
		NewPrice = 0
