...
'Adler Planetarium (7/6/2010)
'==================
'Event Add On Survey

'If a customer has a ticket to:
' Adler After Dark (EventCode 283313)

'they are offered the following two choices:

'(1) a ticket to White Sky Tiger @ 7PM (EventCode 283317) for $5 per ticket
'(2) a ticket to White Sky Tiger @ 8:45PM (EventCode 283318) for $5 per ticket
...
Sub AddOnSurvey
    ...
    If Right(EventList,1) = "," Then
        X = Len(EventList)
        OfferList = Left(EventList,(X-1))
   Else
        OfferList = EventList
   End If
   
   ErrorLog("Available Tickets: " & AvailTickets & "")
