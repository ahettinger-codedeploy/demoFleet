...
If Not rsOrderNum.EOF Then
	...
	If Not rsTix.EOF Then
		...
		'Begin Ad Space
            ...
            'Change Ticket Format for Subscription Events
            If rsTix("EventCode") = "320294" Then
                
                       
                SQLSubCheck = "Select Event.EventCode FROM Event (NOLOCK) INNER JOIN Seat ON Event.EventCode = Seat.EventCode INNER JOIN OrderLine ON Seat.ItemNumber = OrderLine.ItemNumber WHERE OrderLine.OrderNumber = " & rsTix("OrderNumber") & " AND OrderLine.ItemType = 'SubFixedEvent' AND OrderLine.LineNumber =  " & rsTix("ParentLineNumber") & " "
                Set rsSubCheck = OBJdbConnection.Execute(SQLSubCheck)
                
                Select Case rsSubCheck("EventCode")
                    Case 321533,321156,321154,321155,321532 'Subscription
                        Response.Write "<IMG SRC=""/clients/CowboyPoetryGenoa/ETickets/images/Ad-Spons-HolidayInn-1.jpg"" WIDTH=""500"">" & vbCrLf
                    Case Else
                        Response.Write "<IMG SRC=""/clients/CowboyPoetryGenoa/ETickets/images/CarsonValleyInn.jpg"" WIDTH=""500"">" & vbCrLf
                End Select
            ...
            Else 
                 Response.Write "<IMG SRC=""/clients/CowboyPoetryGenoa/ETickets/images/cowboypoetry.jpg"">" & vbCrLf
            End If        
