...
If NbrTickets > 1 Then 'Need to have more than one ticket on order
	...
	If Now() > StartDate And Now() < ExpirationDate Then
		...
		If Clean(Request("AllEvents")) = "Y" Then
			SQLDiscCount = "SELECT COUNT(OrderLine.LineNumber) AS LineCount FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber AND OrderLine.ItemType = 'SubFixedEvent' WHERE OrderLine.OrderNumber = " & OrderNumber & " AND OrderLine.DiscountTypeNumber = " & DiscountTypeNumber 
		Else
...
'JAI 10/7/7 - Removed the following code temporarily.  This prevented discount from being applied in certain instances.
