	...
	Do While Not rsEvents.EOF
		...
		Set rsSoldOut = OBJdbConnection2.Execute(SQLSoldOut)
			...
			If rsSoldOut.EOF Then 'Event Status is sold out.
				EventStatus = "<FONT FACE=" & FontFace & " COLOR=" & TableCategoryBGColor & " SIZE=2>Sold Out</FONT>"
			ElseIf rsEvents("SaleEndDate") <= Now() Then 'Event Status is off sale
				EventStatus = "<FONT FACE=" & FontFace & " COLOR=" & TableCategoryBGColor & " SIZE=2>Not Available</FONT>"
			Else 'Event Status is on sale
				EventStatus = "<FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & "><A HREF=/Event.asp?Event=" & rsEvents("EventCode") & ">Tix & Info</A>&nbsp;</FONT>"
			End If
