...
'Restrictions: 			All tickets must be the same ticket type
'						All tickets must be in the same price tier
...
ActCodeList = "117152,117470,95978"
...
'----------------------------------------------------------
	...
	SQLActCount = "SELECT COUNT(ActCode) AS ActCount FROM (SELECT Event.ActCode FROM Event (NOLOCK) INNER JOIN Seat (NOLOCK) ON Event.EventCode = Seat.EventCode INNER JOIN OrderLine (NOLOCK) ON Seat.ItemNumber = OrderLine.ItemNumber INNER JOIN Section (NOLOCK) ON Seat.EventCode = Section.EventCode AND Seat.SectionCode = Section.SectionCode WHERE Seat.OrderNumber = " & OrderNumber & " AND Event.ActCode IN (" & ActCodeList & ") AND Color = '" & Color & "' AND OrderLine.SeatTypeCode = " & SeatTypeCode & " AND Color IN ('DARKBLUE','DARKGOLD','DARKGREEN') GROUP BY Event.ActCode) AS ActCount"
