...
FullPath = Server.Mappath(PathOnly)
set Folder = FileSysObj.GetFolder(FullPath)
...
'TicketLogo = "logo.gif" (size: 215x68 pixels)

DIM ETicketLogo
ETicketLogo = ""

For Each FileIndex in FileList
ErrorLog("FileName " & FileIndex.Name & "")
	If Lcase(left(FileIndex.Name, 4)) = "logo" then
		ETicketLogo = PathOnly & "/logo.gif"
	Else
		Select Case TicketLogo
			Case "Tix"
				ETicketLogo = "/Images/ETicketLogo.gif"
			Case "none"
				ETicketLogo  = "/Images/clear.gif"
			Case Else 
				ETicketLogo  = "/Images/ETicketLogo.gif"
		End Select
	End If
...
'TicketBackground = "none" = (no background)
'TicketBackground = "background.gif" (size: 613x237 pixels)

DIM ETicketBackground
ETicketBackground = ""

For Each FileIndex in FileList
	If Lcase(left(FileIndex.Name, 10)) = "background" then
		ETicketBackground = PathOnly & "/background.gif"
	Else
		Select Case TicketBackground
			Case "Tix"
			...
			Case "none"
				ETicketBackground = "/Images/clear.gif"
			Case Else 
				ETicketBackground = "/Images/ETicketBackground.gif"
		End Select
	End If
...
TicketBottomBar = "none"

'Options:
'TicketBottomBar = "Tix" 	(tix bottombar - Black)
'TicketBottomBar = "TixGray" (tix bottombar - Gray)
'TicketBottomBar = "none" = (no background)
'TicketBottomBar = "bottombar.gif" 

DIM ETicketBottomBar
ETicketBottomBar = ""

For Each FileIndex in FileList
	If Lcase(left(FileIndex.Name, 9)) = "bottombar" then
		ETicketBottomBar = PathOnly & "/bottombar.gif"
	Else
		Select Case TicketBottomBar
			Case "Tix"
				ETicketBottomBar = "/Images/ETicketBottomBar.gif"
			Case "TixGray"
				ETicketBottomBar = "/Images/ETicketBottomBarGray.gif"
			Case "none"
				ETicketBottomBar = "/Images/clear.gif"
			Case Else 
				ETicketBottomBar = "/Images/ETicketBottomBar.gif"
		End Select
	End If
...
For Each FileIndex in FileList
	If Lcase(left(FileIndex.Name, 2)) = "ad" then
		ETicketAd = PathOnly & "/ad.gif"
	Else
		Select Case TicketAd
			Case Tix
				ETicketAd = "/images/TixLogo300x225.gif"
			Case "TixSmall"
				ETicketAd = "/images/TixLogo90x70.gif"
			Case "TixMedium"
				ETicketAd = "/images/TixLogo200x150.gif"
			Case "TixLarge"
				ETicketAd = "/images/TixLogo300x225.gif"
			Case "TixXLarge"
				ETicketAd = "/images/TixLogo400x300.gif"
			Case "none"
				ETicketAd = "/Images/clear.gif"
			Case Else 
				ETicketAd = "/images/TixLogo300x225.gif"
		End Select
	End If
...
RequiredItem(1) = 000000  'Image 1
DisplayItem(1) = "000000.jpg"

RequiredItem(2) = 000000 'Image 2
DisplayItem(2) = "000000.jpg"


RequiredItem(3) = 000000 'Image 3
DisplayItem(3) = "000000.jpg"

'--------------------------------------------------

'BarcCode Options

ShowBarCodeTicket = False

ShowBarCodeOrder = True

'Options:
'ShowBarCodeTicket: Controls display of the barcode on the ticket body. Default is True
'ShowBarCodeOrder: Controls display of the barcode within order details. Default is True
...
If Not rsOrderNum.EOF Then
	...
	If Not rsTix.EOF Then
		...
		'Begin Ticket
			...
			Response.Write "<TABLE WIDTH=""615"" HEIGHT=""240"" BORDER=""0"" BACKGROUND=""/PrivateLabel/DurangoConcerts/ETickets/images/background.gif"" BGCOLOR=""#FFD718"" CELLPADDING=""0"" CELLSPACING=""0"">" & vbCrLf   
			...
			Response.Write "<TR>" & vbCrLf
			Response.Write "<TD ALIGN=""center"" COLSPAN=""2"" VALIGN=""bottom"" HEIGHT=""20"">" & vbCrLf
		...
		'Begin Body Space
			...
			If ParkingSection Then
				Response.Write "<FONT FACE=""Arial, Helvetica"" SIZE=""2"">" & vbCrLf
				Response.Write "<B>PARKING INFORMATION</B><BR>" & vbCrLf
				Response.Write "Parking Information Text" & vbCrLf
				Response.Write "<BR>" & vbCrLf
				Response.Write "</FONT>" & vbCrLf
			End If
		...
		'End Body Space
			...
			If ShowEventAd Then
				...
				For i = LBound(RequiredItem) to UBound(RequiredItem)
					If rsTIX("EventCode") =  RequiredItem(i) Then
						ETicketAd = PathOnly & DisplayItem(i)
					End If
				Next
			...
			Response.Write "<TR>" & vbCrLf
			Response.Write "<TD ALIGN=""center"">" & vbCrLf
			Response.Write "<IMG src=""" & ETicketAd & """ border=""0"">" & vbCrLf
			Response.Write "</TD>" & vbCrLf
			Response.Write "</TR>" & vbCrLf
			Response.Write "</TABLE>" & vbCrLf 
...
'Proper Case Formating for Patron Names
