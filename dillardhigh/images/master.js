...
jQuery(document).ready(function($) {
	...
	vh_movies_list_get(jQuery('#movies_list_per_page').val(), $.datepicker.formatDate('mm.dd.yy', new Date()),'','',jQuery('.wrapper-dropdown-6 .dropdown li.active').find('input[type=hidden]').val(),jQuery('#movies_list_events').val(),'', my_ajax.ajaxurl);

	jQuery('.movies_list_pagenum').live('click', function(e) {
		var date = jQuery('#movie-datepicker').val();
		if ( date == null ) { date = '' };
		var theatre = jQuery('.wrapper-dropdown-1 .dropdown li.active').find('input[type=hidden]').val();
		if ( theatre == null ) { theatre = '' };
	...
	jQuery( ".spotlight_cont .spotlight_container" ).mouseenter(function() {
		...
		jQuery(this).children('.line').show().stop().animate({
			width: "100%",
			left: "0%", 
		}, 300, function() {
			// Animation complete.
		 });
		jQuery(this).children('.movie_title').show().stop().animate({
			top: "0",
		}, 300, function() {
			// Animation complete.
		});
