...
{ // convenient way to start the form onLoad
  ...
  for(var i= 0; i < els.length; i++)
    if(els[i].type != 'hidden') {
            els[i].focus();
            if(isRequired(els[i])) requireValue(els[i]);
            return; }
}
...
//  if(fld.id)
//  {
    lbl=fld.getAttribute('label');
    if(!lbl || lbl == '') lbl=fld.name;
    return lbl;
    /*
...
function requireSingleCheckedByName(fld)
{ // require at least one checkbox to be checked within a name grouping
	  if(fld.disabled) return true;
	  var el = document.getElementsByName(fld.name);
...
function requireValueIfChecked(fld,targetId) {
    var targetElem = document.getElementById(targetId);
    if (targetElem && targetElem.checked) return requireValue(fld);
    return true;

}

function requireSelectedIfChecked(fld,targetId,options) {
    var targetElem = document.getElementById(targetId);
    if (targetElem && targetElem.checked) return requireSelected(fld,options);
    return true;
}

function requireSelected(fld,options) {
    var firstOptionIsBlank = false;
    var index = fld.selectedIndex;
    if (options) {
        firstOptionIsBlank = options.firstOptionIsBlank || false;
    }

    if (index == -1 || (firstOptionIsBlank && 0 == fld.selectedIndex)) {
        status = 'The ' + fieldname(fld) + ' must be selected.';
        return false;
    }
    return true;
}
...
{ // require at least one radio in this group to be checked
  ...
  for(var i= 0; i < radios.length; i++)
  {
    ...
    else if((typeof(radios[i].offsetWidth) != 'undefined') &&  (radios[i].offsetWidth > 0)) {
         visible = true;
     }
  }
  if(!visible||!enabled) return true; // no visible/enabled options in this group
  status = 'You must select one of the '+ fieldname(radios[0]) + ' options.';
  return false;
}
...
{ // simple email check
  ...
  { status= 'The '+fieldname(fld)+' field must contain a valid email address and may not contain any spaces.'; return false; }
  return true;
}
...
{ // tenacious credit card correction; fieldname isn't a big consideration, probably only one card per form
  ...
  if(val.indexOf('*')>=0) return true; // rivista uses safecc to replace first 12 digits of card with * so we can ignore these here because it indicates card on file
  val= val.replace(/\D/g,'');
...
/* The given URL string must apply to the following criteria to pass the test:
 *   must start with http or https, followed by ://
 *   domain name must consist of alphanumeric characters and dash, and at least one dot
 *   query string can consist of alphanum chars and a variety of special chars (see regular exp. below)
 *   port declaration is optional
 * NOTE: pattern matching is case insensitive
 */
function fixUrl(fld) {
  if(!fld.value.length||fld.disabled) return true; // blank fields are the domain of requireValue
  //http://([\w-]+\.)+[\w-]+(/[\w- ./?%&=]*)?
  var pattern = "^(http|https)://([a-zA-Z0-9-]+\\.)+[a-zA-Z0-9-]+(:[\\d]+)?(/[\\w-./?%&=+;]*)?$";
  var regex = new RegExp(pattern,"i");
  var val= fld.value;
  if(!regex.test(val)) {
      status = val + ' is not a valid URL (E.g. make sure it starts with http:// or https://).';
      return false;
  }
  return true;
...
{ // uses the onBlur handler of form elements to check value
  ...
  for(var index= 0; index < frm.elements.length; index++)
  {
    ...
    {
            if(isRequired(el) && !el.onblur) {
            /**
             * IE not correctly works with onblur attribute, so this is a small hack to fix the problem #23364
             *
             * @author Nikita Chirkov, Warecorp <nikita.chirkov@warecorp.com>
             */
            //el.onblur=requireValue(el);
            el.onblur = function() { requireValue(el); };
        }
...
function submitCheck(el) {
	...
	for (var i=0;i<children.length;i++) {
		...
		if (name === 'submit') {
			children[i].name = 'Submit';
		}
		if (id === 'submit') {
			children[i].id = 'Submit';
		}
	}
}
...
{ // uses the best available method to check form values
  ...
  // if there is an element with the name 'submit' - we need to change this to something else
  submitCheck(frm);
