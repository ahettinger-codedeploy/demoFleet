...
Lightbox.prototype = {
	...
	*/
	...
	fileType : function(url){
		...
		if (page.test(url) || url.substr((url.length-1), url.length) == 'index.html') return 'page';
	...
	getDomain : function(url) {    
        ...
        var nextSlash = withoutResource.indexOf('index.html');
    ...
    parseVideoURL: function(url) {
		...
		*/
		...
		if (urlLc.include('youtube.com/watch?v=')) {
			newUrl = url.replace(/watch\?v\=/i,'v/index.html');
			...
			miscVar = newUrl.indexOf('&');
			if (miscVar > -1) {newUrl = newUrl.substr(0,miscVar);}
		} else if (urlLc.include('vimeo.com/index.html')) {
			newUrl = url.replace('vimeo.com/index.html','vimeo.com/moogaloop32b3.html?clip_id=');
			...
			miscVar = urlLc.lastIndexOf('index.html')+1;
			newUrl += urlLc.substr(miscVar,urlLc.length - miscVar);
