...
If Not rsOrderNum.EOF Then
	...
	If Not rsTix.EOF Then
		...
		'Begin Ticket
			...
			Response.Write "<TABLE WIDTH=""620"" HEIGHT=""240"" BORDER=""0"" BACKGROUND=""/Images/ETicketBackground.gif"" CELLPADDING=""0"" CELLSPACING=""0"">" & vbCrLf
			Response.Write "<TR>" & vbCrLf
			...
			Response.Write "<FONT FACE=""Arial,Helvetica"" SIZE=""2"">" & rsTix("Section") & " " & vbCrLf
			Response.Write "</TD>" & vbCrLf
			Response.Write "</TR>" & vbCrLf
			Response.Write "</TABLE>" & vbCrLf
			Response.Write "</TD>" & vbCrLf
		...
		'Begin Body Space
			...
			Response.Write "<BR>" & vbCrLf
			'Response.Write "<FONT FACE=""Arial, Helvetica"" SIZE=""2"">" & vbCrLf
			'Response.Write "<B>DIRECTIONS</B>" & vbCrLf
			'Response.Write "<br />" & vbCrLf
			'Response.Write "<FONT FACE=""Arial, Helvetica"" SIZE=""1"">" & vbCrLf
			'Response.Write "FROM THE SOUTH" & vbCrLf
			...
			'Response.Write "<br />" & vbCrLf
			'Response.Write "<FONT FACE=""Arial, Helvetica"" SIZE=""1"">" & vbCrLf
			'Response.Write "FROM THE EAST" & vbCrLf
			'Response.Write "Take the 10 WEST, transition to the 110 NORTH" & vbCrLf
			'Response.Write "(Downtown). EXIT at PICO Blvd. The PICO" & vbCrLf
			'Response.Write "off-ramp becomes CHERRY St. Turn RIGHT" & vbCrLf
			'Response.Write "into the West Hall parking garage." & vbCrLf
			'Response.Write "<br />" & vbCrLf
			'Response.Write "FROM THE WEST" & vbCrLf
			'Response.Write "Take the 10 EAST, transition to the 110 NORTH," & vbCrLf
			'Response.Write "exit at 9th St. RIGHT on FLOWER St. RIGHT" & vbCrLf
			'Response.Write "on 11TH St. LEFT on CHERRY St. LEFT into" & vbCrLf
			'Response.Write "the West Hall parking garage." & vbCrLf		
		...
		ElseIf rsTix("ItemType") = "SubSeat" Then
		    ...
		    'Begin Ticket
			    ...
			    Response.Write "<TD VALIGN=""top"">" & vbCrLf
			    Response.Write "<FONT FACE=""Arial,Helvetica"" SIZE=""2""><I>FADA Presents</I></FONT><BR>" & vbCrLf
			    Response.Write "<FONT FACE=""Arial,Helvetica"" SIZE=""5""><B>25th Annual Los Angeles Art Show</B></FONT><BR>" & vbCrLf
		    ...
		    'Begin Body Space
			    ...
			    'Begin Middle Column
			    Response.Write "<TD VALIGN=""top"" WIDTH=""34%"" ALIGN=""middle"">" & vbCrLf
			    
			    'Print all barcodes for children events including this one Thursday
			    SQLChildBarCode = "SELECT Ticket.TicketNumber, Event.EventDate, Act.ShortAct FROM Ticket (NOLOCK) INNER JOIN OrderLine (NOLOCK) ON Ticket.ItemNumber = OrderLine.ItemNumber INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber INNER JOIN Event (NOLOCK) ON Seat.Eventcode = Event.EventCode INNER JOIN Act (NOLOCK) ON Event.Actcode = Act.Actcode WHERE OrderLine.OrderNumber = " & rsOrderNum("OrderNumber") & " AND OrderLine.ItemType = 'SubSeat' AND ParentLineNumber IN (SELECT ParentLineNumber FROM OrderLine (NOLOCK) WHERE OrderNumber = " & rsOrderNum("OrderNumber") & " AND OrderLine.ItemNumber = " & rsTix("ItemNumber") & ") ORDER BY Event.EventDate"
			    Set rsChildBarCode = OBJdbConnection.Execute(SQLChildBarCode)
			    Do While Not rsChildBarCode.EOF 
			        Response.Write "<FONT FACE=""Arial, Helvetica"" SIZE=""2"">" & vbCrLf
			        Response.Write FormatDateTime(rsChildBarCode("EventDate"), vbLongDate) & "<BR><IMG alt=""Barcode Image"" src=""/Barcode.asp?Barcode=" & rsChildBarCode("TicketNumber") & """>" & vbCrLf
			        Response.Write "</FONT>" & vbCrLf
