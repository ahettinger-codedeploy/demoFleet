...
Sys.WebForms.PageRequestManager.getInstance().add_initializeRequest(function(h,d){var g=d.get_postBackElement().name||d.get_postBackElement().id;
var e=h._form.__EVENTARGUMENT.value;
for(var f=0;
f<b.length;
f++){if(g.indexOf(b[f])!=-1||a.test(e)){d.set_cancel(true);
h._form.__EVENTTARGET.value=g.replace(/\_?ctl[0-9]+\_?/g,function(i){return i.replace(/^\_|\_$/g,"$");
});
h._form.submit();
...
this.set_height(m);
this._storeBounds();
},_calculateBoundsToFit:function(H,r){var u=this._renderMode==c.RenderMode.Lite;
var I=this.get_minWidth()-this._getHandlesWidth();
var G=this._tableElement?(h(this._tableElement.offsetHeight,10)-h(this.get_height(),10)):0;
var J=this._getTitleElement();
if(J){J.style.width=!u?"1px":"";
}var p;
var q;
var A=H.parentNode;
var B=A.style.height;
var C=A.style.width;
H.style.height=!u?"1px":"";
H.style.width=!u?"1px":"";
if(u){A.style.height="1px";
A.style.width="1px";
}if(this._dockMode){var z=H.style.overflow;
var D=H.style.position;
a(H).css({position:"absolute",overflow:"visible"});
try{var E=H.scrollWidth+$telerik.getBorderBox(H).horizontal;
q=E>I?E:I;
p=H.scrollHeight;
if(this._contentCell){Sys.UI.DomElement.addCssClass(this._contentCell,"rwLoading");
}}finally{a(H).css({position:D,overflow:z});
}}else{var m=r.getElementsByTagName("body")[0];
var o=m&&m.scrollWidth||0;
var n=m&&m.scrollHeight||0;
q=Math.max(r.scrollWidth,o);
if(q<I){H.style.width=I+"px";
q=r.scrollWidth;
}p=Math.max(r.scrollHeight,n);
}if(u){A.style.height=B;
A.style.width=C;
}var F=this._getRestrictionZoneBounds();
var K=F?F:this._getViewportBounds();
var M=this._getHandlesWidth()+q;
var t=this.get_defaultMinHeight()+p;
if(this._enableShadow&&$telerik.isIE&&!$telerik.isIE6&&this._tableElement){if(G>0){t-=G;
}}var L=M;
var s=t;
if(M>K.width){L=K.width;
s+=j;
}if(t>K.height){s=K.height;
L+=j;
}var y=this.get_minWidth();
var x=this.get_minHeight();
var v=h(this.get_maxHeight(),10);
var w=h(this.get_maxWidth(),10);
if(y>L){L=y;
}if(x>s){s=x;
}if(v&&v<s){s=v;
}if(w&&w<L){L=w;
}return{width:Math.ceil(L),height:Math.ceil(s),contentPageWidth:q,contentPageHeight:p};
...
if(!t&&this.get_height()<n.height){r.width+=j;
}}if(z){r.x=this.calcPosition(s.x,s.width,n.width,x.width,false);
}if(t){r.height=n.height;
if(!u){var o=this.get_width()<this.ui.content.scrollWidth;
r.height+=o?j:1;
}}if(q){r.y=this.calcPosition(s.y,s.height,n.height,x.height,true);
...
},_getCalculatedPopupBounds:function(){var t=this;
var m=t.isModal();
var r=this._getStoredBounds();
if(r){return r;
}var p=this._getCurrentBounds();
var o=this._offsetElement;
if((!m&&t._top==null&&t._left==null&&!o)||(m&&t.get_centerIfModal())){p=this._getCentralBounds();
}else{if(o){p.y=0;
p.x=0;
}else{var q=this._getViewportBounds();
p.x=q.scrollLeft;
p.y=q.scrollTop;
}var n=this._left?this._left:0;
p.x+=n;
var s=this._top?this._top:0;
p.y+=s;
}return p;
...
},set_modal:function(m){if(this._modal!=m){this._modal=m;
...
},clearCommandButton:function(p){$telerik.offEvent(p);
...
var h=g.max;
var i=g.min;
function d(o,m,n){return h(m,i(o,n));
}b.LightweightView=function(m){b.LightweightView.initializeBase(this,[m]);
this.ui=this.window.ui;
};
b.LightweightView.prototype={moveTo:function(m,n){this.ui.popupBehavior._setCoordinates(m,n);
this.ui.popupBehavior.show();
this._removeExplicitMSAjaxWidth();
this.ui.updateTitleWidth();
this.window.set_left(m);
this.window.set_top(n);
},setContent:function(n){var m=this.ui.content;
if(m&&n!=m){$telerik.disposeElement(m);
m.innerHTML="";
m.appendChild(n);
n=m;
}if(!this.window.isCreated()){this.ui.pendingContent=n;
}this.ui.createUI();
if(this.window._isPredefined&&this.ui.contentFrame){this.ui.contentFrame.style.display="none";
this.ui.contentFrame.style.height="";
}if(this.ui.contentFrame&&this.ui.contentFrame.parentNode!=n){this.ui.contentFrame.style.display="none";
}else{if(!this.window._dropDownTouchScroll&&Telerik.Web.UI.TouchScrollExtender._getNeedsScrollExtender()){this.window._createTouchScrollExtender(true);
}}this.ui.setContent(n);
},setUrl:function(n){var m=this.ui.contentFrame;
if(!m){return;
}m.src=n;
...
var m=this.ui.container;
$telerik.removeCssClasses(m,["rwNormalWindow","rwMinimizedWindow","rwMinimizedWindowShadow"]);
Sys.UI.DomElement.addCssClass(m,"rwMaximizedWindow");
this.window._configureMaximizeButton(true);
this.window._configureMinimizeButton();
this.window._maintainMaximizedSize();
this.window._maintainMaximizedSize();
var o=m._hideWindowedElementsIFrame;
if(o){Sys.UI.DomElement.removeCssClass(o,"rwMinimizedWindowOverlay");
this.ui.popupBehavior._handleElementResize();
}if(this.window.get_showOnTopWhenMaximized()){var p=m.style.zIndex;
var n=100000;
if(p){this.window._restoreZindex=p;
n=k(p,10)*2;
}m.style.zIndex=n;
}this.ui.updateTitleWidth();
},setMaximizeSize:function(){var m=this.ui.container;
if(!m){return;
}var p=this.window._getViewportBounds();
p.height-=1;
m.style.top=(p.scrollTop+p.y)+"px";
m.style.left=(p.scrollLeft+p.x)+"px";
var o=k(this.window.get_maxWidth(),10);
var n=k(this.window.get_maxHeight(),10);
if(o){p.width=o;
}if(n){p.height=n;
}$telerik.setSize(m,{width:p.width,height:p.height});
var q=this.window._getRestrictionZoneBounds();
if(!q){this.window._enablePageScrolling(false);
}if(this.window._dockMode){this.window.setWidthDockMode(p.width);
this.window.setHeightDockMode(p.height);
}this.ui.updateTitleWidth();
if(this.ui.contentFrame){this.setContentFixedHeight(p.height,this.ui.content);
}},minimize:function(){this.window._configureMinimizeButton(true);
this.window._configureMinimizeButton(true);
this.window._enablePageScrolling(true);
var m=this.ui.container;
$telerik.removeCssClasses(m,["rwNormalWindow","rwMaximizedWindow"]);
Sys.UI.DomElement.addCssClass(m,"rwMinimizedWindow");
if(this.window._enableShadow&&!$telerik.isIE6){this.ui.setShadowCssClass(true);
Sys.UI.DomElement.addCssClass(m,"rwMinimizedWindowShadow");
}var n=m._hideWindowedElementsIFrame;
if(n){Sys.UI.DomElement.addCssClass(n,"rwMinimizedWindowOverlay");
}this.setWidth(200);
this.setHeight(this.ui.titlebar.offsetHeight);
this.ui.getTitleNode().style.width="";
this.minimizeToZone(this.window.get_minimizeZoneID());
},setTabIndex:function(o){var n=this.ui.container;
var m=this.ui.getCommandButtons();
if(o!==null&&o!==l){n.setAttribute("tabIndex",o);
m.attr("tabIndex",o);
}else{n.removeAttribute("tabIndex");
m.removeAttr("tabIndex");
}},setWidth:function(m){this._setWidth(m);
this._repaintResizable();
},_setWidth:function(q){var p=this.ui.container;
var m=a(p);
var r=k(q,10);
var n=j(m.css("border-left-width"),10);
var o=j(m.css("border-right-width"));
r-=(n+o);
r-=this.ui.getHandlesWidth();
p.style.width=r+"px";
},setHeight:function(m){this._setHeight(m);
this._repaintResizable();
},_setHeight:function(t){var s=this.ui;
var p=s.container;
var m=a(p);
var r=k(t,10);
if(r==0){return;
}this.setContentFixedHeight(r,s.content);
var n=j(m.css("border-left-width"),10);
var o=j(m.css("border-right-width"),10);
var q=n+o;
q+=s.getHandlesWidth()/2;
r=r>q?(r-q):r;
p.style.height=r+"px";
},setContentFixedHeight:function(o,n){if(!n){return;
}var q=this.ui;
var p=this._substractWrappersBorder(o)-(q.getHandlesWidth()/2);
var r=this.window.isVisible();
if(this.window._visibleTitlebar){p-=r?k($telerik.getBounds(q.titlebar).height,10):k($telerik.getCurrentStyle(q.titlebar,"height"),10);
}var m=$telerik.$(n);
p-=j(m.css("padding-top"),10);
p-=j(m.css("padding-bottom"),10);
p-=j(m.css("border-top-width"),10);
p-=j(m.css("border-bottom-width"),10);
if(this.window._visibleStatusbar){p-=r?k(q.statusbar.offsetHeight,10):k($telerik.getCurrentStyle(q.statusbar,"height"),10);
p-=r?0:k($telerik.getBorderBox(q.statusbar).vertical,10);
}if(p>0){n.style.height=p+"px";
}},setContentWidth:function(m){this.ui.content.style.width="";
},setTitleText:function(m){if(!this.ui.title){return;
}this.ui.title.innerHTML=m||"&nbsp;";
this.ui.updateTitleWidth();
},showStatusbar:function(){this._displayStatusbar("",true);
},hideStatusbar:function(){this._displayStatusbar("none",false);
},_displayStatusbar:function(o,m){if(!this.ui.container){return;
}var n=this.ui.getStatusbar();
e(n,o);
if(this._hasAriaSupport()){c(n,m);
}this.setHeight(this.window.get_height());
},showShadow:function(){this.ui.setShadowCssClass(true);
},hideShadow:function(){this.ui.setShadowCssClass(false);
},moveOutOfSight:function(){var n=this.ui.popupBehavior;
if(n){this.window._storeBounds();
var m=n.get_elementToShow();
a(m).css({display:"",position:"absolute",top:"-10000px",left:this.window.get_leftHidingPoint()+"px",overflow:"hidden"});
if(m._hideWindowedElementsIFrame&&m._hideWindowedElementsIFrame.style){m._hideWindowedElementsIFrame.style.display="none";
}}this._ariaHide();
},isOutOfSight:function(){var m=this.ui.container;
if(!m){return false;
}var n=k(m.style.left,10);
return($telerik.isIE&&($telerik.isIE9||$telerik.isIE10))&&n===this.window.get_leftHidingPoint();
},enableMoveResize:function(m){var o=this;
var p=this.window;
this.disableMoveResize(m);
if(m.move&&!this.draggable){this.draggable=new Telerik.Web.UI.Widgets.Draggable(this.ui.container,{handle:this.ui.titlebar,shouldPreventDefault:false});
this.draggable.add_dragStart(Function.createDelegate(this,function(t,q){var y;
this._cancelDragSelection(q.get_domEvent());
p.setActive(true);
var x=p.isPinned()||p.isMaximized()||(p.isMinimized()&&p.get_minimizeZoneID());
q.set_cancel(x);
if(x){return;
}var w=p._getRestrictionZoneElement();
if(w){this.restrictBounds=p._getRestrictionZoneBounds();
var r=$telerik.getBorderBox(w);
var u=this.restrictBounds,v={x:u.x+r.left,y:u.y+r.top,width:u.width-r.horizontal,height:u.height-r.vertical},s={minX:v.x,minY:v.y,maxX:v.width+v.x,maxY:v.height+v.y};
y={x:d(p.get_left(),u.minX,u.maxX),y:d(p.get_top(),u.minY,u.maxY)};
o.moveTo(y.x,y.y);
t.set_constraints(s);
}p.raiseEvent("dragStart",new Sys.EventArgs());
}));
this.draggable.add_dragging(Function.createDelegate(this,function(r,q){this.ui.showContentOverlay();
$telerik.cancelRawEvent(q.get_domEvent());
q.set_cancel(p.isPinned()||p.isMaximized()||(p.isMinimized()&&p.get_minimizeZoneID()));
}));
this.draggable.add_dragEnd(Function.createDelegate(this,function(r,q){this.ui.hideContentOverlay();
var s=q.get_newPosition();
this.moveTo(s.x,s.y);
p._storeBounds();
this._repaintResizable();
p.raiseEvent("dragEnd",new Sys.EventArgs());
}));
}if(m.resize&&!this.resizable){this.resizable=new Telerik.Web.UI.Widgets.Resizable(this.ui.container,{appendHandleToElement:!$telerik.isIE,constraints:{minWidth:this.ui.get_uiMinWidth(),minHeight:this.ui.get_uiMinHeight()},handleSize:12,shouldPreventDefault:false});
this.resizable.add_resizeStart(a.proxy(this._resizeStartHandler,this));
this.resizable.add_resizing(a.proxy(this._resizingHandler,this));
this.resizable.add_resizeEnd(a.proxy(this._resizeEndHandler,this));
if(this.ui.bottomResizer&&!this.bottomResizerHandle){var n=new Telerik.Web.UI.Widgets.Handle(this.ui.bottomResizer,"SE",{cursorType:"se-resize"});
n.add_dragStart(a.proxy(this.resizable._handleDragStart,this.resizable));
n.add_dragging(a.proxy(this.resizable._handleDragging,this.resizable));
n.add_dragEnd(a.proxy(this.resizable._handleDragEnd,this.resizable));
this.bottomResizerHandle=n;
}}},_resizeStartHandler:function(o,m){this.ui.showContentOverlay();
$telerik.cancelRawEvent(m.get_domEvent());
this.window.setActive(true);
if(this.window.isMinimized()||this.window.isMaximized()){m.set_cancel(true);
}o.set_constraints({minWidth:this.ui.get_uiMinWidth(),minHeight:this.ui.get_uiMinHeight()});
this.restrictBounds=this.window._getRestrictionZoneBounds();
this.resizeHelper={width:this.window.get_width(),height:this.window.get_height()||this.ui.container.clientHeight,offset:{top:k(this.window.get_top(),10),left:k(this.window.get_left(),10)},borders:$telerik.getBorderBox(this.ui.container)};
var n=new Sys.CancelEventArgs();
this.window.raiseEvent("resizeStart",n);
m.set_cancel(n.get_cancel());
},touchCount:0,_resizingHandler:function(s,m){$telerik.cancelRawEvent(m.get_domEvent());
var p=this.restrictBounds,o=this._getResizeBounds(m),n=this.resizeHelper.borders,q={x:o.x-n.left,y:o.y-n.top,width:o.width+n.horizontal,height:o.height+n.vertical};
var r=this.window._checkRestrictionZoneBounds(p,q);
o.width=g.max(s.options.constraints.minWidth,o.width);
o.height=g.max(s.options.constraints.minHeight,o.height);
m.set_cancel(true);
if(r&&this.touchCount++>0){var t=this.ui.container.style;
t.left=o.x+"px";
t.top=o.y+"px";
this._setWidth(o.width);
this._setHeight(o.height);
this._currentResizeBounds=o;
}},_getResizeBounds:function(m){var o=m.get_direction();
var q=function(t){return o.indexOf(t)>-1;
};
var n=m.get_delta();
var r=this.resizeHelper;
var p=q("N")?-1:1;
var s=q("W")?-1:1;
return{x:r.offset.left+(q("W")?n.x:0),y:r.offset.top+(q("N")?n.y:0),width:r.width+s*n.x,height:r.height+p*n.y};
},_getMoveBounds:function(m){var n=m.get_newPosition();
var o=this.resizeHelper;
return{x:n.x,y:n.y,width:o.width,height:o.height};
},_resizeEndHandler:function(o,m){this.touchCount=0;
$telerik.cancelRawEvent(m.get_domEvent());
this.ui.hideContentOverlay();
var n=this._currentResizeBounds;
if(n){this.moveTo(k(n.x,10),k(n.y,10));
this.window.set_width(k(n.width,10));
this.window.set_height(k(n.height,10));
...
}},_isDir:function(m,n){return m.indexOf(n)>-1;
},disableMoveResize:function(m){if(this.draggable){this.draggable.dispose();
this.draggable=null;
...
this.resizable=null;
}},setCommandButtons:function(m){var n=[];
this.ui.clearCommandButtons();
for(var o=0;
o<m.length;
o++){n.push(this.ui.createCommandButton(m[o]));
}this.ui.updateTitleWidth();
return n;
},toggleCommand:function(n){var m=this.ui.getCommandButton(n);
if(!m){return;
}Sys.UI.DomElement.toggleCssClass(m,"on");
},minimizeToZone:function(n){if(!n){return;
}if(typeof(n)=="string"){n=$get(n);
}var m=this.ui.container;
if(m.parentNode!=n){m.parentNode.removeChild(m);
n.appendChild(m);
this.window.setVisible(true);
m.style.position="static";
if($telerik.isIE){m.style.display="inline";
}else{m.style.cssFloat="left";
}}},moveToDefaultParent:function(){var m=this.window._getDefaultParent();
m.insertBefore(this.ui.container,m.firstChild);
},revertToDefaultParent:function(){var m=this.window._getDefaultParent();
m.insertBefore(this.ui.container,m.firstChild);
},setActive:function(n){var m=this.ui.container;
if(n){this.window.setZIndexCss();
Sys.UI.DomElement.removeCssClass(m,"rwInactiveWindow");
}else{Sys.UI.DomElement.addCssClass(m,"rwInactiveWindow");
}this._repaintResizable();
},isActive:function(){var m=this.ui.container;
return(m&&!Sys.UI.DomElement.containsCssClass(m,"rwInactiveWindow"));
},onUrlChanging:function(){if(!this.ui.contentFrame){return;
}if(this.window.get_showContentDuringLoad()){var n=this.ui.statusbar;
if(n){Sys.UI.DomElement.addCssClass(n,"rwLoading");
}}else{var m=this.ui.contentFrame;
var o=m.style;
m.originalPosition=o.position;
m.originalHeight=o.height;
o.position="absolute";
o.top="-10000px";
if($telerik.isIE9Mode){this.setHeight(this.window._height);
}if(this.window._isWindowRightToLeft()&&$telerik.isChrome){o.width="1px";
}Sys.UI.DomElement.addCssClass(this.ui.container,"rwLoading");
}},onUrlChanged:function(){var q=this.ui.statusbar,o=this.ui.contentFrame;
if(this.window.get_showContentDuringLoad()){if(q){Sys.UI.DomElement.removeCssClass(q,"rwLoading");
}}else{o.style.position="";
o.style.height=o.originalHeight;
o.originalHeight=null;
if(this.window._isWindowRightToLeft()&&$telerik.isChrome){o.style.width="100%";
}Sys.UI.DomElement.removeCssClass(this.ui.container,"rwLoading");
if($telerik.isIE9Mode){if(this.window.isMaximized()){this.window._maintainMaximizedSize();
}else{if(!this.window.isMinimized()){this.setHeight(this.window.get_height());
}}}}if(q){this.window.set_status(this.window.get_navigateUrl());
}try{var m=o.contentWindow.document;
var p=m.title;
if(p&&p!=this.window.get_title()){this.window.set_title(p.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"));
}}catch(n){}},updatePopupZindex:function(){var m=this.ui.popupBehavior;
if(m){if(this.window.isVisible()){m.show();
}}},dispose:function(){},_substractWrappersBorder:function(m,n){var p=this.ui.container;
if(!p){return m;
}var o=$telerik.getBorderBox(p);
m-=n?o.horizontal:o.vertical;
return m;
},_removeExplicitMSAjaxWidth:function(){if(!this.window.get_width()){this.ui.container.style.width="";
}},_ariaHide:function(){if(this.window.get_enableAriaSupport()&&this.ui.container&&!this.window.isVisible()){this.ui.container.setAttribute("aria-hidden","true");
}},_cancelDragSelection:function(m){if(!$telerik.isTouchDevice){$telerik.cancelRawEvent(m);
}},_hasAriaSupport:function(){return this.window.get_enableAriaSupport();
},_preventDefault:function(m){if(m.preventDefault){m.preventDefault();
}m.returnValue=false;
}};
b.LightweightView.registerClass("Telerik.Web.UI.Window.LightweightView",b.ViewBase,b.IView);
function e(m,n){m&&(m.style.display=n);
}function c(m,n){m&&m.setAttribute("aria-hidden",!n);
}})($telerik.$,Telerik.Web.UI.Window);
Type.registerNamespace("Telerik.Web.UI.Window");
...
j.show();
return j;
},_initializeRadAlertKeyboardNavigation:function(k){var i=k;
var j;
if(i){j=a(i.get_popupElement()).find(g)[0];
i._keyboardNavigation=new d(i);
i._keyboardNavigation.addShortCut({eventName:e,element:j,key:f.SPACEBAR,action:function(){i.close(true);
}});
...
k.show();
return k;
},_initializeRadConfirmKeyboardNavigation:function(m){var j=m;
var k;
var l;
var i;
if(j){k=a(j.get_popupElement()).find(g);
l=k[0];
i=k[1];
j._keyboardNavigation=new d(j);
j._keyboardNavigation.addShortCut({eventName:e,element:l,key:f.SPACEBAR,action:function(){j.close(true);
}});
...
},_initializeRadPromptKeyboardNavigation:function(m){var j=m;
var k;
var l;
var i;
if(j){k=a(j.get_popupElement()).find(g);
...
i=k[1];
j._keyboardNavigation=new d(j);
j._keyboardNavigation.addShortCut({eventName:e,element:l,key:f.SPACEBAR,action:function(){var n=a(j.get_popupElement()).find(c)[0];
j.close(a(n).val());
}});
...
},_getStandardPopup:function(s,r,n,m){var q=this._createWindow(s+this._getUniqueId(),false);
q.set_destroyOnClose(true);
q.set_restrictionZoneID(null);
q.set_modal(true);
r=r.replace(/\$/g,"$$$");
var i=document.getElementById(this.get_id()+"_"+s.toLowerCase()+"template");
var j=$telerik.isIE&&!$telerik.isIE9Mode?"''":"";
var k=this._stringFormat(i.innerHTML,q.get_id(),r,n?n:j);
k=this._replaceLocalization(k,Telerik.Web.UI.RadWindowUtils.Localization);
var p=document.createElement("div");
p.innerHTML=k;
p.id=q.get_id()+"_content";
a(".rwDialogText, .rwDialogMessage",p).attr("id",q.get_id()+"_message");
q.set_behaviors(Telerik.Web.UI.WindowBehaviors.Close+Telerik.Web.UI.WindowBehaviors.Move);
q.set_visibleStatusbar(false);
q._isPredefined=true;
q.set_contentElement(p);
if(s!="prompt"&&m!=null&&m!="null"){var l=$telerik.$(p).find("div[class*='rwDialog']")[0];
if(l){if(m!=""){l.style.backgroundImage="url('"+m+"')";
}else{$telerik.$(l).removeClass("rad"+s);
$telerik.$(l).removeClass("rw"+s.substr(0,1).toUpperCase()+s.substr(1)+"Dialog");
}}}var o=q.get_contentElement().getElementsByTagName("INPUT")[0];
if(!o){o=q.get_contentElement().getElementsByTagName("A")[0];
}if(o&&o.focus){window.setTimeout(function(){var u=true;
if(o.setActive){try{o.setActive();
u=false;
}catch(t){}}if(u){o.focus();
}},0);
}return q;
...
(function() {
    function loadHandler() {
        var hf = $get('RadScriptManager_InfoServeNet_TSM');
        if (!hf) return;
        if (!hf._RSM_init) { hf._RSM_init = true; hf.value = ''; }
        hf.value += ';;System.Web.Extensions, Version=4.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35:en-US:5924cf72-83cf-477d-98eb-a608a92942c5:ea597d4b:b25378d2;Telerik.Web.UI:en-US:4b258e7c-7002-4d3e-9fa4-709d417eed98:16e4e7cd:f7645509:22a6274a:ed16cbdc:88144a7a:24ee1bba:c128760b:19620875:4877f69a:874f8ea2:f46195d3:92fe8ea0:fa31b949:490a9d4e:bd8f85e4';                                    
