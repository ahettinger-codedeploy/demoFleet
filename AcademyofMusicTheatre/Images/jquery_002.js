...
(/** @lends <global> */function( $ ) {
	...
	var DataTable = function( oInit )
	{
		...
		function _fnDraw( oSettings )
		{
			...
			if ( flag1 > 1 ) {
				jQuery("#blox_lists").find("tbody tr").each( function() {
			...
			if ( currUrl.indexOf("library.php") != -1 ) {
				var bloxList = [];
				bloxList[0] = 'myBlox';
				bloxList[1] = 'groupBlox';
				bloxList[2] = 'marketBlox';
				
				for ( var i = 0; i < 3; i++ ) {
					var sWord = jQuery("." + bloxList[i]).parent().find("#blox_lists_filter").find("input").val();
					if ( sWord == '' ) {
						//init( bloxList[i] );
						//MakeCurrBlox( bloxList[i] );
						var obj = jQuery("." + bloxList[i]).find(".currBlox");
						var countTr = obj.length;
						var className;
						var setClass;
						var expandCnt = 0;
						for ( var j = 0; j < countTr; j++ ) {
							if ( j % 2 == 0 ) setClass = "odd";
							else setClass = "even";
							obj.eq(j).removeClass("odd");
							obj.eq(j).removeClass("even");
							obj.eq(j).addClass(setClass);
						}
					}
				}
			}
			else {
				var sWord = jQuery("#blox_lists_filter").find("input").val();
				if ( sWord == '' ) {
					init();
					MakeCurrBlox();
					var obj = jQuery(".currBlox");
					var countTr = obj.length;
					var className;
					var setClass;
					var expandCnt = 0;
					for ( var i = 0; i < countTr; i++ ) {
						if ( i % 2 == 0 ) setClass = "odd";
						else setClass = "even";
						obj.eq(i).removeClass("odd");
						obj.eq(i).removeClass("even");
						obj.eq(i).addClass(setClass);
					}
				}
			}
		}
		...
		function _fnFeatureHtmlFilter ( oSettings )
		{
			...
			jqFilter.bind( 'keyup.DT', function(e) {
				...
				{
					if ( jQuery(".dataTables_filter").find("input").val() == '' ) sFlag = 0;
					else sFlag = 1;
	...
	DataTable.defaults = {
		 ...
		 *       <ul>
		 *         <li>'H' - jQueryUI theme "header" classes ('fg-toolbar xiUi-widget-header xiUi-corner-tl xiUi-corner-tr xiUi-helper-clearfix')</li>
		 *         <li>'F' - jQueryUI theme "footer" classes ('fg-toolbar xiUi-widget-header xiUi-corner-bl xiUi-corner-br xiUi-helper-clearfix')</li>
		 *       </ul>
		 *     </li>
	...
	$.extend( DataTable.ext.oJUIClasses, DataTable.ext.oStdClasses, {
		...
		/* Features */
		"sPaging": "dataTables_paginate fg-buttonset xiUi-buttonset fg-buttonset-multi "+
			"xiUi-buttonset-multi paging_", /* Note that the type is postfixed */
