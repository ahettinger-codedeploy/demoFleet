...
If OrderTypeFlag = "Y" And AllowedSeatTypeFlag = "Y" And AllowedSectionOK = "Y" And MaxDiscountsPerOrderPerEventOK = "Y" And MaxDiscountsPerOrderOK = "Y" And MaxDiscountSystemUsageOK = "Y" And MaxDiscountsPerEventOK = "Y" Then 'It's okay to apply to this order type.
	...
	If Now() < ExpirationDate Then
	    ...
	    SQLPrice = "SELECT Price FROM Price(NOLOCK) WHERE EventCode=" & EventCode & " AND SectionCode='" & SectionCode & "' AND SeatTypeCode=16" 
	    Set rsPrice = OBJdbConnection.Execute(SQLPrice)
        If Not rsPrice.EOF Then
    	    IndividualPrice = rsPrice("Price")
		    NewPrice = round(IndividualPrice * (1-CDbl(Clean(Request("Percentage")))/100), 2)
		    DiscountAmount = IndividualPrice - NewPrice
