...
<BR>
<BR>
<FONT FACE="<%= FontFace %>" COLOR="<%= HeadingFontColor %>" SIZE="2"><H3>Please select the wine dinner of your choice:</H3></FONT>
...
<%
    ...
    If EventCodeSelected = "128288" Or EventCodeSelected = "128049" Or EventCodeSelected = "128052" Or EventCodeSelected = "128050" Or EventCodeSelected = "128044" Then
        'EventCodeList = "128050, 128051"
        EventCodeList = "128044"
    ElseIf EventCodeSelected = "128305"  Or EventCodeSelected = "128055" Or EventCodeSelected = "128058" Or EventCodeSelected = "128056" Or EventCodeSelected = "128054" Then
        EventCodeList = "128054"
    ElseIf EventCodeSelected = "128309"  Or EventCodeSelected = "128066" Or EventCodeSelected = "128063" Or EventCodeSelected = "128062" Then
    ...
    SQLTicketCount = "SELECT COUNT(LineNumber) AS TotalTickets FROM OrderLine (NOLOCK) WHERE OrderNumber = " & CleanNumeric(Session("OrderNumber"))
    Set rsTicketCount = OBJdbConnection.Execute(SQLTicketCount)
    If not rsTicketCount.EOF Then
        NumTickets = rsTicketCount("TotalTickets")/4
    Else
        NumTickets = 1
    End If
    rsTicketCount.Close
...
<BR>
<BR>
<FONT FACE="<%= FontFace %>" COLOR="<%= HeadingFontColor %>" SIZE="2"><H3>Please select the seminar of your choice:</H3></FONT>
...
<%
    ...
    If EventCodeSelected = "128288" Or EventCodeSelected = "128049" Or EventCodeSelected = "128052" Or EventCodeSelected = "128050" Or EventCodeSelected = "128044" Then
        'EventCodeList = "128044"
        EventCodeList = "128050"
    ElseIf EventCodeSelected = "128305"  Or EventCodeSelected = "128055" Or EventCodeSelected = "128058" Or EventCodeSelected = "128056" Or EventCodeSelected = "128054" Then
        EventCodeList = "128056"
    'slm 4.18.08
    'ElseIf EventCodeSelected = "128309"  Or EventCodeSelected = "128066" Or EventCodeSelected = "128063" Or EventCodeSelected = "128064" Or EventCodeSelected = "128062" Then
        'EventCodeList = "128064"
    Else 'Invalid EventCode
        Call Continue
    End If
    ...
    If Not rsPackages.EOF Then
%>
    <tr><td align="center"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><FONT face="Geneva, Arial, Helvetica, san-serif"><%=rsPackages("Act")%> - <%=FormatDateTime(rsPackages("EventDate"),vbShortDate)%></FONT></FONT></td></tr>
    <tr><td align="center"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><FONT face="Geneva, Arial, Helvetica, san-serif"><%=rsPackages("ActComments")%></FONT></FONT></td></tr>
<%
        Do While Not rsPackages.eof
%>
...
Sub AddToCart(EventCode, SectionCode, OfferQuantity, SurveyType, OriginalEventCode, SeatTypeCode)
    ...
    If TotalSeatCount <= MaxTickets Then 'Process Request
	    ...
	    For i = 1 To OfferQuantity
		    ...
		    'TTT - 3/3/2008 - Modified to update the SeatTypeCode/Price/Discount for Subscription Events
		    SQLUpdateSeatType = "UPDATE OrderLine WITH (ROWLOCK) SET SeatTypeCode = " & SeatTypeCode & ", Price = " & Price & ", Discount = " & Price & " WHERE OrderNumber = " & Session("OrderNumber") & " AND ItemNumber IN (SELECT ItemNumber FROM SEAT (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber") & " AND EventCode = " & EventCode & ")"
