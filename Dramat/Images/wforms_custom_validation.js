...
/**
 *	Handle deleting resumed upload elements
 */
/*
...
wFORMS.helpers.deleteResumedFilesFinder = function(context){
	...
	return base2.DOM.HTMLElement.querySelectorAll(context,".deleteUploadedFileControl");
}
wFORMS.helpers.deleteResumedFilesHandler = function(action,context){
	wFORMS.helpers.deleteResumedFilesFinder().forEach(function(elem){
		if(!elem.addEventListener){
			wFORMS.standardizeElement(elem);
		}
		elem.addEventListener('click',function(){
			var id = elem.id.replace("tfa_uploadControl_","");
			wFORMS.helpers.deleteResumedFiles(id);
		});
	});
}

base2.DOM.HTMLElement.addEventListener(document,"DOMContentLoaded",function(){
	wFORMS.helpers.deleteResumedFilesHandler();
});

_bOnRemove = wFORMS.behaviors.repeat.onRemove;
wFORMS.behaviors.repeat.onRemove = function(elem){
	_f = document.forms[0];
	wFORMS.helpers.deleteResumedFilesFinder(elem).forEach(function(e){
		_f.appendChild(elem);
		var id = e.id.replace("tfa_uploadControl_","");
		var checkbox = document.getElementById("tfa_uploadDelete_"+id);
		checkbox.style.display = "none";
		checkbox.checked = true;
		_f.appendChild(checkbox);
		_f.removeChild(elem);
	});
	_bOnRemove(elem);
}

_bOnRepeat = wFORMS.behaviors.repeat.onRepeat;
