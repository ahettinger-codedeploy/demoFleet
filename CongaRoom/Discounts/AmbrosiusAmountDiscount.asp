...
'JAI 6/2/9 - Added HistoryBeginDate - Only look at orders which are more recent than HistoryBeginDate.  Used for MaxDiscountsSystemUsage.
If Clean(Request("HistoryBeginDate")) <> "" Then
	HistoryBeginDate = CDate(Clean(Request("HistoryBeginDate")))
End If
...
If MaxDiscountSystemUsage <> 0 Then
    ...
    End If
	SQLDiscCount = "SELECT COUNT(OrderLine.LineNumber) AS LineCount FROM OrderLine (NOLOCK) WHERE OrderLine.DiscountTypeNumber = " & DiscountTypeNumber & SQLHistoryBeginDate
	Set rsDiscCount = OBJdbConnection.Execute(SQLDiscCount)
	TicketCount = rsDiscCount("LineCount")
	rsDiscCount.Close
	Set rsDiscCount = nothing
...
'JAI 9/23/9 - Compare # of Infusion tickets on order with # of applied Ambrosius tickets
InfusionCountOK = "Y"
'Count total # of Infusion tickets
SQLDiscCount = "SELECT COUNT(*) AS InfusionCount FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber AND OrderLine.ItemType IN ('Seat', 'SubFixedEvent') WHERE OrderLine.OrderNumber = " & OrderNumber & " AND Seat.EventCode = 217173"
Set rsDiscCount = OBJdbConnection.Execute(SQLDiscCount)
InfusionCount = rsDiscCount("InfusionCount")
rsDiscCount.Close
Set rsDiscCount = nothing
'Count # of Applied Ambrosius tickets
SQLDiscCount = "SELECT COUNT(*) AS AmbrosiusCount FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber AND OrderLine.ItemType IN ('Seat', 'SubFixedEvent') WHERE OrderLine.OrderNumber = " & OrderNumber & " AND Seat.EventCode = " & EventCode & " AND OrderLine.DiscountTypeNumber = " & DiscountTypeNumber 
Set rsDiscCount = OBJdbConnection.Execute(SQLDiscCount)
AmbrosiusCount = rsDiscCount("AmbrosiusCount")
rsDiscCount.Close
Set rsDiscCount = nothing
If AmbrosiusCount >= InfusionCount Then
	InfusionCountOK = "N"
End If

'REE 11/29/6 - Added MaxDiscountsPerEventOK to criteria
If InfusionCountOK = "Y" AND OrderTypeFlag = "Y" And AllowedSeatTypeFlag = "Y" And MaxDiscountsPerEventOK = "Y" And MaxDiscountsPerOrderPerEventOK = "Y" And MaxDiscountsPerOrderOK = "Y" And MaxDiscountSystemUsageOK = "Y" And AllowedSectionOK = "Y" Then 'It's okay to apply to this order type and seat type.
	...
	If Now() > StartDate And Now() < ExpirationDate Then
		...
		If Clean(Request("Price")) > 0 Then
			NewPrice = Round(Clean(Request("Price")),2) - Round(Clean(Request("DiscountAmount")),2)
			AppliedFlag = "Y"
		End If
		If NewPrice < 0 Then
			NewPrice = 0
