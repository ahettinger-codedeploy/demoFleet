 ...
 *	    contentitemlist.grid
 * 
 */

var ContentItemListGridLayout = (function() {
	...
	function setSlideWidth($gridModule, numSlides) {
		...
		// set margin-right of last slide in each row to 0
		var lastNum = numSlides - 1;
		$slides.filter(function (index) {return (index % numSlides == lastNum);}).css("margin-right","0px");
	...
	return {
		...
		init: function(settings) {
			settings = rjQuery.extend({
				...
				overrideAutoWidth: "true",	// override automatic width setting; TODO: remove when grid redo is complete
			...
			// TODO: Remove when grid rework is done (override always on)
			$gridModule.data("autoWidth", false);
...
WindowResizeResponder.register({
	"method": ContentItemListGridLayout.layout,
	"getElements": ContentItemListGridLayout.getGridModules,
	"when": "ready",
	"startupLoop": true,
	"startupCallback": ContentItemListGridLayout.showContent,
