...
SQLEP = "SELECT EP.EventCode, EP.Area, EP.SeatPrefix, SeatNo, OL.ID, OL.OrderNumber FROM ALPLMFoundationEventPerformances AS EP (NOLOCK) INNER JOIN ALPLMFoundationOrderLineItems AS OL (NOLOCK) ON EP.ID = OL.Performance WHERE OL.ItemNumber IS NULL AND EP.EventType <> 'general'"
Set rsEP = dbCustomerImport.Execute(SQLEP)

Do Until rsEP.EOF

    EventCode = rsEP("EventCode")
    SectionCode = rsEP("Area")
    Row = rsEP("SeatPrefix")
    Seat = rsEP("SeatNo")
    ...
    SQLSeat = "SELECT ItemNumber FROM Seat (NOLOCK) WHERE EventCode = " & EventCode & " AND SectionCode = '" & SectionCode & "' AND Row = '" & Row & "' AND Seat = '" & Seat & "' AND StatusCode = 'A' AND (OrderNumber = 0 OR OrderNumber IS NULL)"
    ...
    If Not rsSeat.EOF Then
        ...
        SQLUpdateSeat = "UPDATE Seat WITH (ROWLOCK) SET StatusCode = 'S', StatusDate = '9/28/9', OrderNumber = " & OrderNumber & " WHERE ItemNumber = " & ItemNumber & " AND EventCode = " & EventCode & " AND StatusCode = 'A'"
        Set rsUpdateSeat = OBJdbConnection.Execute(SQLUpdateSeat)
        
        SQLUpdateOL = "UPDATE ALPLMFoundationOrderLineItems WITH (ROWLOCK) SET ItemNumber = " & ItemNumber & " WHERE ID = " & ID & " AND ItemNumber IS NULL"
    ...
    Else
        ...
        ErrorLog("ALPLM Foundation Conversion Error - Seat Not Found - " & SQLSeat)
        Exit Do
