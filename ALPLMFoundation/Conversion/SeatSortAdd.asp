...
SQLOL = "SELECT ID, Performance FROM ALPLMFoundationOrderLineItems (NOLOCK) WHERE IntFee1 = 0 AND Performance IN (SELECT ID FROM ALPLMFoundationEventPerformances (NOLOCK) WHERE EventType = 'general') ORDER BY Performance"
...
Do Until rsOL.EOF
    ...
    SeatSort = SeatSort + 1
    SQLSS = "UPDATE ALPLMFoundationOrderLineItems WITH (ROWLOCK) SET IntFee1 = " & SeatSort & " WHERE ID = " & ID
