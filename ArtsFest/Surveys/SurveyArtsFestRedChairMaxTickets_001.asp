...
'#############################
'This script is to be used as a template for other maximum ticket limits by customer.  
'Add the appropriate SurveyNumber and copy the script to 'Survey[Client Name]MaxTickets.asp' or something similar.  
'Please leave this template in place.
'#############################

Page = "SurveyArtsFestRedChairMaxTickets.asp"
SurveyNumber = 227 

MaxAllowed = CInt(CleanNumeric(Request("MaxAllowed")))
OnlineFlag = UCase(Clean(Request("OnlineFlag")))
OfflineFlag = UCase(Clean(Request("OfflineFlag")))
