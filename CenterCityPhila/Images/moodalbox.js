function bindLeftLink() {
	if ($('PhotoViewerLeft'))
	{
		$('PhotoViewerLeft').addEvent('click', function(){
																										
			var strLeftLink =  $('PhotoViewerLeft').getProperty('href');
			$('PhotoViewerLeft').setProperty('href', 'javascript:;');
			
			new Ajax(strLeftLink, {
				method: 'get',
				update: $('mb_contents'),
				onComplete: function(){
					bindLeftLink();
...
function bindRightLink() {
	if ($('PhotoViewerRight'))
	{
		$('PhotoViewerRight').addEvent('click', function(){
																										 
			var strLeftLink =  $('PhotoViewerRight').getProperty('href');
			$('PhotoViewerRight').setProperty('href', 'javascript:;');
...
var _INITIAL_WIDTH			= 508;		// Initial width of the box (px)
var _INITIAL_HEIGHT			= 608;		// Initial height of the box (px)
var _CONTENTS_WIDTH 		= 508;		// Actual width of the box (px)
var _CONTENTS_HEIGHT		= 608;		// Actual height of the box (px)
var _DEF_CONTENTS_WIDTH		= 508;		// Default width of the box (px) - used for resetting when a different setting was used
var _DEF_CONTENTS_HEIGHT	= 608;		// Default height of the box (px) - used for resetting when a different setting was used
...
var MOOdalBox = {
	...
	init: function (options) {
		...
		$A($$('a')).each(function(el){
			...
			if(el.rel && el.href && el.rel.test('^overlay', 'i')) {
		...
		// cw
		this.centercontainer = new Element('div').setProperty('id', 'mb_centercontainer').setStyles({width: this.options.initialWidth+'px', height: this.options.initialHeight+'px', marginLeft: '-'+(this.options.initialWidth/2)+'px', display: 'none'}).injectInside(document.body);
		this.centercontainerinner = new Element('div').setProperty('id', 'mb_centercontainerinner').setStyles({width: this.options.initialWidth+'px', height: this.options.initialHeight+'px', marginLeft: '-'+(this.options.initialWidth/2)+'px', display: 'none'}).injectInside(document.body);
		// the center element
		...
		this.fx = {
			overlay: 	this.overlay.effect('opacity', { duration: 0, onComplete: nextEffect }),
			contents: 	this.contents.effect('opacity', { duration: 1000, onComplete: nextEffect })
		};
	...
	open: function(sLinkHref, sLinkTitle, sLinkRel) {
		...
		this.centercontainer.setStyles({top: this.top+'px', display: ''});
		this.centercontainerinner.setStyles({top: this.top+'px', display: ''});
	...
	loadContents: function() {		
		...
		this.overlay.className = 'mb_loading';
	...
	ajaxFailure: function (){
		...
		this.centercontainer.setStyle('cursor', 'pointer');
		this.centercontainerinner.setStyle('cursor', 'pointer');
	...
	nextEffect: function() {
		...
		case 1:
			...
			this.centercontainer.className = '';
			this.centercontainerinner.className = '';
		...
		case 2:
			this.center.addClass('mb_ready');
			this.step++;
			//console.log('2');
		
		case 3:
			if (this.center.hasClass('mb_ready'))
			{
	...
	close: function() {
		this.overlay.removeClass("mb_loading");
		if(this.step < 0) return;
		this.step = -1;
		for(var f in this.fx) this.fx[f].clearTimer();
		this.center.style.display = this.centercontainer.style.display = 'none';
		this.centercontainerinner.style.display = 'none';
