...
If DepartureActCode = 66492 Then
    ...
    If CDate(Request("ReturnDate")) >= CDate("10/1/2011") Then
        ReturnTime = "4:00 PM"
    Else
        If CDate(FormatDateTime(Now(), vbShortDate)) >= CDate("10/1/2011") Then
            ReturnTime = "4:00 PM"
        Else
            ReturnTime = "5:00 PM"
        End If
    End If
Else
    If CDate(Request("DepartureDate")) >= CDate("10/1/2011") Then
...
<HTML>
<HEAD>
<TITLE>Tix - Event Selection</TITLE>
...
<BODY BGCOLOR="#FFFFFF"<% If Message <> "" Then %> onload="alert('<%=Message%>');"<% End If %>>
<CENTER>
...
End If    
%>
<form action="EventSelectionCatalinaFerries.asp" method="post">
...
'ErrorLog("Return Date - 453 " & ReturnDate & "")  
'ErrorLog("DepartureDate - 453 " & DepartureDate & "")   
...
If ReturnDate < DepartureDate Then        
    Message = "Please select a Return Date after Departure Date or not."
    Call DisplayForm(Message)
    Exit Sub
End If
...
ErrorLog("ReturnEventCode 544 " & ReturnEventCode & "")
ErrorLog("DepartureEventCode 544 " & DepartureEventCode & "")
...
ErrorLog("OptionCount " & OptionCount & "")
...
Response.Redirect("/Management/CustomerLookup.asp")
