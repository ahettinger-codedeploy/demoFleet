...
%>
    ...
    <FONT FACE="<%= FontFace %>" COLOR="<%= HeadingFontColor %>" SIZE="2"><H3><%=rsPackages("Act")%></H3></FONT>
    
    <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=rsPackages("ActComments")%></FONT><br /><br />    
    
    <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><b>Event Date: <%=rsPackages("EventDate")%> - Ticket Price: <%=FormatCurrency(rsPackages("Price"),2)%></b></FONT><br /><br />
    ...
    <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2">Enter Quantity: <INPUT TYPE="text" NAME="Quantity" maxlength="3" size="3" /></FONT>
...
Sub AddTickets
	...
	For Each EventCd In Request("EventCode")
	    If Request("Quantity")(index) <> "" And CInt(Request("Quantity")(index)) > 0 Then
	        If CInt(Request("Quantity")(index)) > CInt(Request("Qty_" & Request("EventCode")(index))) Then
	            Message = "You cannot exceed the maximum of VIP Lounge tickets of " & Request("Qty_" & Request("EventCode")(index)) & " for Add-on Event # " & index
	            Call AddOnSurvey(Message)
	            Exit Sub
	        End If
	    End If
	...
	For Each EventCd In Request("EventCode")
	    If Request("Quantity")(index) <> "" And Request("Quantity")(index) <> "0" Then
	        Call AddToCart(CleanNumeric(Request("EventCode")(index)), Clean(Request("SectionCode")(index)), CleanNumeric(Request("Quantity")(index)))
	    End If
	    index = index + 1
	Next
	Call PartTwo(Message)
	...
	Select Case ReturnCode
		Case 0	'Stored Procedure executed properly.  Continue.
			...
			For i = 1 To OfferQuantity
				...
				SQLPrice = "SELECT Price, Surcharge FROM Price (NOLOCK) WHERE EventCode = " & EventCode & " AND SectionCode = '" & SectionCode & "' AND SeatTypeCode = 5609" '& Clean(Request("SeatTypeCode"))
				...
				SQLUpdateSeatType = "UPDATE OrderLine WITH (ROWLOCK) SET SeatTypeCode = 5609, Price = " & Price & ", Surcharge = 0 WHERE OrderNumber = " & Session("OrderNumber") & " AND ItemNumber IN (SELECT TOP 1 ItemNumber FROM OrderLine (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber") & " AND SeatTypeCode IS NULL AND ItemType IN ('Seat', 'SubFixedEvent') ORDER BY LineNumber DESC)"
...
%>
    ...
    <tr><td><font size=2>
        <select name="Answer2">
            <option value="">- Select Port -</option>
            <option value="Departure Port: Avalon">Avalon</option>
            <option value="Departure Port: Two Harbors">Two Harbors</option>
        </select>
    </font></td></tr>
<%    
    End If
