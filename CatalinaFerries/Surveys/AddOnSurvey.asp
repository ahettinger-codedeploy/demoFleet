...
<thead>
    <tr>
        ...
        <th scope="col" class="category" colspan="2"><b>Special Offers/Add-Ons</b></th>
...
<% If rsPackages("SectionCode") = "ADDL" Then %>
    <% If rsPackages("SeatTypeCode") = "5613" Then 'display add-on tickets' description only once for the first tickettype %>
        <H3>Would you like to add any of the following?</H3>
        <font size="1">**Please note: A ticket is required for all infants 2 and under.</font><br />
    <% End If %>
<% Else %>
        <H3><%=rsPackages("Act")%><BR><%=rsPackages("EventDate")%></H3>
        <HR>
        <H3>Upgrade your seats to the Admiral's Lounge!</H3>Travel back to the mainland in style in the VIP Admiral's Lounge. Includes complimentary drink & snack, priority boarding, and private, reserved indoor seating on the top deck with 360° views. Only $15 per person (each way) in addition to base fair. Enjoy a beer or cocktail as you relax on your way back home.<br /><font size="1">**You'll need one Admiral's Lounge upgrade for each ticket you'd like to upgrade.</font><br />
<% End If %>
        <B><%=rsPackages("SeatType")%> - Price: <%=FormatCurrency(rsPackages("Price"),2)%></b>
        ...
        Enter Quantity: 
<%
    If rsPackages("SeatTypeCode") = "5609" Then
        SQLSeatCnt = "SELECT EventCode FROM Seat (NOLOCK) WHERE EventCode = " & rsPackages("EventCode") & " AND SectionCode = 'ADML' AND StatusCode = 'A'"
        Set rsSeatCnt = OBJdbConnection.Execute(SQLSeatCnt)
        If rsSeatCnt.EOF Then
%>
...
<tr>
<td align="center" colspan="4"><BR><INPUT type="submit" value="Add to Cart" /></td>
</tr>
</TABLE>
</FORM>
...
<%    
    ...
    SQLReqAct = "SELECT Event.EventDate FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber INNER JOIN Event (NOLOCK) ON Seat.EventCode = Event.EventCode WHERE OrderLine.OrderNumber = " & Session("OrderNumber") & " AND OrderLine.ItemType IN ('Seat', 'SubFixedEvent') AND Event.ActCode IN (66503)"
    Set rsReqAct = OBJdbConnection.Execute(SQLReqAct)
    If Not rsReqAct.EOF Then
        If CDate(FormatDateTime(rsReqAct("EventDate"), 2)) >= CDate("10/1/2011") Then
            AvalonTime = " (4:00pm)"
            TwoHarborsTime = " (3:00pm)"
        Else
            AvalonTime = " (5:00pm)"
            TwoHarborsTime = " (4:00pm)"
        End If
%>
    ...
    <tr><td><font size=2>
        <select name="Answer2">
            <option value="">- Select Port -</option>
            <option value="Departure Port: Avalon<%=AvalonTime%>">Avalon</option>
            <option value="Departure Port: Two Harbors<%=TwoHarborsTime%>">Two Harbors</option>
        </select>
    </font></td></tr>
<%    
    End If
