...
<!--LSP 7/8/11 - Initial DEvelopment-->

<%
'RAS 8.24.11 - Marina del Rey ticket modifications

'Update their E-Ticket Format so that if the E-Ticket is for 
'ActCode = 66503 Catalina to Marina del Rey 
'then it should display Answer #2 from Survey # 1059 for the corresponding order.  
'The answer should be displayed next to the Ticket Type field.
...
If Not rsOrderNum.EOF Then
	...
	If Not rsTix.EOF Then
		...
		SQLPort = "SELECT Answer FROM SurveyAnswers (NOLOCK) WHERE SurveyAnswers.SurveyNumber = 1059 AND OrderNumber = " & OrderNumber & " AND AnswerNumber = 2"
		Set rsPort = OBJdbConnection.Execute(SQLPort)  
		    If Not rsPort.EOF Then
		        DeparturePort = rsPort("Answer")
		    End If
	    rsPort.Close
	    Set rsPort = nothing          
		...
		'Begin Ticket
			...
			Response.Write "<TABLE WIDTH=""615"" HEIGHT=""240"" BORDER=""1"" BACKGROUND=""/clients/CatalinaFerries/ETicket/Images/Background.gif"" BGCOLOR=""#FFD718"" CELLPADDING=""0"" CELLSPACING=""0"">" & vbCrLf
			Response.Write "<TR>" & vbCrLf
			...
			Response.Write "<IMG SRC=""/clients/CatalinaFerries/ETicket/Images/Logo.gif"">" & vbCrLf
			...
			If rsTIX("ActCode") = 66503 Then
			    Response.Write "<FONT FACE=""Arial,Helvetica"" SIZE=""3"">" & FormatDateTime(rsTix("EventDate"), vbLongDate) & "</FONT><BR>" & vbCrLf
			    Response.Write "<FONT FACE=""Arial,Helvetica"" SIZE=""2""><B>" & DeparturePort & "</B></FONT><BR><BR>" & vbCrLf
			    'Response.Write "<FONT FACE=""Arial,Helvetica"" SIZE=""2"">" & rsTix("SeatType") & "  $" & FormatNumber(rsTix("Price"),2) &"</FONT><BR>" & vbCrLf
                Response.Write "<FONT FACE=""Arial,Helvetica"" SIZE=""2""><B>" & rsTix("Section") & "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" & rsTix("SeatType") & "&nbsp;&nbsp;&nbsp;" & "$" & FormatNumber(rsTix("Price"),2) & "</B></FONT><BR>" & vbCrLf
			Else
			...
			Response.Write "<IMG SRC=""/clients/CatalinaFerries/ETicket/Images/Bottom_Bar.gif"">" & vbCrLf
			Response.Write "</TD>" & vbCrLf
			Response.Write "</TR>" & vbCrLf
			Response.Write "</TABLE>" & vbCrLf
			Response.Write "</TD>" & vbCrLf
		...
		'Begin Body Space
			...
			Response.Write "<BR>" & vbCrLf
			Response.Write "Directons from LAX<BR>" & vbCrLf
			Response.Write "Distance: 4.9 mi (about 10 mins)<BR>" & vbCrLf
			Response.Write "1. Head north - go 0.1 mi<BR>" & vbCrLf
			Response.Write "2. Bear left onto the Sky Way ramp - go 0.1 mi<BR>" & vbCrLf
			Response.Write "3. Continue on Sky Way - go 0.3 mi<BR>" & vbCrLf
			Response.Write "4. Take the ramp to Sepulveda Blvd North (CA-1) - go 0.3 mi<BR>" & vbCrLf
			Response.Write "5. Turn right at W 96th St - go 0.1 mi<BR>" & vbCrLf
			Response.Write "6. Bear right at S Sepulveda Blvd - go 0.1 mi<BR>" & vbCrLf
			Response.Write "7. Turn left at Lincoln Blvd - go 3.3 mi<BR>" & vbCrLf
			Response.Write "8. Turn left at Fiji Way - go 0.6 mi<BR>" & vbCrLf
			Response.Write "<BR>" & vbCrLf	
			Response.Write "Nearest lot is public lot #1 which charges $10 per 24 hours; cash only.<BR>" & vbCrLf
			Response.Write "Boats depart from dock directly behind El Torito restaurant, to the right of the lighthouse.<BR>" & vbCrLf
			...
			Response.Write "<BR>" & vbCrLf
			Response.Write "<BR>" & vbCrLf
			Response.Write "<IMG SRC=""/clients/CatalinaFerries/ETicket/Images/map.gif"">" & vbCrLf
			Response.Write "<BR>" & vbCrLf
			Response.Write "</FONT>" & vbCrLf
			Response.Write "</TD>" & vbCrLf
			Response.Write "</TR>" & vbCrLf
		...
		'Begin Ad Space
			...
			Response.Write "<TR>" & vbCrLf
			Response.Write "<TD ALIGN=""center"">" & vbCrLf
			Response.Write "<IMG SRC=""/clients/CatalinaFerries/ETicket/Images/Ad.gif"" HEIGHT=""200"">" & vbCrLf
			Response.Write "</TD>" & vbCrLf
			Response.Write "</TR>" & vbCrLf
			Response.Write "</TABLE>" & vbCrLf
