WebCom.Components.Navigation.Positioner  = {
  ...
  init : function(subNav, options, parentOptions) {
    ...
    this.root = this.findRoot(subNav.parents("ul[className*='webcomMenu']:first"));
  ...
  findRoot : function(menu) {
    var p = menu.parent("div[className*='webcomMenu-']");
    var $d = null;
    while (p.length > 0) {
      $d = p;
      p = p.parent("div[className*='webcomMenu-']:not([className*='webcomMenu-offset'])");
    }
    return $d;
  },
