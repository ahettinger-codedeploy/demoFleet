...
function mediaAction (thevar){
	...
	var properties = 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=801,height=' + popHeight;
	if (thevar.indexOf('grp=') != -1) {
		thevar = thevar.replace('grp=','g=');
	}
	var theurl = pathprefix + 'cf_media2/index.cfm?' + thevar;
...
function embedFlash(divID,playername,embedWidth,embedHeight,aspectRatio,flashvars,params,attributes,minVersion){
	$j(function(){
		...
		if (typeof flashvars.mini == 'boolean' && !flashvars.mini) {
			if (typeof flashvars.siteurl == 'string') expressInstallPath = flashvars.siteurl + '/cf_media2/scripts/expressInstall.swf';
			else expressInstallPath = '../cf_media2/scripts/expressInstall.swf';
		}
...
function mailMe(which){
	if(typeof pageTracker != "undefined"){
		...
		pageTracker._trackEvent('email links', ah , trackingpath + ' [' + unescape(which).replace(/\[nospam\]/,"@") + ']');
	}
...
function editpopUp(id,text,layout,bgcolor,path,width){
	...
	var url = path + "editor/tinymce/popup.cfm?whichelement="+text+idString;
	var properties = 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=yes,width=560,height=500';
...
function editorFileBrowser(field_name, url, type, win) {
	...
	popUrl = "../../../../../../cf_filemanager/adminfiles.cfm?do=refresh&opener="+theOpener;
