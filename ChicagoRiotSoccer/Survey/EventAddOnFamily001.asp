...
'===============================================
'Chicago Riot Soccer (12/9/2010)

'Survey counts the number of "Family Pack" seats per event on each order.
'Requires that patron purchase a minimum of 4 seats. 

SectionList = "109F,112F,108F,101F,106F,113F,107F"
RequiredSeatList = "16"
RequiredSeatName = "Family Pack"
MinimumSeatCount = 4

'Silvia’s Client
'Required Event codes: 
'316157  1/30/2011   Chicago Riot vs. Baltimore Blast 
'316158  2/13/2011   Chicago Riot vs. Omaha Vipers 
'316160  2/26/2011   Chicago Riot vs. Missouri Comets 

'Offer Event Codes:
'335322  1/30/2011   Food Ticket
'335350  2/13/2011   Food Ticket
'320249  2/26/2011   Food Ticket
...
Dim RequiredEventCode(3)
RequiredEventCode(1) = "316157"
RequiredEventCode(2) = "316158"
RequiredEventCode(3) = "316160"

NumberRequiredSeatType = 1

Dim RequiredSeatCode(1)
RequiredSeatCode(1)="16" 'Premium

OfferEventCount = 3

Dim OfferEventCode(3)
OfferEventCode(1) = "335322"
OfferEventCode(2) = "335350"
OfferEventCode(3) = "320249"
...
' the number of required tickets on the required event
...
For i = 1 To RequiredEventCount
        ...
        If Not rsRequiredCheck.EOF Then	
            ...
            RequiredTicketCount = RequiredTicketCount + 1
        
    	    SQLEventList = "SELECT EventCode FROM Event (NOLOCK) WHERE Event.EventCode = " & RequiredEventCode(i) & " "
...
RequiredEventList = Left(EventList,((Len(EventList))-1))

ErrorLog("RequiredEventList: " & RequiredEventList & "")
...
If OfferTicketCount<> 0 Then
    Call DisplayOfferEvent(RequiredEventList)
Else
    Call Continue
End If
...
Sub DisplayOfferEvent(RequiredEventList)
...
'Offer Act and Date
SQLPackages = "SELECT Event.EventCode, Event.EventDate, Event.Comments AS EventComments, Act.Act, Act.Comments AS ActComments FROM Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode WHERE Event.EventCode IN (" & RequiredEventList & ") "
