...
'Fixed Subscription Event Code
'allows this discount to be applied to the fixed subscription event

DIM FixedSubEventCode
DIM FixedSubActCode

If Clean(Request("FixedSubEventCode")) <> "" Then

	FixedSubEventCode = Clean(Request("FixedSubEventCode"))
	
	SQLActLookUp = "SELECT Event.ActCode FROM Event (NOLOCK) INNER JOIN ACT ON Event.ActCode = Act.ActCode WHERE EventCode = " & FixedSubEventCode & ""
	Set rsActLookUp = OBJdbConnection.Execute(SQLActLookUp)	
		FixedSubActCode = rsActLookUp("ActCode")
	rsActLookUp.Close
	Set rsActLookUp = nothing	
	
	'ErrorLog("ActCode 117609: " & FixedSubActCode & "")
	
End If

'--------------------------------------------------------

'Fixed Subscription discount expiration date
'sets the date when discount is no longer applied to the fixed subscription

DIM ExpDate

If Clean(Request("FixedSubExpDate")) <> "" Then
	ExpDate = Clean(Request("FixedSubExpDate"))
	'ErrorLog("ExpDate: " & ExpDate & "")
End If

'--------------------------------------------------------

'Additional Per Ticket Surcharge - Percentage

DIM AddFeePercentage

If Clean(Request("AddFeePercentage")) <> "" Then	
	AddFeePercentage = Clean(Request("AddFeePercentage"))
	'ErrorLog("AddFeePercentage : " & AddFeePercentage  & "")
End If

'--------------------------------------------------------

'Additional Per Ticket Surcharge - Amount

DIM AddFeeAmount

If Clean(Request("AddFeeAmount")) <> "" Then	
	AddFeeAmount = Clean(Request("AddFeeAmount"))
	'ErrorLog("AddFeeAmount : " & AddFeeAmount & "")
End If
...
'--------------------------------------------------------
	...
	If AllowDiscount = "Y" Then 'It's okay to apply to this order.
		...
		Set rsActLookUp = OBJdbConnection.Execute(SQLActLookUp)	
			...
			If rsActLookUp("ActCode") = FixedSubActCode Then
...
Public Function ProcessSurcharge
	...
	'Fax Order Type there is a .25 cent service fee
	...
	'Fix Sub Event there is a 1.50 cent service fee
	If rsActLookUp("ActCode") = FixedSubActCode Then
		SubTotal = SubTotal + 1.50
	End If
			
	'Apply any additional per ticket service fees		
	DIM AddSurCharge		
			
	If AddFeePercentage <> "" Then		
		AddSurCharge = round(((NewPrice)*((AddFeePercentage)/100)),2)
		
	ElseIf AddFeeAmount <> "" Then
		AddSurCharge = AddFeeAmount
		
	End If

	'ErrorLog("ActCode: " & rsActLookUp("Act")  & " Price: " & Price & " ServiceFee-SubTotal: " & SubTotal & " AddSurCharge: " & AddSurCharge & " Discount: " & DiscountAmount & " NewPrice: " & NewPrice+SubTotal+AddSurcharge & "  ")
