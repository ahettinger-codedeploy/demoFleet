...
'LSP 07/09/15 - Update format. Added height restriction of 250px to Ad image'
...
'TTT 01/11/16 - Updated to always show barcode for printing
'SSR 01/29/16 - Updated format. Added height restriction of 200px max on map image.
'SSR 03/04/16 - Updated format. Suppress printing of 2nd address if that field is blank.
'SSR 03/23/16 - Updated format. Suppressed display of  label "Phone: " if there isn't a phone number
...
'----------------------------------------
'CUSTOM E-TICKET DEFAULTS

DIM DefaultETicketGeneralInfo
DIM Outdoor_Stage_Directions
DIM Theatre_Directions
DIM Gjerde_Center_Directions

DefaultETicketGeneralInfo = "NO REFUNDS. All ticket sales are final. DO NOT COPY THIS TICKET. MULTIPLE COPIES INVALID.<BR><BR>SEATING AFTER CURTAIN TIME IS NOT GAURANTEED. Once the performance has started, it may not be possible to seat late arriving patrons or those leaving their seats for other reasons. The CBC Arts Center accepts no responsibility for inconvenience to latecomers or people leaving their seats.  CHILDREN under seven-years-old, babes in arms, and pets are NOT ALLOWED.  We reserve the right to ask you to leave for disruptive behavior.  If you have not arrived by the time the show starts we reserve the right to RESELL YOUR SEAT, with no refund.<BR><BR>EXCHANGES, contact the CBC Arts Center at 509.542.5531.  The management reserves the right to revoke the license granted by this ticket by refunding the purchase price."
Outdoor_Stage_Directions = "From I-182 take exit 12B<BR>Turn onto 20th Ave., head North<BR>Turn left onto campus, at light (Sun Willows Blvd)<BR>Follow road around campus<BR>At stop sign turn right<BR>P-building is the three story concrete building on the right<BR>Outdoor Theatre is located outside, west of the P-building"
Theatre_Directions = "From I-182 take exit 12B<BR>Turn onto 20th Ave., head North<BR>Turn left onto campus, at light (Sun Willows Blvd)<BR>Follow road around campus<BR>At stop sign turn right<BR>P-building is the three story concrete building on the right"	
Gjerde_Center_Directions = "From I-182 take exit 12B<BR>Turn onto 20th Ave., head North<BR>Turn left onto Argent St., at  2nd light<BR>Turn left onto Saraceno Way, at light<BR>Turn right at round about and then left <BR>The parking lot is to the left<BR>The H-building is strait in front of you<BR>The Gjerde Center will be to your left as you enter"
...
If Not rsOrderNum.EOF Then
	...
	If Not rsTix.EOF Then
		...
		Response.Write "img.MapImage {max-height: 200px; width: auto; max-width:200px;}" & vbCrLf
		...
		'Begin Ticket
			...
			Response.Write "<IMG alt=""Barcode Image"" src=""https://www.tix.com/Barcode.asp?Barcode=" & rsTix("TicketNumber") & "&Rotate=270"">&nbsp;" & vbCrLf
			Response.Write "</TD>" & vbCrLf
			Response.Write "</TR>" & vbCrLf
		...
		'Begin Body Space
			...
			If rsTix("VenueAddress1") <> "" Then
				VenueAddress = rsTix("VenueAddress1") & "<BR>"
			End If

			If rsTix("VenueAddress2") <> "" Then
				VenueAddress = VenueAddress & rsTix("VenueAddress2") & "<BR>"
			End If
			
			Response.Write VenueAddress
			
			Response.Write rsTix("VenueCity") & ", " & rsTix("VenueState") & " " & rsTix("VenuePostalCode") & "<BR>" & vbCrLf
                  
                  If Len(rsTix("EventPhoneNumber")) <> 0 Then  
			...
			If Not IsNull(rsTix("ETicketDrivingDirections")) Then
				Response.Write "<B>DRIVING DIRECTIONS</B><BR>" & rsTix("ETicketDrivingDirections") & vbCrLf
			Else
				Select Case rsTix("VenueCode") 
					Case "15788"
						Response.Write "<B>DRIVING DIRECTIONS</B><BR>" & Outdoor_Stage_Directions & vbCrLf
					Case "12063"
						Response.Write "<B>DRIVING DIRECTIONS</B><BR>" & Theatre_Directions & vbCrLf
					Case "15955"
						Response.Write "<B>DRIVING DIRECTIONS</B><BR>" & Gjerde_Center_Directions & vbCrLf
					Case Else
						Directions1 = "&nbsp;"
				End Select
			End If
			...
			Response.Write "<IMG alt=""Barcode Image"" src=""https://www.tix.com/Barcode.asp?Barcode=" & rsTix("TicketNumber") & """><BR>" & vbCrLf
			Response.Write "<BR>" & vbCrLf

			'ETicket MapPath
			If Not IsNull(rsTix("ETicketMapPath")) Then
				Response.Write "<IMG SRC=""" & rsTix("ETicketMapPath") & """ class=""MapImage"">" & vbCrLf
			End If
			...
			Else
				Response.Write DefaultETicketGeneralInfo & vbCrLf
			End If
