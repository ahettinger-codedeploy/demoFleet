...
function GetHttpRequest(counter, fileID, fileName, fileType, arrFiles)
{
	...
	else if (window.ActiveXObject || (commonspot.util.browser.ie && commonspot.util.browser.v >= 11)) // IE
	{
		try
		{
...
{
	var wnd = newWindow('testWnd', '/commonspot/dhtmledit/blank.html', 'Testing new window', true, 'width=1,height=1,scrollbars=no,menubar=no,status=no,toolbar=no,top=' + window.screen.height + ',left=' + window.screen.width + ',resizable=no');
