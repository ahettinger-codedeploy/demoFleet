...
if(!window.WIDGETBOX)(function(){var initialized=false;var pageLoadCallback=function(){WIDGETBOX.setPageLoaded();};var pageUnloadCallback=function(){WIDGETBOX.setPageUnloaded();};WIDGETBOX={libs:{},globals:{token:"",tokenTime:0,widgets:[],widgetCount:0,pageLoaded:false,pageUnloaded:false,pageLoadListeners:[],pageUnloadListeners:[],panels:[],panelCount:0,showPanelMarks:true,suppressGetWidget:false,disableGetWidget:false,suppressQuantcast:false,enableLogging:false,log:"",trustedPage:false,disableInstallerMenu:false,adHelperLoaded:false,adHelperChannel:"",renderInstallerMenuInline:false},init:function(){if(!initialized){initialized=true;if(window.WIDGETBOXLOADLISTENERS){for(var i=0;i<WIDGETBOXLOADLISTENERS.length;i++){var listener=WIDGETBOXLOADLISTENERS[i];self.addPageLoadListener(listener);}}
...
var head=document.getElementsByTagName("head")[0];script=document.createElement("script");script.id="widgetbox_lib_"+libInfo.id;script.type="text/javascript";if(!useCdn){script.src="http://widgetserver.com/syndication/"+libInfo.path+".js";}else{script.src="http://cdn.widgetserver.com/syndication/"+libInfo.path+".js";}
...
{flashTag+='id="'+this.id+'" ';}
flashTag+='codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version='+this.version+'" ';if(this.width>=0){flashTag+='width="'+this.width+'" ';flashTag+='height="'+this.height+'"';}
...
{flashTag+='<embed src="'+this.src+'"';if(this.width>=0){flashTag+=' width="'+this.width+'"';flashTag+=' height="'+this.height+'"';}
...
return;}
self.addStartListener(onStartListener);if(!flashHelperInstalled){flashHelperInstalled=true;helperId="widgetbox_jsproxy_"+widgetId;var helperSwf="http://widgetserver.com/syndication/flash/v6/Helper.swf";var helperVer="6,0,65,0";if(useV8){helperSwf="http://widgetserver.com/syndication/flash/v8/Helper.swf";helperVer="8,0,0,0";}
var ft=new WIDGETBOX.flash.Utils.FlashTag(helperSwf,1,1,helperVer);ft.addFlashVar("token",token);ft.addFlashVar("widgetId",widgetId);ft.setId(helperId);ft.setAllowScriptAccess("always");ft.setAllowNetworking("all");if(!useV8){ft.setEnableFSCommand(true);}
var divName='widgetbox_helper_div_'+token;if(document.body){var target=document.getElementById(divName);if(!target)
{target=document.createElement("div");target.id=divName;if(WIDGETBOX.globals.firstParentNode){WIDGETBOX.globals.firstParentNode.parentNode.appendChild(target);}else{document.body.appendChild(target);}}
...
else{var jsProxyId="jsproxy_"+token+"_"+widgetId;var jsContainerId="widgetbox_jsproxy_"+widgetId;var flashProxy=new FlashProxy(jsProxyId,jsContainerId,'http://widgetserver.com/syndication/flash/v6/JavaScriptFlashGateway.swf',null);flashProxy.call("sendMessage",token,widgetId,msgName,msgPayload);}}},setMessageHandler:function(msgName,handler){msgHandlers[msgName]=handler;},messageListener:function(msgName,msgPayload){var handler=msgHandlers[msgName];if(handler){try{handler(msgPayload);}
catch(e){WIDGETBOX.logMessage(e);}}
...
var lastChar=url.charAt(url.length-1);if(url.indexOf("?")<0&&url.indexOf("&")<0)url+="?";else if((lastChar!="&")&&(lastChar!="?"))url+="&";url+=paramString;return url;}}
...
if(headerStr)send.headers=headerStr;send.parse=this.parseAtProxyServer;send.format=this.format;send.token=WIDGETBOX.globals.token;WIDGETBOX.net.JSONRequest.get("${helper.proxyBaseUrl}/ws_proxy.js",send,callback,0);}},setRequestHeader:function(header,value){if(this.initialized){this.requestHeaders[header]=value;}}}
...
WIDGETBOX.namespace("subscriber");if(!WIDGETBOX.subscriber.Widget)(function(){var JSONRequest=WIDGETBOX.net.JSONRequest;var netUtils=WIDGETBOX.net.Utils;function Widget(appId,regId,parentNode){if(!appId)appId="00000000-0000-0000-0000-000000000000";this.appId=appId;this.regId=regId;this.fId=this.regId;this.providerId=null;this.id=WIDGETBOX.globals.widgets.length;this.loaded=false;this.hasError=false;this.sendsMessages=false;this.subscribeMode=null;this.defer=false;this.info=null;this.eventListeners=null;this.activityImage=null;this.widgetImage=null;this.iframe=null;this.adDiv=null;this.divNode=null;this.headNodes=null;this.parentNode=parentNode;this.divId="widgetbox_widget_div_"+this.id;this.divNode=document.createElement("div");this.divNode.setAttribute("id",this.divId);this.getWidgetLoaded=false;this.adLoaded=false;this.adFriendly=false;this.adEnabled=false;this.channels=null;this.adPlacement=null;this.isPreviewInstance=false;this.inventoryInstId=false;this.inventoryRegId=false;this.appPK=null;this.regPK=null;this.providerPK=null;this.userPK=null;this.fromGalleryPK=null;this.installPartnerNetworkPK=null;this.isUseDefaultDomain=false;this.defaultDomain=null;this.detailsUrl=null;this.configUrl=null;this.configParams=null;this.partnerNetwork=null;this.fromPartnerNetwork=null;this.fromCustomGallery=null;this.suppressEditWidget=false;this.serviceLevel=0;this.counterUrl=null;this.cfgString;parentNode.appendChild(this.divNode);WIDGETBOX.globals.widgets[this.id]=this;}
...
this.subscribeMode=info.subscribeMode;this.configUrl=info.configUrl;this.adFriendly=info.isAdFriendly;this.adEnabled=info.isAdEnabled;this.channels=info.channels;this.adPlacement=info.adPlacement;this.isPreviewInstance=info.isPreviewInstance;this.inventoryInstId=info.inventoryInstId;this.inventoryRegId=info.inventoryRegId;this.providerId=info.providerId;this.appPK=info.appPK;this.regPK=info.regPK;this.providerPK=info.providerPK;this.userPK=info.userPK;this.fromGalleryPK=info.fromGalleryPK;this.installPartnerNetworkPK=info.installPartnerNetworkPK;this.isUseDefaultDomain=info.isUseDefaultDomain;this.defaultDomain=info.defaultDomain;this.serviceLevel=info.serviceLevel;this.fromCustomGallery=info.fromGalleryCode;if(info.installPartnerNetworkCode){this.partnerNetwork=info.installPartnerNetworkCode;}else{this.partnerNetwork="code";}
if(info.fromPartnerNetworkCode){this.fromPartnerNetwork=info.fromPartnerNetworkCode;}else{this.fromPartnerNetwork="wbx";}
this.detailsUrl="http://www.widgetbox.com/widget/"+this.fId
+"?wbx.refer=1"
...
if(configName.indexOf("wbx.")==0){var wbxParamName=configName.substring(4);var wbxParamValue=configParam.value;if((!info.isPreviewInstance&&wbxParamName=="counter")||(info.isPreviewInstance&&wbxParamName=="counter_preview")){var counterUrl=configParam.value;this.counterUrl=wbxParamValue=counterUrl.replace(/__WBX_REFERER__/g,WIDGETBOX.net.Utils.urlEncode(window.location.href));}
wbxParams[wbxParamName]=wbxParamValue;continue;}
urlParams[configName]=configParam.value;}
if(info.widgetType=='FLASH'&&!hasWMode&&info.isAdFriendly&&info.isAdEnabled){pluginParams["wmode"]="opaque";}
this.configParams=configParams;}
var wbxParamsValue=wbxParams["paramMode"];var hideAllParams=(wbxParamsValue&&wbxParamsValue=="suppressAll"&&info.widgetType!="HTML");var hideWidgetParams=(wbxParamsValue&&(wbxParamsValue=="suppressAll"||wbxParamsValue=="configOnly"));var hidePKParams=(!wbxParamsValue||wbxParamsValue!="all");widgetParams["appId"]=this.appId;widgetParams["regId"]=this.regId;widgetParams["friendlyId"]=this.fId;widgetParams["name"]=info.name;widgetParams["token"]=WIDGETBOX.subscriber.Main.getToken();widgetParams["id"]=""+this.id;widgetParams["location"]=window.location.href;widgetParams["timestamp"]=""+netUtils.newTimestamp();widgetParams["serviceLevel"]=""+this.serviceLevel;widgetParams["width"]=""+info.appWidth;widgetParams["height"]=""+info.appHeight;if(info.hasDynamicSize&&info.widgetType!="FLASH")widgetParams["hasDynamicSize"]=""+info.hasDynamicSize;if(info.isAdFriendly)widgetParams["isAdFriendly"]=""+info.isAdFriendly;if(info.widgetType=="HTML")widgetParams["output"]="htmlcontent";if(!hidePKParams){widgetParams["appPK"]=this.appPK;widgetParams["regPK"]=this.regPK;widgetParams["providerPK"]=this.providerPK;widgetParams["userPK"]=this.userPK;if(this.partnerNetworkPK){widgetParams["partnerNetworkPK"]=this.partnerNetworkPK;}
...
if(info.isRenderedInline||WIDGETBOX.globals.trustedPage)ft.setAllowScriptAccess("always");else ft.setAllowScriptAccess("sameDomain");ft.setId("widgetbox_widget_flash_"+this.id);ft.addParams(pluginParams);ft.addFlashVarsParams(flashVars);divNode.innerHTML=ft.toString();this.notifyEventListeners("load");if(info.hasDynamicSize)this.notifyEventListeners("resize");if(!this.hasError){WIDGETBOX.subscriber.Main.recordWidgetLoad(WIDGETBOX.globals.widgets[this.id]);}
return;}
...
var doWidgetPost=false;if(url.length>2000)doWidgetPost=true;if(doWidgetPost){if(info.widgetType=='HTML')url="http://widgetserver.com/syndication/get_widget.html";else url=info.url;}
...
window["widgetbox_widget_iframe_"+this.id].location=url;}else{var jsUrl=url.replace(/"/g,"\" + '\"' + \"");jsUrl=jsUrl.replace(/'/g,"\" + \"'\" + \"");var fdoc=window["widgetbox_widget_iframe_"+this.id].document;fdoc.open();fdoc.write("<html>");fdoc.write("<head><title>Loading...</title>");fdoc.write("<script> ");fdoc.write("function loadWidget() { ");if(formString){fdoc.write("document.widgetForm.submit();");}
else{fdoc.write("var url=\""+jsUrl+"\"; ");fdoc.write("window.location.replace(url); ");}
fdoc.write("} ");fdoc.write("</scr"+"ipt>");fdoc.write("</head>");fdoc.write("<body onload=\"loadWidget();\">");fdoc.write("<table width=\"100%\" height=\"100%\" style=\"Border: 1Px Solid Black\" >");fdoc.write("<tr>");fdoc.write("<td align=\"center\" style=\"font-family: Trebuchet, Helvetica, sans-serif; font-size: 11px; font-weight: bold;\">");fdoc.write("<img src=\"http://cdn.widgetserver.com/syndication/images/indicator.gif\" width=\"16\" height=\"16\" alt=\"Loading...\" /><br />");fdoc.write("loading...");fdoc.write("</td>");fdoc.write("</tr>");fdoc.write("</table>");if(formString)fdoc.write(formString);fdoc.write("</body>");fdoc.write("</html>");fdoc.close();}
...
if(this.headNodes){var head=document.getElementsByTagName("head").item(0);for(var i=0;i<this.headNodes.length;i++){var headNode=this.headNodes[i];if(headNode)head.removeChild(headNode);this.headNodes[i]=null;}}
this.headNodes=null;this.styles=null;WIDGETBOX.dom.Utils.removeAllChildren(this.divNode);this.loadInfo(info);},unload:function(){this.activityImage=null;this.widgetImage=null;this.markNode=null;this.markNodeContainer=null;this.divNode=null;this.eventListeners=null;this.info=null;this.loaded=false;this.styles=null;this.headNodes=null;}};WIDGETBOX.subscriber.Panel=Panel;WIDGETBOX.setReady("subscriber.Panel");})();WIDGETBOX.namespace("subscriber");if(!WIDGETBOX.subscriber.Main)(function(){var globals=WIDGETBOX.globals;var initialized=false;var helperSetupRequested=false;var helperSetup=false;var behaviourSetup=false;var widgetInfoRequestPending=false;var panelInfoRequestPending=false;var JSONRequest=WIDGETBOX.net.JSONRequest;var netUtils=WIDGETBOX.net.Utils;var Behaviour=WIDGETBOX.behaviour.Behaviour;var Helper=WIDGETBOX.flash.Helper;var ua=navigator.userAgent.toLowerCase();var bv=parseFloat(navigator.appVersion);var isOpera=(ua.indexOf('opera')>-1),isSafari=(ua.indexOf('safari')>-1),isGecko=(!isOpera&&!isSafari&&ua.indexOf('gecko')>-1),isIE=(!isOpera&&ua.indexOf('msie')>-1),isOldIE=(isIE&&(ua.indexOf('msie 6')>-1||ua.indexOf('msie 5')>-1)),isFirefox=(ua.indexOf('firefox')>-1),isOldFirefox=(isFirefox&&(ua.indexOf('firefox/2')>-1||ua.indexOf('firefox/2')>-1)),isCamino=(ua.indexOf('camino')),isMac=(ua.indexOf('mac')>-1);var onScrollFn=null;var onResizeFn=null;var isListening=false;var onCloseFn=null;var installWin=null;var lightboxedElement=null;WIDGETBOX.subscriber.Main={init:function(){if(!initialized){initialized=true;var unloader=function(){self.unloadWidgets();};WIDGETBOX.addPageUnloadListener(unloader);self.setupBehaviourForSubscriber();self.loadWidgets();}},getToken:function(){return globals.token;},setupHelperForSubscriber:function(){if(!helperSetupRequested&&!helperSetup){helperSetupRequested=true;var onHelperSetup=function(){if(!helperSetup){helperSetup=true;helperSetupRequested=false;self.loadWidgets();}};var resizeHandler=function(msgPayload){self.resizeWidget(msgPayload.widgetId,msgPayload.width,msgPayload.height);};var requestStyleHandler=function(msgPayload){self.sendStyleMesssageToWidget(msgPayload.widgetId);};Helper.startHelper(globals.token,1001,onHelperSetup);Helper.setMessageHandler("resizeWidget",resizeHandler);Helper.setMessageHandler("requestStyle",requestStyleHandler);}},isHelperSetupForSubscriber:function(){return helperSetup;},setupBehaviourForSubscriber:function(){if(!behaviourSetup){behaviourSetup=true;var tagRules={"a[rel=tag]":function(element){var tagRef=new Object();var href=element.getAttribute("href");tagRef.href=href;tagRef.keywords=unescape(href.substring(href.lastIndexOf('/')+1));tagRef.text=Behaviour.getInnerText(element);if(!WIDGETBOX.globals.taggingMarkup)WIDGETBOX.globals.taggingMarkup=new Array();WIDGETBOX.globals.taggingMarkup.push(tagRef);},"a[rel=wbx-keywords]":function(element){var keywordRef=new Object();var name=element.getAttribute("name");if(!name)name="*";keywordRef.name=name;keywordRef.keywords=element.getAttribute("title");keywordRef.text=Behaviour.getInnerText(element);if(!WIDGETBOX.globals.keywordMarkup)WIDGETBOX.globals.keywordMarkup=new Object();if(!WIDGETBOX.globals.keywordMarkup[name])WIDGETBOX.globals.keywordMarkup[name]=keywordRef;},"wbx:keywords":function(element){var keywordRef=new Object();var name=element.getAttribute("name");if(!name)name="*";keywordRef.name=name;keywordRef.keywords=element.getAttribute("value");keywordRef.text=Behaviour.getInnerText(element);if(!WIDGETBOX.globals.keywordMarkup)WIDGETBOX.globals.keywordMarkup=new Object();if(!WIDGETBOX.globals.keywordMarkup[name])WIDGETBOX.globals.keywordMarkup[name]=keywordRef;},"wbx:widget":function(element){var appId=element.getAttribute("appId");if(appId)self.insertWidget(appId,null,element);},"wbx:panel":function(element){var panelId=element.getAttribute("panelId");if(panelId)self.insertPanel(panelId,element);},"div.wbx-widget":function(element){var appId=element.getAttribute("id");if(appId)self.insertWidget(appId,null,element);},"div.wbx-panel":function(element){var panelId=element.getAttribute("id");if(panelId)self.insertPanel(panelId,element);},"span.wbx-widget":function(element){var appId=element.getAttribute("id");if(appId)self.insertWidget(appId,null,element);},"span.wbx-panel":function(element){var panelId=element.getAttribute("id");if(panelId)self.insertPanel(panelId,element);}};Behaviour.register(tagRules);WIDGETBOX.addPageLoadListener(function(){Behaviour.apply();self.loadWidgets();});}},insertWidget:function(appId,regId,parentNode,suppressGetWidget,suppressEditWidget,loadListener){if(!WIDGETBOX.globals.firstParentNode)WIDGETBOX.globals.firstParentNode=parentNode;WIDGETBOX.dom.Utils.removeAllChildren(parentNode);var widget=new WIDGETBOX.subscriber.Widget(appId,regId,parentNode);if(!suppressGetWidget&&!WIDGETBOX.globals.suppressGetWidget){var getWidgetAppender=function(wgt){if(wgt&&!wgt.getWidgetLoaded){var gw=WIDGETBOX.subscriber.Main.buildGetWidgetElement(wgt,suppressEditWidget);wgt.parentNode.appendChild(gw);wgt.getWidgetLoaded=true;}};widget.addEventListener("load",getWidgetAppender);}else if(!WIDGETBOX.globals.suppressQuantcast){WIDGETBOX.subscriber.Main.buildQuantcastElement(parentNode);}
var adUnitAppender=function(wgt){if(!wgt.info.hasDynamicSize||wgt.info.wasResized){var adsAllowed=wgt.info.isAdEnabled&&wgt.info.isAdFriendly;var hasChannels=false;if(adsAllowed){hasChannels=wgt.info.channels&&WIDGETBOX.subscriber.Main.getAdChannelsAsQueryString(wgt)!="";}
var canDisplayAds=adsAllowed&&hasChannels;if(wgt&&wgt.adLoaded){var wrapperDivId="wbx-ad-wrapper-"+wgt.id;var existingDiv=document.getElementById(wrapperDivId);if(existingDiv)existingDiv.parentNode.removeChild(existingDiv);}
if(wgt&&canDisplayAds){if(!globals.adHelperChannel||globals.adHelperChannel==""){globals.adHelperChannel="_wbx_ad_"+globals.token;}
wgt.adDiv=WIDGETBOX.subscriber.Main.buildAdUnitElement(wgt);wgt.divNode.appendChild(wgt.adDiv);wgt.divNode.style.position="relative";wgt.adLoaded=true;if(!globals.adHelperLoaded){globals.adHelperLoaded=true;var adHelperDiv=WIDGETBOX.subscriber.Main.buildAdHelperElement();wgt.parentNode.appendChild(adHelperDiv);}}}else{}};widget.addEventListener("load",adUnitAppender);widget.addEventListener("resize",adUnitAppender);if(loadListener)widget.addEventListener("load",loadListener);self.loadWidgets();return widget;},getWidget:function(widgetId){var widget=null;try{widget=globals.widgets[widgetId];}
catch(e){WIDGETBOX.logMessage(e);}
return widget;},getWidgetDiv:function(widgetId){var divNode=document.getElementById("widgetbox_widget_div_"+widgetId);return divNode;},resizeWidget:function(widgetId,w,h){var widget=self.getWidget(widgetId);if(!widget)return;widget.resize(w,h);},sendStyleMesssageToWidget:function(widgetId){var externalSheets=new Array();for(var i=0;i<document.styleSheets.length;i++){var sheet=document.styleSheets[i];if(sheet&&sheet.href)externalSheets.push(sheet.href);}
Helper.sendMessage(globals.token,widgetId,"sendStyle",{widgetId:widgetId,externalSheets:externalSheets});},unloadWidgets:function(){var widgets=globals.widgets;for(var i=0;i<widgets.length;i++){var widget=widgets[i];if(widget)widget.unload();}},loadWidgets:function(){var widgets=globals.widgets;var all_widgets_loaded=true;for(var i=0;i<widgets.length;i++){var widget=widgets[i];if(!widget.loaded&&!widget.defer){all_widgets_loaded=false;}
if(widget.defer)widget.loadInfo();}
if(all_widgets_loaded){return;}
self.startWidgetInfoRequest();},reloadWidgets:function(){self.unloadWidgets();self.loadWidgets();},startWidgetInfoRequest:function(){var d=document;var scriptId="widgetbox_getwidgetinfo_script";var getWidgetInfoScript=d.getElementById(scriptId);var head=d.getElementsByTagName("head").item(0);if(getWidgetInfoScript){try{head.removeChild(getWidgetInfoScript);}catch(e){}}
getWidgetInfoScript=d.createElement("script");var pollUrl="http://widgetserver.com/syndication/get_widget.js?token="+globals.token+"&callback=WIDGETBOX.subscriber.Main.onWidgetInfoResponse"+"&output=json"+"&location="+WIDGETBOX.net.Utils.urlEncode(window.location.href)+"&timestamp="+netUtils.newTimestamp();var widgets=globals.widgets;for(var i=0;i<widgets.length;i++){var widget=widgets[i];if(!widget.loaded){pollUrl+="&appId."+i+"="+widget.appId;if(widget.regId){pollUrl+="&regId."+i+"="+widget.regId;}}}
getWidgetInfoScript.setAttribute("src",pollUrl);getWidgetInfoScript.setAttribute("id",scriptId);widgetInfoRequestPending=true;head.appendChild(getWidgetInfoScript);},onWidgetInfoResponse:function(infoResponse){widgetInfoRequestPending=false;try{var infoSet=infoResponse.widgets;if(!globals.token){globals.tokenTime=(new Date()).getTime();globals.token=infoResponse.token;}else{infoResponse.token=globals.token;}
if(infoSet){for(var i=0;i<infoSet.length;i++){var widgetInfo=infoSet[i];var widget=self.getWidget(widgetInfo.widgetId);if(widget)widget.loadInfo(widgetInfo);}}}
catch(e){WIDGETBOX.logMessage(e);}
self.loadWidgets();},buildGetWidgetElement:function(widget,suppressEditWidget){var wrapperDiv=document.createElement("div");wrapperDiv.setAttribute("class","wbx-gw-wrapper");wrapperDiv.setAttribute("id","wbx-gw-button-wrapper-"+widget.id)
widget.suppressEditWidget=suppressEditWidget;WIDGETBOX.subscriber.Main.renderGetWidgetButton(wrapperDiv,widget,false);return wrapperDiv;},buildAdUnitElement:function(widget){var viewingWidth=widget.info.appWidth;var viewingHeight=widget.info.appHeight;var wrapperDiv=document.createElement("div");wrapperDiv.setAttribute("class","wbx-ad-wrapper");wrapperDiv.setAttribute("id","wbx-ad-wrapper-"+widget.id);wrapperDiv.style.height="1px";wrapperDiv.style.width=viewingWidth+"px";wrapperDiv.style.position="absolute";wrapperDiv.style.top="0px";wrapperDiv.style.left="0px";wrapperDiv.style.textAlign="center";wrapperDiv.style.overflow="hidden";var adFrameBaseUrl="http://cdn.widgetserver.com/syndication/promo/promo.jsp";var adFrameQS="?";adFrameQS+="widget.id="+widget.id;adFrameQS+="&channel="+WIDGETBOX.globals.adHelperChannel;adFrameQS+="&widget.regId="+widget.regId;adFrameQS+="&widget.appId="+widget.appId;adFrameQS+="&widget.width="+viewingWidth;adFrameQS+="&widget.height="+viewingHeight;adFrameQS+="&widget.location="+encodeURIComponent(window.location.href);adFrameQS+="&widget.token="+WIDGETBOX.subscriber.Main.getToken();adFrameQS+="&ua="+encodeURIComponent(ua);adFrameQS+="&toPN="+widget.partnerNetwork;adFrameQS+="&adPlacement="+widget.adPlacement;adFrameQS+="&widget.regPK="+widget.regPK;adFrameQS+="&widget.appPK="+widget.appPK;adFrameQS+="&widget.providerPK="+widget.providerPK;adFrameQS+="&widget.userPK="+widget.userPK;adFrameQS+="&fromGalleryPK="+widget.fromGalleryPK;adFrameQS+="&installPartnerNetworkPK="+widget.installPartnerNetworkPK;adFrameQS+="&udd="+widget.isUseDefaultDomain;adFrameQS+="&dd="+encodeURIComponent(widget.defaultDomain);if(widget.fromPartnerNetwork)adFrameQS+="&fromPN="+widget.fromPartnerNetwork;if(widget.fromCustomGallery)adFrameQS+="&fromCG="+widget.fromCustomGallery;adFrameQS+="&adChannels="+encodeURIComponent(WIDGETBOX.subscriber.Main.getAdChannelsAsQueryString(widget));var adFrameUrl=adFrameBaseUrl+adFrameQS;var adFrameHTML="<iframe src=\""+adFrameUrl+"\" width=\"100%\" scrolling=\"no\" height=\"1\" frameborder=\"0\" name=\"wbx-adframe-"+widget.id+"\" id=\"wbx-adframe-"+widget.id+"\" border=\"0\" allowtransparency=\"true\" style=\"position:absolute; top:0px; left:0px; visibility: visible;\"/>";wrapperDiv.innerHTML=adFrameHTML;return wrapperDiv;},getAdChannelsAsQueryString:function(wgt){var channels=wgt.channels;var channelString="";for(var channelId in channels){channelString+="&"+WIDGETBOX.net.Utils.urlEncode(channelId)+"="+WIDGETBOX.net.Utils.urlEncode(channels[channelId].channelName);}
if(channelString!=""){channelString=channelString.substr(1);}
return channelString;},buildAdHelperElement:function(){var wrapperDiv=document.createElement("div");wrapperDiv.setAttribute("class","wbx-ad-helper");wrapperDiv.setAttribute("id","wbx-ad-helper");wrapperDiv.style.position="absolute";wrapperDiv.style.top="0px";wrapperDiv.style.overflow="hidden";var adHelperUrl="http://cdn.widgetserver.com/syndication/flash/promo/adhelper.swf?channel="+WIDGETBOX.globals.adHelperChannel;var adHelperTag=new WIDGETBOX.flash.Utils.FlashTag(adHelperUrl,1,1,"8,0,0,0");adHelperTag.setId("wbx-ad-helper");adHelperTag.setAllowScriptAccess("always");adHelperTag.setWmode("transparent");wrapperDiv.innerHTML=adHelperTag.toString();return wrapperDiv;},buildQuantcastElement:function(parentNode){var qUrl='http://cdn.widgetserver.com/syndication/flash/wrapper/quantcast.swf';var ft=new WIDGETBOX.flash.Utils.FlashTag(qUrl,1,1,"8,0,0,0");ft.setAllowScriptAccess("always");ft.setWmode("transparent");var wrapperDiv=document.createElement("div");wrapperDiv.style.visibility="hidden";wrapperDiv.style.position="absolute";wrapperDiv.style.height="1px";wrapperDiv.style.width="1px";wrapperDiv.style.bottom="0px";wrapperDiv.style.right="0px";wrapperDiv.innerHTML=ft.toString();parentNode.appendChild(wrapperDiv);},renderGetWidgetButton:function(parentDiv,widget,isOwner,isReload){var ft=null;if("DISABLE_GW"!=widget.subscribeMode){var gwUrl="http://pub.widgetbox.com/flash/getwidget.swf";var buttonWidth=widget.info.hasDynamicSize?141:widget.info.appWidth;ft=new WIDGETBOX.flash.Utils.FlashTag(gwUrl,buttonWidth,23,"9,0,0,0");ft.setAllowScriptAccess("always");ft.setId("wbx-gw-button-"+widget.id);ft.setWmode("transparent");ft.addFlashVar("w",buttonWidth);if(!WIDGETBOX.globals.disableGetWidget){if("REQUIRE_CONFIG"==widget.subscribeMode){if(!widget.configUrl){ft.addFlashVar('gwUrl',widget.detailsUrl);}else{ft.addFlashVar('gwUrl',widget.configUrl);}}else{ft.addFlashVar('gwArg',new String(widget.id));}
if(!widget.suppressEditWidget&&isOwner){ft.addFlashVar('cw',"true");}
ft.addFlashVar('instanceId',widget.appId);}
ft.addFlashVar('cdnRuntimeUrl','http://cdn.widgetserver.com/syndication');if(!widget.suppressEditWidget){var configUrl="http://www.widgetbox.com/dash/subscription.jsp?id="+widget.appId;if(widget.configUrl){configUrl=widget.configUrl;configUrl+=(configUrl.indexOf('?')>0)?"&":"?";configUrl+="wbx.edit=true&wbx.id="+instId;configUrl+="&appInstance.name="+escape(widget.name);configUrl+="&appInstance.width="+widget.width;configUrl+="&appInstance.height="+widget.height;if(widget.configParams){for(var configName in widget.configParams){configUrl+="&"+configName+"="+escape(widget.configParams[configName].value);}}}
ft.addFlashVar('cwUrl',configUrl);}else{ft.addFlashVar('suppressCW','true')}
if(ft)parentDiv.innerHTML=ft.toString();}else if(isReload){WIDGETBOX.subscriber.Main.buildQuantcastElement(parentDiv);}},unloadPanels:function(){var panels=globals.panels;for(var i=0;i<panels.length;i++){var panel=panels[i];if(panel)panel.unload();}},loadPanels:function(){var panels=globals.panels;var all_panels_loaded=true;for(var i=0;i<panels.length;i++){var panel=panels[i];if(panel&&!panel.loaded&&!panel.defer){all_panels_loaded=false;}
if(panel.defer)panel.loadInfo();}
if(all_panels_loaded)return;self.startPanelInfoRequest();},startPanelInfoRequest:function(){var d=document;var scriptId="widgetbox_getpanelinfo_script";var getPanelInfoScript=d.getElementById(scriptId);var head=d.getElementsByTagName("head").item(0);if(getPanelInfoScript){head.removeChild(getPanelInfoScript);}
getPanelInfoScript=d.createElement("script");var pollUrl="http://widgetserver.com/syndication/get_panel.js?token="+globals.token+"&callback=WIDGETBOX.subscriber.Main.onPanelInfoResponse"+"&output=json"+"&location="+escape(window.location.href)+"&timestamp="+netUtils.newTimestamp();var panels=globals.panels;for(var i=0;i<panels.length;i++){var panel=panels[i];if(!panel.loaded){pollUrl+="&panelId."+i+"="+panel.panelId;}}
getPanelInfoScript.setAttribute("src",pollUrl);getPanelInfoScript.setAttribute("id",scriptId);panelInfoRequestPending=true;head.appendChild(getPanelInfoScript);},insertPanel:function(panelId,parentNode){WIDGETBOX.dom.Utils.removeAllChildren(parentNode);var panel=new WIDGETBOX.subscriber.Panel(panelId,parentNode);self.loadPanels();return panel;},getPanelDiv:function(panelId){var divNode=document.getElementById("widgetbox_panel_div_"+panelId);return divNode;},getPanel:function(panelId){var panel=null;try{panel=globals.panels[panelId];}
catch(e){WIDGETBOX.logMessage(e);}
return panel;},onPanelInfoResponse:function(infoResponse){panelInfoRequestPending=false;try{var infoSet=infoResponse.panels;globals.token=infoResponse.token;if(infoSet){for(var i=0;i<infoSet.length;i++){var panelInfo=infoSet[i];var panel=self.getPanel(panelInfo.panelIndex);if(panel){if(!panelInfo.isError){panel.loadInfo(panelInfo);}else{panel.loaded=true;}}}}}
catch(e){WIDGETBOX.logMessage(e);}
self.loadPanels();},getViewportHeight:function(){var height=-1;var mode=document.compatMode;if((mode||isIE)&&!isOpera){switch(mode){case'CSS1Compat':height=document.documentElement.clientHeight;break;default:height=document.body.clientHeight;}}else{height=(window.innerHeight)?window.innerHeight:0;}
...
if(window.scrollBy){window.scrollBy(0,1);window.scrollBy(0,-1);}},resizeMask:function(){var maskDivObj=document.getElementById("wbx-mask-div");if(maskDivObj){var docWidth=self.getDocumentWidth();var docHeight=self.getDocumentHeight();if(docWidth+"px"!=maskDivObj.style.width)maskDivObj.style.width=docWidth+"px";if(docHeight+"px"!=maskDivObj.style.height)maskDivObj.style.height=docHeight+"px";}},onScrollResizeCallback:function(){self.positionGetWidgetDiv();self.resizeMask();if(self.isListening){setTimeout(function(){WIDGETBOX.subscriber.Main.onScrollResizeCallback()},50);}},setAdUnitSizeAndLocation:function(widgetId,x,y,w,h,ww,wh){if(widgetId){var adFrame=document.getElementById("wbx-adframe-"+widgetId);var adDiv=document.getElementById("wbx-ad-wrapper-"+widgetId);if(adDiv&&adFrame){var firstWidgetElement;try{firstWidgetElement=WIDGETBOX.globals.widgets[widgetId].divNode.childNodes[0];}catch(e){WIDGETBOX.logMessage(e);}
var newWidth="0px";if(!isNaN(w)){newWidth=w+"px";}
var newHeight="0px";if(!isNaN(h)){newHeight=h+"px";}
var leftPosition=(isNaN(x))?"0":x;var topPosition=(isNaN(y))?"0":y;if(!isOldIE){var parentNode=WIDGETBOX.globals.widgets[widgetId].parentNode;var divNode=WIDGETBOX.globals.widgets[widgetId].divNode;var wasParentNodeGenerated=(parentNode.id=="widgetbox_widget_parent_"+widgetId);var isDivNodeWider=(divNode.scrollWidth>firstWidgetElement.width);var useOffset=firstWidgetElement&&(!isIE||(!wasParentNodeGenerated&&isDivNodeWider));if(useOffset){leftPosition=Number(leftPosition)+Number(firstWidgetElement.offsetLeft);topPosition=Number(topPosition)+Number(firstWidgetElement.offsetTop);}}
adDiv.style.width=newWidth;adFrame.style.width=newWidth;adDiv.style.left=leftPosition+"px";adFrame.style.height=newHeight;adDiv.style.height=newHeight;if(topPosition<0){topPosition=0;}
adDiv.style.top=topPosition+"px";}}},makeLightbox:function(hideElements){self.displayMask(true,hideElements);var divObj=document.getElementById('wbx-lb-cont-div');if(!divObj){divObj=document.createElement("div");divObj.id='wbx-lb-cont-div';divObj.style.cssFloat="left";divObj.style.styleFloat="left";divObj.style.position="absolute";divObj.style.zIndex="5002";divObj.style.border="0px";divObj.style.padding="5px";divObj.style.margin="0px";divObj.style.border="2px solid #999";divObj.style.background="#FFF";document.getElementsByTagName("body")[0].appendChild(divObj);}
lightboxedElement=divObj;return lightboxedElement;},renderWidgetInLightbox:function(appId,cfgString){var divObj=self.makeLightbox(true);var wrapper=document.createElement("div");wrapper.id="wbxLightboxedElement";divObj.appendChild(wrapper);var widget=self.insertWidget(appId,null,wrapper,true,true,self.positionLightbox);widget.cfgString=cfgString;},positionLightbox:function(widget){var remoteIframeObj=document.getElementById("wbxLightboxedElement");self.positionGetWidgetDiv(widget.info.appWidth,widget.info.appHeight);var closeX=Number(lightboxedElement.offsetLeft)+Number(widget.info.appWidth)+3;var closeY=Number(lightboxedElement.offsetTop)-13;self.renderCloseButton(lightboxedElement,"outside","right","WIDGETBOX.subscriber.Main.closeGetWidgetDiv()","Turn the lights on!");},renderCloseButton:function(parentNode,placement,location,action,title){if(!placement)placement="outside";if(!location)location="right";if(!title)title="Close"
...
formObj.submit();}else{var gwUrl="http://pub.widgetbox.com/flash/installer.swf";var menuWidth=(WIDGETBOX.globals.renderInstallerMenuInline)?281:255;var menuHeight=458;var docWidth=WIDGETBOX.subscriber.Main.getDocumentWidth();var docHeight=WIDGETBOX.subscriber.Main.getDocumentHeight();var needsScale=false;if(docHeight<menuHeight){menuHeight=docHeight;needsScale=true;}
...
divObj.style.display="";divObj.innerHTML=ft.toString();}
...
if(topPos<0){divObj.style.top="0px";}else if(topPos+"px"!=divObj.style.top){divObj.style.top=topPos+"px";}}},closeGetWidgetDiv:function(){self.isListening=false;var objToRemove=lightboxedElement;if(WIDGETBOX.globals.renderInstallerMenuInline){objToRemove.style.display="none";}else{if(objToRemove!=null){setTimeout(function(){if(objToRemove.parentNode&&objToRemove.parentNode.hasChildNodes()){WIDGETBOX.dom.Utils.removeAllChildren(objToRemove);objToRemove.parentNode.removeChild(objToRemove);}
objToRemove=null;},100);if(self.onCloseFn)self.onCloseFn();}
self.hideMask();}},openLoadingPopup:function(winName){self.installWin=window.open('http://www.widgetbox.com/getwidget/progress.jsp',winName,'toolbar=yes, resizable=yes, location=yes, status=no, width=800, height=600, scrollbars=yes');},focusLoadingPopup:function(){if(self.installWin)self.installWin.focus();},recordWidgetLoad:function(widget){var tracker=new Image(1,1);var tSrc='http://widgetserver.com/metrics/image.gif?';tSrc+='i='+widget.appId;if(globals.token)tSrc+='&t='+globals.token;tSrc+='&aik='+widget.appPK;tSrc+='&ark='+widget.regPK;tSrc+='&apk='+widget.providerPK;tSrc+='&uid='+widget.userPK;tSrc+='&pnk='+widget.installPartnerNetworkPK;if(widget.fromGalleryPK)tSrc+='&gk='+widget.fromGalleryPK;tSrc+='&ts='+new Date().getTime();tSrc+='&CXNID=2000003.0000000001NXC';tSrc+='&udd='+widget.isUseDefaultDomain;if(widget.defaultDomain){tSrc+='&dd='+widget.defaultDomain;}
...
return classElements;}};var self=WIDGETBOX.subscriber.Main;self.init();WIDGETBOX.setReady("subscriber.Main");})();if(!WIDGETBOX.subscriber.WidgetMenuConfig)(function(){function WidgetMenuConfig(widget){this.regId;this.sourceId;this.instId;this.panelId;this.context;this.toPn;this.fromPn;this.galleryUrl;this.syndUrl;this.location;this.galleryCode;this.configData;this.jsOnly;this.widgetId=0;if(widget){this.regId=widget.regId;this.sourceId=widget.appId;this.instId=null;this.panelId=null;this.context="wild";this.toPn=null;this.fromPn=widget.partnerNetwork;this.syndUrl='http://widgetserver.com/syndication';this.galleryUrl='http://www.widgetbox.com';this.location=window.location.href;this.galleryCode=null;this.configData=null;this.jsOnly=null;this.widgetId=widget.id;}}
