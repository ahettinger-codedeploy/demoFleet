...
EMailSubject = "Castrol Raceway 2014 Race Season Renewal - Only 5 days left!" 
...
Do Until rsOrderLine.EOF
	...
	If rsOrderLine("OrderNumber") <> LastOrderNumber Then 'It's different.  Print it.
		...
		If Not rsCustInfo.EOF Then
	        ...
	        SQLEventInfo = "SELECT DISTINCT CASE WHEN Seat.EventCode IN (608495,608658,608659,608660,608661) THEN 'RMN' WHEN Seat.EventCode IN (608676) THEN 'HAN' WHEN Seat.EventCode IN (608669,608671,608672,608673,608674) THEN 'WOO' END AS LetterFormat FROM OrderHeader (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderHeader.OrderNumber = Seat.OrderNumber WHERE OrderHeader.OrderNumber = " & rsOrderLine("OrderNumber")
			...
			EMailMessage = EMailMessage & "<table><tr><td><FONT FACE=Calibri,Charcoal,Arial>" & vbCrLf
			EMailMessage = EMailMessage & "If you are mailing in your Save My Seat form to Castrol Raceway, please fill out the payment information below.<BR><BR>" & vbCrLf
			...
			EMailMessage = EMailMessage & "</FONT></TD></TR></TBODY></TABLE></CENTER>" & vbCrLf
			EMailMessage = EMailMessage & "<DIV>&nbsp;</DIV>" & vbCrLf

			EMailMessage = EMailMessage & "<CENTER><HR><BR><BR>" & vbCrLf
            EMailMessage = EMailMessage & "<IMG SRC=""http://www.tix.com/PrivateLabel/CastrolRaceway/Conversion/CastrolRaceway.png""><IMG SRC=""http://www.tix.com/PrivateLabel/CastrolRaceway/Conversion/SaveMySeat.gif""><BR>" & vbCrLf
