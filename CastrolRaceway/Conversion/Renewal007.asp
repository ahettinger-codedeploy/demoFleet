<%
'CHANGE LOG
'JAI 9/25/12 -  New for Castrol 2013.  Copy of generic renewal script.
%>
...
OrderExpirationDate = "12/1/12"
RenewDescription = Left("CASTROL 2013", 15)
...
EventArray(0,0) = 406845
EventArray(0,1) = 509529

EventArray(1,0) = 406844
EventArray(1,1) = 509528

EventArray(2,0) = 406846
EventArray(2,1) = 509530

EventArray(3,0) = 406847
EventArray(3,1) = 509531

EventArray(4,0) = 406843
EventArray(4,1) = 509527

EventArray(5,0) = 406849
EventArray(5,1) = 509533

EventArray(6,0) = 406850
EventArray(6,1) = 509534

EventArray(7,0) = 406851
EventArray(7,1) = 509536

EventArray(8,0) = 406848
EventArray(8,1) = 509532

EventArray(9,0) = 477359
EventArray(9,1) = 509557
...
'Run Renewal steps
Response.Write "Begin " & RenewDescription & " - " & Now() & "<BR><BR>"
Dim RenewalMismatch
RenewalMismatch = "N"
Call RenewalCheck()
...
Response.Write "<br />End " & RenewDescription & " - " & Now() & "<BR>"
...
Do Until rsTickets.EOF
	...
	SQLSeat = "SELECT ItemNumber, Event.EventCode, EventType, Map, SectionType FROM Seat (NOLOCK) INNER JOIN Event (NOLOCK) ON Seat.EventCode = Event.EventCode INNER JOIN Section (NOLOCK) ON Seat.EventCode = Section.EventCode AND Seat.SectionCode = Section.SectionCode WHERE Event.EventCode = " & NewEventCode & " AND Seat.SectionCode = '" & rsTickets("SectionCode") & "' AND Row = '" & rsTickets("Row") & "' AND Seat = '" & rsTickets("Seat") & "' AND Seat.StatusCode IN ('A', 'H')"
	'ErrorLog( SQLSeat)
    ...
    If Not rsSeat.EOF Then
		If UCASE(rsSeat("EventType")) = "SUBFIXEDEVENT" Then  'Subscription Event
			If UCase(rsSeat("Map")) = "GENERAL" Or UCase(rsSeat("SectionType")) = "GENERAL" Then 'General Admission Subscription
		...
		Response.Write "OldEventCode = " & rsTickets("EventCode") & " | NewEventCode = " & NewEventCode & " | OldOrderNumber = " & OldOrderNumber & " | NewOrderNumber = " & OrderNumber & " | ItemNumber = " & rsSeat("ItemNumber") & " | ReturnCode = " & ReturnCode & "<BR>"
    Else
        ReturnCode = 99
    End If
	...
	If ReturnCode = 0 Then 'It was successful
        ...
        If Not rsPrice.EOF Then
            ...
            End If
        Else
            Response.Write "Price Record Not Found - NewOrderNumber = " & OrderNumber & " | EventCode = " & NewEventCode & " | NewItemNumber = " & rsSeat("ItemNumber") & " | SeatTypeCode = " & SeatTypeCode & "<BR><BR>"
            Exit Do
        End If
