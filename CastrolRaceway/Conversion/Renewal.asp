<%
'CHANGE LOG
'JAI 10/16/13 - New for Castrol 2014.  Copied from Generic script.
                'Custom for Castrol 2014.  Do not use in subsequent years.
                'Hot August Night - Hot Spot is not being renewed through this process, since we are switching to Reserved.  Need to email all Hot Spot customers with a pre-sale code to order early.
'JAI 10/24/14 - Updated for RMN 2015, based off generic.
'JAI 11/10/14 - Updated for HAN & WOO 2015.
'SLM 10/27/15 - Updated for RMN, HAN, DSP 2016
'SLM 11/21/16 - Updated for RMN 2017
...
OrderExpirationDate = "12/16/16"
RenewDescription = Left("CASTROL RMN2017", 15)
...
EventArray(0,0) = 796471  'RMN Crew
EventArray(0,1) = 918590  

EventArray(1,0) = 796473  'RMN ET
EventArray(1,1) = 918591  

EventArray(2,0) = 796474  'RMN JR
EventArray(2,1) = 918592  

EventArray(3,0) = 796475  'RMN Camp
EventArray(3,1) = 918581  

EventArray(4,0) = 795804  'RMN Fri
EventArray(4,1) = 918597  

EventArray(5,0) = 795806  'RMN Sat
EventArray(5,1) = 918598  

EventArray(6,0) = 795807  'RMN Sun
EventArray(6,1) = 918599  

EventArray(7,0) = 795808  'RMN Wknd
EventArray(7,1) = 918596  

EventArray(8,0) = 869446  'RMN JR Beginner
EventArray(8,1) = 918593  

EventArray(9,0) = 889534  'RMN GA
EventArray(9,1) = 918595  
...
Public Function LookupNewSeatTypeCode(OldEventCode, OldSectionCode, OldSeatTypeCode)

LookupNewSeatTypeCode = OldSeatTypeCode
Select Case OldSeatTypeCode
Case 54589  'Adult
	LookupNewSeatTypeCode = 54595 'Adult (13 and over)
Case 54593  'Junior 6-12
	LookupNewSeatTypeCode = 54616 'Youth 6-12
Case 54592  'Individual
    Select Case OldSectionCode
    Case "HOTS"
    Case Else
	    LookupNewSeatTypeCode = 54595 'Adult (13 and over)
    End Select
...
For i = 0 To UBound(EventArray)
    ...
    SQLCheck = "SELECT DISTINCT LY.SectionCode, LY.Section, LY.Row, LY.Seat FROM (SELECT DISTINCT Seat.EventCode, Seat.SectionCode, Section.Section, Row, Seat FROM Seat (NOLOCK) INNER JOIN Section (NOLOCK) ON Seat.EventCode = Section.EventCode AND Seat.SectionCode = Section.SectionCode WHERE Seat.EventCode = " & LYEventCode & " AND Seat.StatusCode = 'S') AS LY LEFT JOIN (SELECT DISTINCT Seat.EventCode, Seat.SectionCode, Section.Section, Row, Seat FROM Seat (NOLOCK) INNER JOIN Section (NOLOCK) ON Seat.EventCode = Section.EventCode AND Seat.SectionCode = Section.SectionCode WHERE Seat.EventCode = " & TYEventCode & ") AS TY ON LY.SectionCode = TY.SectionCode AND LY.Row = TY.Row AND LY.Seat = TY.Seat WHERE LY.EventCode = " & LYEventCode & " AND TY.EventCode = " & TYEventCode & " AND TY.SectionCode IS NULL ORDER BY LY.SectionCode, LY.Row, LY.Seat"
...
Do Until rsTickets.EOF
	...
	If ReturnCode = 0 Then 'It was successful
		...
		SeatTypeCode = LookupNewSeatTypeCode(rsTickets("EventCode"), rsTickets("SectionCode"), rsTickets("SeatTypeCode"))
