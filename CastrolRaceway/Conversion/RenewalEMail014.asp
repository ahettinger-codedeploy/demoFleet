...
If TestEMailAddress <> "" Then
	...
	TestModeStatus = "<FONT FACE=""verdana,arial,helvetica"" SIZE=""2""><B>TEST MODE</B> - " & TestEmailCount & " test emails will be sent to: " & TestEMailAddress & "<BR><BR></FONT>"
End If

HANEventList = "800555"
RMNEventList = "795804,795806,795807,795808,796471,796473,796474,796475"
DPSEventList = "796486"
...
Do Until rsOrderLine.EOF
	...
	If rsOrderLine("OrderNumber") <> LastOrderNumber Then 'It's different.  Print it.
		...
		If Not rsCustInfo.EOF Then
	        ...
	        SQLEventInfo = "SELECT DISTINCT CASE WHEN Seat.EventCode IN (" & RMNEventList & ") THEN 'RMN' WHEN Seat.EventCode IN (" & HANEventList & ") THEN 'HAN' WHEN Seat.EventCode IN (" & DPSEventList & ") THEN 'DPS' END AS LetterFormat FROM OrderHeader (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderHeader.OrderNumber = Seat.OrderNumber WHERE OrderHeader.OrderNumber = " & rsOrderLine("OrderNumber")
            ...
            Do Until rsEventInfo.EOF
				...
				QuickRenewalURL = EMailRenewalURL & "?o=" & rsOrderLine("OrderNumber") & "&c=" & RenewalCode
           ...
           ' EMailMessage = EMailMessage & "<CENTER><IMG SRC=""http://www.tix.com/PrivateLabel/CastrolRaceway/Conversion/CastrolRaceway.png"" WIDTH=""175""><IMG SRC=""http://www.tix.com/PrivateLabel/CastrolRaceway/Conversion/SaveMySeat.gif"" WIDTH=""225""><BR>" & vbCrLf
			...
			EMailMessage = EMailMessage & "<B>Renewal Code:&nbsp;" & RenewalCode & "</B><BR></TD></TR><TR><TD><FONT FACE=Calibri,Charcoal,Arial><B>To renew, <A HREF=" & QuickRenewalURL & ">click here</A>.</B><BR></TD></TR></TABLE>" & vbCrLf
