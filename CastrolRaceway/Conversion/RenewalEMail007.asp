...
RMNEventList = "796471,796473,796474,795808,795804,796475,795806,795807"
DPSEventList = "796486"
...
Do Until rsOrderLine.EOF
	...
	If rsOrderLine("OrderNumber") <> LastOrderNumber Then 'It's different.  Print it.
		...
		If Not rsCustInfo.EOF Then
	        ...
	        SQLEventInfo = "SELECT DISTINCT CASE WHEN Seat.EventCode IN (" & RMNEventList & ") THEN 'RMN' WHEN Seat.EventCode IN (" & DPSEventList & ") THEN 'DPS' END AS LetterFormat FROM OrderHeader (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderHeader.OrderNumber = Seat.OrderNumber WHERE OrderHeader.OrderNumber = " & rsOrderLine("OrderNumber")
