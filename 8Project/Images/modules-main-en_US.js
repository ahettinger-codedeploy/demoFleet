...
apsinth.msg.main_MessageDialog={close:"Close"};apsinth.msg.main_Error={requestError:"An error has occurred. Please try again later."};
apsinth.lang="en_US";var AjaxUtil=Klazz.extend({errorHandler:null,initialize:function(a){this.errorHandler=a
...
}if(a.exception){console.log(a.exception)}}}}else{if(a){if(console&&console.log){console.log(a)}}}}}if(d.length>0&&b){d+="\n"
}if(b){d+=b}}if(!apsinth.debug){d=apsinth.msg.main_Error.requestError}else{if(d.length==0){d="Error during request, see console"
}}if(top.jQuery(top.window).humanMsg){top.jQuery(top.window).humanMsg(d.replace(/\n/gm,"<br/>"))}else{console.error(d)
...
};proto.show=function(k){k=jQuery.extend({x:0,y:0,centerX:false,centerY:false,alignRight:false,winWidthMargin:50,winHeightMargin:50,width:null,height:null,addParentWidth:false,addParentHeight:false,addWinWidth:false,addWinHeight:false,effect:"slide",minWidth:0,reposition:false,zIndex:null},k);
...
}if(k.zIndex!==null){e.zIndex=k.zIndex}this._mainJQ.css(e);this.scrollIntoView();var j=this;var f=function(){j._active=true
};if(k.effect=="slide"){this._mainJQ.slideUp(0).slideDown(undefined,f)}else{if(k.effect=="fade"){this._mainJQ.fadeOut(0).fadeIn(undefined,f)
}else{this._mainJQ.show();f()}}jQuery(this).trigger("showLayer")};proto.scrollIntoView=function(){var f=this._mainJQ.offset().top;
...
this._versionJQ=null}};apsinth.util.VersionHint.init();apsinth.ApsinthModule=Modul.extend({CONFIG_MIN_WIDTH:300,proxy:null,static_proxy:null,handler_proxy:null,view_proxy:null,configSaveHandler:"config",mainView:"main",data:null,data_admin:null,configIsOpen:false,loadMainViewAfterSave:true,ajaxUtil:null,initialize:function(d,c,b,f){try{apsinth.util.EventingMixin.mixin(this);
...
})}}}},onValidationFailed:function(d){var b="";var c=d.cSSClass||"error-msg";jQuery.each(d.errors,function(f,g){jQuery.each(g,function(e,h){b+=h+"<br/>"
})});var a=jQuery(".hover_n").find("."+c);a.html(b);a.fadeIn("100")},onConfigSaveFailed:function(c){var g="";
var j=[];var a=[];if(c.errors instanceof Array){for(var d=0,f=c.errors.length;d<f;d++){var h=c.errors[d];
if(h instanceof Object&&h.field){a.push(h)}else{j.push(h)}}var b="";if(a&&a instanceof Array&&a.length>0){b="\n<br/> - User-side errors:\n<br/>";
...
jQuery("#ccloading-mask").remove();if(!a){a=this.getMainJQ()}a.removeClass("hideitbady")},showLoading:function(d){if(!d){d=this.getMainJQ()
}d.addClass("hideitbady");var c={width:d.width(),height:d.height(),offset:d.offset()};var b=jQuery('<div id="ccloading-mask"></div>');
b.css({top:(c.offset.top)+"px",left:(c.offset.left)+"px",width:c.width,height:c.height,display:"block"});
jQuery(document.body).append(b);var a=jQuery('<div id="ccloading">'+loadinggif+translation[0]+" </div>");
a.css({top:(c.offset.top+c.height/2-12)+"px",left:(c.offset.left+c.width/2-110)+"px",display:"block"});
jQuery(document.body).append(a)},validateConfig:function(){return true},getConfigData:function(){return null
},_iFrameJQ:null,_iFrame:function(d){var b=null;if(this._iFrameJQ===null){var a=this;b="ewoao"+Math.floor(Math.random()*123456);
var f=a.getConfigJQ();var e=null;if(jQuery.browser.msie){e=jQuery('<iframe src="about:blank" id="'+b+'" name="'+b+'" style="display: none;" />')
...
}},_initDefaultInputValues:function(){var b=this.getConfigJQ();var a=b.find("input[data-default-value]");
...
d.unbind("focus.defaultValues");d.unbind("blur.defaultValues")}})}});(function($){$.extend($.ui,{datepicker:{version:"1.6"}});
...
$.datepicker.version="1.6"})(jQuery);window.counter={msg:{}};var Counter=apsinth.ApsinthModule.extend({CONFIG_MIN_WIDTH:355,_showingSubSelector:false,_colorSelectionDim:{box:null,picker:null},initialize:function(c,b,a,d){this._super(c,b,a,d)
},_initMainView:function(){},_initConfigView:function(){var b=this;this._selectedColor=this.data_admin.border_color;
var c=this.getConfigJQ();var a=c.find(".skins-tab");this._skinsTabWidth=a.width();window.setTimeout(this.toHandler(function(){b._colorSelector=c.find(".apsinth-colorselector").colorPicker({mode:"button",showButtons:true,showStyledTitle:false,color:"#"+this._selectedColor,onColorSelect:function(d){b.getMainJQ().find(".ngh-counter").css("borderColor","#"+d)
},onColorAccept:this.toHandler(function(d){c.find("#"+this.idPrefix+"_border_color").val(d);this._selectedColor=d;
b.getMainJQ().find(".ngh-counter").css("borderColor","#"+d)})})}),0);this._skinSelector=new counter.SkinSelector(this,a,this.data_admin.skinMeta);
this._skinSelector.setSelectedSkin(this.data_admin.skin);c.find("#"+this.idPrefix+"_counter_value").keyup(this._onCounterValueKeyup).blur(this._onCounterValueBlur);
...
a["mod.admin"]=1;this.reloadMainView(a)}});counter.Counter=Counter;var RSSAggregator=apsinth.ApsinthModule.extend({CONFIG_MIN_WIDTH:450,CATALOG_FEED_URL:"http://api.feedzilla.com/v1/categories/%catId%/subcategories/%subId%/articles.rss",newRssTabs:null,newRssSettingsJQ:null,newRssSettingsLayer:null,feeds:new Hashmap({}),feedIdOffset:0,_initMainView:function(){var a=this;
...
a.hideLoading()}},this)},_initConfigView:function(){var b=this.getConfigJQ();b.find(".ccoption a.viewStyle").click(this.toHandler(function(d){var c=jQuery(d.currentTarget);
if(!c.hasClass("selected")){b.find(".ccoption a.viewStyle.selected").removeClass("selected");c.addClass("selected");
this.refreshFeedView()}}));b.find(".rssToggleIcon").click(this.toHandler(function(c){c.preventDefault();
jQuery(c.currentTarget).parent().next().slideToggle()}));b.find(".ccoption a.new-feed").click(this.toHandler(this.showAddNewLayer));
b.find('.ewoao_rss_user_feeds a[rel="removeUserFeed"]').click(this.toHandler(this.removeFeed));this.feeds=new Hashmap(this.data_admin.feedInfo);
var a=0;jQuery.each(this.feeds,function(c){a++});this.feedIdOffset=a;this.feeds=new Hashmap();this.feeds.setHashmapObject(this.data_admin.feedInfo)
},getConfigData:function(){var b=this.getConfigJQ();var a=b.find('select[name="num_articles"]').val();
if(parseInt(a)<1){b.find('select[name="num_articles"]').val(1);a=1}return{num_articles:a,user_feeds:this.feeds.collect(),view_mode:b.find(".ccoption a.viewStyle.selected").attr("rel")}
...
return this.addFeeds([a])}return true},addManualFeeds:function(){var a=[];var b=this.newRssSettingsJQ.find("input[name='empty_rss_text']").val();
...
j.attr("id",d.idPrefix+"_"+l.id);j.find('a[rel="removeUserFeed"]').click(d.toHandler(d.removeFeed));h.append(j);
d.feeds.set(l.id,l)})}},this)},removeFeed:function(b){var a=jQuery(jQuery(b.currentTarget).parents("li")[0]);
var c=a.attr("id").substr(this.idPrefix.length+1);a.remove();this.feeds.unset(c);b.preventDefault()},showAddNewLayer:function(){this.callRpc("private.viewNew",null,function(d,f){if(!f&&d&&d.html){var b=this;
...
this.newRssSettingsJQ.data("ownerModule",this);this.newRssSettingsLayer=new apsinth.util.Layer(this.newRssSettingsJQ,{zIndex:1000001});
...
a.html("");a.append('<option value="">'+a.attr("defaultvalue")+"</option>");jQuery.each(j.categories,function(l,m){a.append('<option value="'+l+'">'+this+"</option>")
});a.attr("disabled",false);if(b.trigger instanceof Function){b.trigger("onload",b)}}})}));var c=this.newRssSettingsJQ.find(".ewoao_rss_user_feeds li input");
...
}},this)},addFeedsAfterReload:function(a){}});var DocViewer=apsinth.ApsinthModule.extend({CONFIG_MIN_WIDTH:350,_docChanged:null,initialize:function(c,b,a,d){this._super(c,b,a,d)
...
if(b&&b.zoomResponse){a=b.zoomResponse()}else{a=100}return{viewstyle:c.find(".ngh-docviewer-config .ccoption a.active").attr("rel"),allowdownload:c.find("input[name=ngh-docviewer-allowDownload]:checked").length,allowprint:c.find("input[name=ngh-docviewer-allowPrint]:checked").length,tempDoc:this._docChanged?1:0,zoom:a}
},uploadStart:function(){var c=this.getConfigJQ();var b=c.find(".ngh-docviewer-config");var a=c.find(".ngh-docviewer-config .uploadDialog");
a.removeClass("apsinth-error").hide();b.loadingBox().show()},uploadComplete:function(c){var b=this;var e=this.getConfigJQ();
var d=e.find(".ngh-docviewer-config");var a=e.find(".ngh-docviewer-config .uploadDialog");d.loadingBox().hide();
a.text(c.data);this._docChanged=true;this.reloadMainView({zoom:0,tempDoc:1})},uploadError:function(b){var d=this.getConfigJQ();
var c=d.find(".ngh-docviewer-config");var a=d.find(".ngh-docviewer-config .uploadDialog");c.loadingBox().hide();
if(b.errors){a.text(b.errors.join(", ")).addClass("apsinth-error").addClass("message-alert").show();console.error("DocViewer::uploadError",b)
}},setHeight:function(a){var b=this.getFlashElem();if(b){jQuery(b).attr("height",a+"px")}},getFlashElem:function(){var a="docviewer_"+this.moduleId;
return document[a]||window[a]},close:function(a,b){b=(typeof b=="undefined")||(b!==false);if(b&&this._docChanged){this.callRpc("private.canceledit")
}this._docChanged=false;this._super.apply(this,arguments)}});DocViewer.onViewerClicked=function(a){mm[a].open()
};window.prodpres={msg:{}};var ProductPresentation=apsinth.ApsinthModule.extend({keywords_loaded:0,msg:null,msg_main:null,eventhandlers_registered:false,layout:null,detailVisible:false,settingsActive:false,text2ImageUrl:"",maileditorId:"maileditor-area",termseditorId:"terms",ackeditorId:"ack_text",initialize:function(c,b,a,d){this._super(c,b,a,d);
...
b.find('[name="getContent"]').attr("id","link_"+this.textareaId);b.find(".industry-template").click(this.toHandler(function(c){this.selectTemplate("industry");
c.preventDefault()}));b.find(".customer-template").click(this.toHandler(function(c){this.selectTemplate("customer");
c.preventDefault()}));b.find(".switch-layout1").click(this.toHandler(function(c){c.preventDefault()}));
b.find(".switch-layout2").click(this.toHandler(function(c){c.preventDefault()}));b.find(".switch-layout3").click(this.toHandler(function(c){c.preventDefault()
}));b.find(".ccclose").click(this.toHandler(function(c){b.find(".gb_upload").hide()}));b.find(".speichern").click(this.toHandler(function(c){b.find(".gb_upload").hide()
}));a.mainTabbar=new apsinth.util.Tabs(this.getConfigJQ());a.uploadLayer=null;b.find(".uploadflash").click(this.toHandler(function(c){a.callRpc("private.viewUploadlayer","",function(e,d){if(!d){a.uploadLayer=new apsinth.util.Layer(null,{zIndex:100000});
...
tinyMCE.execCommand("mceAddControl",false,this.ackeditorId);this.searchCriteriaHandler(d.find(".search-keys-tab"));
if(a){d.find("a[href='#"+a+"']").click()}}},this);return true},showSettingsLayer:function(a){var c=this.getConfigJQ();
var b=this.getMainJQ();this.settingsLayer=new apsinth.util.Layer(null,{zIndex:100000});this.settingsLayer.showAbove(c,true,-25,{minWidth:800,x:(jQuery(document).width()-800)/2,y:b.offset().top+50});
uploadJQ=this.settingsLayer.getContent();uploadJQ[0].innerHTML=a;cms.help.GuidedAssistant._instance._initSiteAdminButtonByClass(cms.help.GuidedAssistant.options.modulesLinkClass);
return uploadJQ},initSettingsHandlers:function(d){var a=this;var b=new apsinth.util.Tabs(d);b.bindTabChanged(function(e){a._onSettingsTabChange(e)
});if(!d.find("#payment_type-paymentpp").attr("checked")){d.find("#pp_account-label").parent().hide()
...
},this));this._shippingTaxGreyOut();d.find("input[id^='tax_calculation']").click(this.toHandler(function(){this._shippingTaxGreyOut()
...
}));this.setSettingsCloseHandler(d)},setSettingsCloseHandler:function(a){a.find(".ccclose.settings").click(this.toHandler(function(){var b=jQuery("body > .ngh-pp-settings");
...
uploadJQ.find("#shipping_cnd_free").attr("disabled","disabled")}},_shippingTaxGreyOut:function(){if(uploadJQ.find("#tax_calculation_yes").length==0){return true
}if(uploadJQ.find("#tax_calculation_yes").attr("checked")){uploadJQ.find("#shipping_taxContainerSelection").find(":checkbox").removeAttr("disabled");
uploadJQ.find("#shipping_taxContainerSelection").css("color","black");jQuery(".calculate_tax").hide()
}else{uploadJQ.find("#shipping_taxContainerSelection").find(":checkbox").attr("disabled","disabled");
uploadJQ.find("#shipping_taxContainerSelection").css("color","gray");uploadJQ.find("#tax_calculation_no").attr("checked","checked");
jQuery(".calculate_tax").show()}},_confirmDialog:function(a,b,e,d){if(typeof(this._warnDlg)==="undefined"){var c=uploadJQ.find(".settings-warning");
this._warnDlg=new apsinth.util.Layer(c,{visibleBlocker:false})}this._warnDlg.show({x:250,y:50,width:300,height:150});
...
}var f=null;if(b){f=this.toHandler(function(j){this.showImage(jQuery(j.currentTarget).attr("href"));j.preventDefault()
...
}))},selectTemplate:function(b){var a=this;var c=a.getConfigJQ();this.showLoadingCustom();this.callRpc("private.viewTemplate",{type:b},function(e,d){if(e){a.selectTemplateLayer=new apsinth.util.Layer(null,{zIndex:100000});
a.selectTemplateJQ=a.selectTemplateLayer.getContent();a.selectTemplateJQ.data("ownerModule",a);a.selectTemplateJQ.append('<div class="selectTemplateLayer"><div class="dialog-title"><div>'+a.msg[b+"-template-header"]+'</div></div><div class="popup-content">'+e.html+"</div></div>");
a.selectTemplateLayer.showAbove(c,true,-25,{minWidth:425});a.selectTemplateSaveButton=a.selectTemplateJQ.find("button.save");
a.selectTemplateSaveButton.hide();a.selectTemplateJQ.find('input[name="template_id"]').click(a.toHandler(function(f){a.selectTemplateSaveButton.show()
}));a.selectTemplateJQ.find("button.save").click(a.toHandler(function(g){var f=a.selectTemplateJQ.find('input[name="template_id"]:checked').val();
if(f){a.saveTemplate(b,f)}else{a.selectTemplateLayer.hide();a.selectTemplateJQ=null}}));a.selectTemplateJQ.find(".ccclose").click(a.toHandler(function(){a.selectTemplateLayer.hide();
...
}a.rotateInProgress=true;a.showLoading(a.getConfigJQ());a.rotateImage(jQuery(k.target).attr("imgId"),j,function(){a.hideLoading(a.getConfigJQ());
a.rotateInProgress=false});k.preventDefault()})};var f=jQuery(".rotate-l",a.view_upload);f.click(h(f,-90));
var c=jQuery(".rotate-r",a.view_upload);c.click(h(c,90));jQuery(".delete",a.view_upload).click(a.toHandler(function(i){a.deleteImage(jQuery(i.currentTarget).attr("imgId"));
...
var i=j.next();i.insertBefore(j);k.preventDefault()}));jQuery(".sortable").sortable({items:"div"})}})
...
}}a.refreshMainView()}})},editImage:function(b){var a=this;if(window.diy.cms.PictureManipulation){window.diy.cms.PictureManipulation.show({load:{type:"remote",moduleId:a.moduleId,moduleName:"product_presentation",instanceId:a.instance_id,imageId:b}})
}},viewUpdateCallback:function(){this.postUpload();this.refreshMainView()},viewSearchKeywords:function(){var c=this.getConfigJQ();
var b=c.find(".editSettings");var a=function(){this.unbind("saveSettings",a,this);this.viewSearchKeywords()
};this.callRpc("private.viewArticleKeywords",{},function(e,d){if(!d){b.html(e.html);b.find("a").click(this.toHandler(function(){this.showSettings("search-keys-tab")
}));this.bind("saveSettings",a,this)}},this)},searchCriteriaHandler:function(b){var a=this;b.find("tr").each(function(c){jQuery(".keyword_action",this).each(function(d){if(d==0){jQuery(this).addClass("keyword_action_add").click(a.toHandler(function(f){this.addSearchCriterion(f.currentTarget);
...
var c=jQuery('<div class="keyword_container"><input type="text" class="keyword_value" name="sc_values-'+d[0]+'[]" value=""/><a href="#" class="keyword_action keyword_action_remove"></a></div>');
jQuery(".keyword_action_remove",c).click(this.toHandler(function(f){this.removeKeywordChoice(f.currentTarget);
f.preventDefault()}));var a=jQuery(b).parent().parent();if(a.children().length<10){a.append(c)}},showLoadingCustom:function(){var d=this.getMainJQ();
...
jQuery(b).insertBefore('<div id="ccloading">'+loadinggif+translation[0]+" </div>");var a=(c.height/2-12)+"px 0 0 "+(c.width/2-110)+"px";
jQuery("#ccloading").css({margin:a,display:"block"})},getSession:function(){var b="ClickAndChange=";var a=document.cookie.split(";");
...
var g=jQuery(b.error_icon).offset();if(g){jQuery(".error_popup").css({left:g.left,top:g.top})}jQuery(".error_popup").show()
...
tinyMCE.get(c.maileditorId).show()};d=function(){c.showLoadingCustom();c.callRpc("private.previewMail",{emailtext:b.getText(),convertHtmlToText:true},function(i,j){var h=jQuery("iframe:[id^='med']").contents().find("p br").length;
...
jQuery(".maileditor-reset-button").click(c.toHandler(function(i){i.preventDefault();var h=jQuery("body > .ngh-pp-settings");
...
}})},preparePlaceholderText:function(a){return a.replace(/\$(([a-zA-Z_]|[^\x00-\x80])+)/g,this.createPlaceholderHtml("$1"))
...
a+="?PHPSESSID="+c}}return a},postAction:function(b){var a=false;if(b.hideShoppingCart==true){jQuery(document).trigger("diy.cms.EmotionHeader.shopHasProducts",0);
a=true}if(a&&typeof CC!="undefined"&&typeof CC.EmotionHeader!="undefined"&&typeof CC.EmotionHeader.clearViewCache=="function"){CC.EmotionHeader.clearViewCache("Shop")
}},open:function(){this._super();tinyMCE.execCommand("mceAddControl",false,this.textareaId);if(this.mainTabbar.selectedTab=="editImage"){this.postUpload()
}},close:function(){tinyMCE.execCommand("mceRemoveControl",false,this.textareaId);this._super.apply(this,arguments)
},onConfigSaved:function(b,a){this._super(b,a);if(b.status==="OK"){jQuery(document).trigger("diy.cms.EmotionHeader.shopHasProducts",{instance_id:this.instance_id,moduleId:this.moduleId})
}}});prodpres.msg.main_ProdPres={close:"Close"};window.prodsearch={msg:{}};var ProductSearch=apsinth.ApsinthModule.extend({CONFIG_MIN_WIDTH:300,msg:null,searchResultLayerVisible:false,loadMainViewAfterSave:true,initialize:function(c,b,a,d){this._super(c,b,a,d)
...
g.preventDefault()}));jQuery("#product-search-keyword-save",a.editKeywordsJQ).click(a.toHandler(function(h){var g={};
...
}catch(a){apsinth.util.ErrorUtil.onError(a)}},onDelete:function(){if(jQuery(".evtcal-edit-dlg")){jQuery(".evtcal-edit-dlg").remove()
}},getConfigData:function(){this._lastConfigData=this._configView.getDataChanges();return this._lastConfigData
},onConfigSaved:function(g,a){if(a==null){var f=g.data;if(f.events.length!=this._lastConfigData.eventChanges.length){throw new Error("Server response doesn't match to the sent changes")
...
},uploadStart:function(){var b=this;var c=jQuery(document.body).find("#event-upload-dlg");var a=c.find("#uploadDialog_"+b.moduleId);
a.html("").removeClass("apsinth-error").hide();c.loadingBox().show()},uploadComplete:function(c){var b=this;
var d=b._configView._editEventDlg.uploadJQ.find("#event-upload-dlg");var a=d.find("#uploadDialog_"+b.moduleId);
d.loadingBox().hide();b._configView._editEventDlg._uploadStatus="ok";b._configView._editEventDlg._imageExt=c.data.extension;
if(c.data.isNew!=0){b._configView._editEventDlg._eventId=c.data.isNew;b._configView._editEventDlg._eventInfo.id=c.data.isNew
}b._configView._editEventDlg.uploadLayer.hide();b._configView._editEventDlg._onSaveClicked()},uploadError:function(b){var a=this;
var c=a._configView._editEventDlg.uploadJQ.find("#event-upload-dlg");c.find("#uploadDialog_"+a.moduleId).removeClass("apsinth-uploadbar");
if(b.errors){c.find("#uploadDialog_"+a.moduleId).text(b.errors.join(", ")).addClass("apsinth-error").addClass("message-alert")
}},close:function(a,b){b=(typeof b=="undefined")||(b!==false);if(b){var d=this.getConfigData().eventChanges;
var e=[];for(var c=0;c<d.length;++c){var f=d[c];if(f.id&&f.has_image&&f.imageChanged){e.push(f.id)}}this.callRpc("private.canceledit",{events:e})
}this._super.apply(this,arguments)}});evtcal.EventCalendar=EventCalendar;var clazz=evtcal.EventCalendar;
clazz.DEFAULT_COLOR_LIGHT="#eaeaea";clazz.FIRST_DAY_IN_WEEK=(apsinth.lang=="en_US")?0:1;evtcal.util.LangUtil={removeFromArr:function(a,c){var b=this.arrIndexOf(a,c);
...
z.push("<div eventId='");z.push(C.id);z.push("' itemId='");z.push(g);z.push("' class='eventLine' style='width:");
...
};proto.updateView=function(p,g){var o=this._options;var K=this._dayWidth;var r=this._model.getColorCodesAsMap();
var t=86400000;var F=evtcal.EventCalendar.FIRST_DAY_IN_WEEK;var d=this._shownDate;var h=(7+d.getDay()-F)%7;
var f=new Date(d.getFullYear(),d.getMonth(),d.getDate()-h);var z=f.getTime();var u=new Date(f.getFullYear(),f.getMonth(),f.getDate()+6);
...
this._prevWeekBtn.text(n.format(f));this._nextWeekBtn.text(n.format(u));var y=new Date(f.getTime());var j=o.shownInDialog?this._daynameFormat:this._daynameFormatShort;
var s=this;this._daynameJQ.each(function(i,l){jQuery(l).text(j.format(y));y.setDate(y.getDate()+1)});
this._updateMetaData(z,a,g);var D=this._metaData;var C=o.eventMarginTop+o.eventPadY+o.eventMarginBottom;
var G=o.dayMarginLeft+o.eventMarginLeft+o.eventPadX+o.eventMarginRight+o.dayMarginRight;var B=[];for(var E=0,A=D.multiEvents.length;
E<A;E++){var H=D.multiEvents[E];var J=H.weekFromDay*K+o.dayMarginLeft+o.eventMarginLeft;var I=(H.weekToDay-H.weekFromDay+1)*K-G;
B.push('<div class="event" ngh-idx="');B.push(E);B.push('" style="');if(H.fromDay!=H.weekFromDay){J-=o.eventWeekOverlap;
I+=o.eventWeekOverlap+o.eventBorderLeft;B.push("border-left:none;")}if(H.toDay!=H.weekToDay){I+=o.eventWeekOverlap+o.eventBorderRight;
B.push("border-right:none;")}B.push("left:");B.push(J);B.push("px;top:");B.push(H.slot*o.multiEventHeight+o.eventMarginTop);
B.push("px;width:");B.push(I);B.push("px;background-color:");var v=r[H.event.colorId];B.push(v?v.color:this._model.getConfig().defaultcolor);
B.push('">');B.push(H.event.title);if(H.fromDay!=H.toDay&&H.event.fromTime){B.push("<br>(");B.push(this._durationFormat.format(this._getEventDate(H.event,"from")));
B.push(" - ");B.push(this._durationFormat.format(this._getEventDate(H.event,"to")));B.push(")")}B.push("</div>")
}this._multiEventCanvasJQ.html(B.join(""));this._prepareEvents(true,this._multiEventCanvasJQ.children("div.event"));
...
this._setMultiEventHeight(m+C);var e=o.eventMarginLeft+o.eventPadX+o.eventMarginRight;var c=K-o.dayMarginLeft-o.dayMarginRight;
var B=[];var b=o.slotHeight/30;for(var E=0,A=D.singleEvents.length;E<A;E++){var H=D.singleEvents[E];var q=Math.round(c*H.slot/H.slotCount);
var k=Math.round(c*(H.slot+1)/H.slotCount);B.push('<div class="event" ngh-idx="');B.push(E);B.push('" style="left:');
B.push(H.day*K+o.dayMarginLeft+q+o.eventMarginLeft);B.push("px;top:");B.push(Math.round(H.fromTime*b)+o.eventMarginTop);
B.push("px;width:");B.push(k-q-e);B.push("px;height:");B.push(Math.round((H.toTime-H.fromTime+1)*b)-C);
B.push("px;background-color:");var v=r[H.event.colorId];B.push(v?v.color:evtcal.EventCalendar.DEFAULT_COLOR_LIGHT);
B.push('">');B.push(H.event.title);B.push("<br>(");B.push(this._daytimeFormat.format(this._getEventDate(H.event,"from")));
if(H.event.toTime){B.push(" - ");B.push(this._daytimeFormat.format(this._getEventDate(H.event,"to")))
}B.push(")</div>")}this._singleEventCanvasJQ.html(B.join(""));this._prepareEvents(false,this._singleEventCanvasJQ.children("div.event"))
...
jQuery("#edcont-weeklyhoroscopes-header").css("display","block")}};var Weather=apsinth.ApsinthModule.extend({_initMainView:function(){var a=this.getMainJQ()
},initialize:function(c,b,a,d){this._super(c,b,a,d)},_initConfigView:function(){var a=this.getConfigJQ();
a.find(".ccoption a").click(this.toHandler(this._onConfigButtonClick))},getConfigData:function(){var b=this.getConfigJQ();
...
}else{e.addClass("layout-"+f)}})}});var Newsletter=apsinth.ApsinthModule.extend({CONFIG_MIN_WIDTH:355,tinyMceLoaded:false,initialize:function(c,b,a,d){this._super(c,b,a,d)
},_initMainView:function(){},_initConfigView:function(){var b=this.getConfigJQ();var a=new apsinth.util.Tabs(b);
this.registerEventHandlers(b)},registerEventHandlers:function(d){var a=this.toHandler(this._reloadPreview);
...
c.find("#combinedTax").val(d.combinedTax);c.find("#amount").val(e)}}if(d.price_total_vat){c.find("#price_total_vat").html(d.price_total_vat);
if(c.find("#localeGenuine").val()=="en_CA"||c.find("#localeGenuine").val()=="en_US"){c.find("#taxContainer_taxValue").html(d.price_total_vat);
c.find("#combinedTax").val(d.combinedTax);c.find("#amount").val(e)}}}},_formSubmit:function(){var c=this;
var f=this.getMainJQ();var d={};var b=false;jQuery("#shoppingBasketTable tbody tr").each(function(g,h){var k=jQuery(h).find("td input[id^='id']").val();
var j=jQuery(h).find("td input[id^='count']").val();if(k&&j){b=true;d[k]=j}});var e={};jQuery("#shoppingBasketForm input").each(function(h,j){var g=jQuery(j).attr("name");
if(jQuery(j).attr("type")=="checkbox"){if(jQuery(j).is(":checked")){e[g]=1}else{e[g]=false}}else{if(jQuery(j).attr("type")=="radio"){if(jQuery(j).is(":checked")){e[g]=jQuery(j).val()
}}else{e[g]=jQuery(j).val()}}});jQuery("#shoppingBasketForm select").each(function(h,j){var g=jQuery(j).attr("name");
e[g]=jQuery(j).val()});if(!b){alert(c.msg_main.no_items);return false}if(jQuery("#combinedTax").length>0){var a=jQuery("#combinedTax").val()
}else{var a=""}c.callRpc("public.checkOut",{orders:d,form:e,combinedTax:a},function(j,h){f.find(".error_icon").remove();
if(!h&&j&&!j.errors){f.find(".error-hint").hide();if(j.ack_text){jQuery(".ack-dialog .dlg-text .ack_text").html(j.ack_text)
}if(j.paypal){var i="<a href='https://www.paypal.com/us/cgi-bin/webscr?business="+encodeURIComponent(jQuery("#ppForm #business").val())+"&item_name="+encodeURIComponent(jQuery("#ppForm #business").val())+"&currency_code="+escape(jQuery("#ppForm #currency_code").val())+"&amount="+encodeURIComponent(jQuery("#ppForm #amount").val())+"&cmd="+encodeURIComponent(jQuery("#ppForm #cmd").val())+"' target='_blank'>"+c.msg_main.paypal_link+"</a>";
jQuery(".ack-dialog .dlg-text").append("<br/>"+c.msg_main.paypal_link_text+"<br /><br />"+i);f.find("#ppForm").submit()
}c._showDialog("ack");return true}else{if(j&&j.errors){var g=this.msg_main.form_errors;jQuery.each(j.errors,function(k,l){jQuery.each(l,function(n,m){if(k!=="msg"){c._showValidationError(k,m)
}else{g+="<br />"+m}})});f.find(".error-hint").html(g).show();return false}}},this);return true},_showErrorDlg:function(c){var b=this.getMainJQ();
...
}))},changeImageStatus:function(a){var c=this.getConfigJQ();var b=a.target.parentNode;jQuery("#"+b.parentNode.id).find("img").attr("class","profiseller_banner_admin_image");
a.target.className="profiseller_banner_admin_image_active";c.find("#hActiveImageId").val(a.target.id)
}});var ProfisellerTeaser=apsinth.ApsinthModule.extend({CONFIG_MIN_WIDTH:300,initialize:function(c,b,a,d){this._super(c,b,a,d)
...
var Facebook=apsinth.ApsinthModule.extend({CONFIG_MIN_WIDTH:480,initialize:function(c,b,a,d){this._super(c,b,a,d)
...
if(a.badge&&!a.badge_show_photo&&!a.badge_show_name&&!a.badge_show_fans){c.find(".x-error-msg").html(FacebookModule.msg.error.invalid_badge_options).show();
b=false}return b},getConfigData:function(){var b=this.getConfigJQ();b.find(".x-error-msg").hide().html(" ");
var a={like_box:b.find('input[name="like_box"]').attr("checked")?1:0,page_url:b.find('input[name="page_url"]').val(),show_faces:b.find('input[name="show_faces"]').attr("checked")?1:0,show_stream:b.find('input[name="show_stream"]').attr("checked")?1:0,like_button:b.find('input[name="like_button"]').attr("checked")?1:0,color:b.find('select[name="color"]').val(),badge:b.find('input[name="badge"]').attr("checked")?1:0,badge_url:b.find('input[name="badge_url"]').val(),badge_show_photo:b.find('input[name="badge_show_photo"]').attr("checked")?1:0,badge_show_name:b.find('input[name="badge_show_name"]').attr("checked")?1:0,badge_show_fans:b.find('input[name="badge_show_fans"]').attr("checked")?1:0};
...
}if(b){b.attr("data-width",a);var d=35;if(a<225){d=85}else{if(a<450){d=60}}FB.Event.subscribe("xfbml.render",function(){var e={width:a+"px",height:d+"px"};
b.find("iframe").css(e);b.css(e)})}if(this.mode=="admin"&&window.FB){FB.XFBML.parse(this.getMainJQ().get(0))
}},_initConfigView:function(){var a=this.getConfigJQ();this.disableToggleInit("like_box","likeBox");this.disableToggleInit("like_button","likeButton");
this.disableToggleInit("badge","badge")},disableToggleInit:function(a,b){var e=this.getConfigJQ();var d=e.find('input[name="'+a+'"]');
var c=e.find("."+b+" .facebook-optionset-disable input, ."+b+" .facebook-optionset-disable select");if(!d.attr("checked")){c.attr("disabled","disabled")
}d.change(function(f){if(d.attr("checked")){c.removeAttr("disabled")}else{c.attr("disabled","disabled")
...
var Twitter=apsinth.ApsinthModule.extend({CONFIG_MIN_WIDTH:470,settings:{widget:null,widget_type:null,widget_title:null,widget_search:null,widget_profile_username:null,widget_list_username:null,widget_list_id:null,follow:null,follow_username:null,follow_type:null,tweet:null},errorContainer:null,msg:null,listsRetrievedUsername:null,listsRetrievedTime:null,widget:null,initialize:function(c,b,a,d){this._super(c,b,a,d);
...
a.widget=b.find('input[name="widget"]').attr("checked")?1:0;a.widget_type=b.find('input[name="widget_type"]:checked').val();
...
this._initDefaultInputValues()},validateConfig:function(){var c=true;this.cleanErrors();try{this.readSettings();
var b=this.settings;if(b.widget){if(!b.widget_type){throw this.msg.config_error.no_widget}switch(b.widget_type){case"search":if(!b.widget_search||b.widget_search.replace(/\s/g,"")==""){throw this.msg.config_error.no_search_query
}break;case"list":if(!b.widget_list_username||b.widget_list_username.replace(/\s/g,"")==""){throw this.msg.config_error.no_list_username
}if(!b.widget_list_id||b.widget_list_id.replace(/\s/g,"")==""){throw this.msg.config_error.no_list}break;
case"profile":if(!b.widget_profile_username||b.widget_profile_username.replace(/\s/g,"")==""){throw this.msg.config_error.no_profile_username
}break}if(!b.widget_tweets||b.widget_tweets<1||b.widget_tweets>100){throw this.msg.config_error.invalid_tweet_count
}}if(b.follow){if(!b.follow_username||b.follow_username.replace(/\s/g,"")==""){throw this.msg.config_error.no_follow_username
}if(!b.follow_type||b.follow_type.replace(/\s/g,"")==""){throw this.msg.config_error.no_follow_type}}}catch(a){c=false;
this.showError(a)}return c},getConfigData:function(){var a=this.getConfigJQ();return this.settings},_initMainView:function(){var c=this.getMainJQ();
...
break}this.widget.render().start()},_initConfigView:function(){this.bind("open-config",this.toHandler(this.configOpenHandler),this)
},configOpenHandler:function(a){var c=this.getConfigJQ();this.errorContainer=c.find("div.x-error-msg");
this.triggerGetLists();c.find("input.twtr-widget-select-checkbox").change(this.toHandler(this.toggleInputs));
...
c.find("button.widget-list-find-button").click(this.toHandler(function(f){f.preventDefault();this.readSettings();
var g=this.settings.widget_list_username;var d=false;if(!this.settings.widget||this.settings.widget_type!="list"){d=this.msg.config_error.list_widget_not_selected
}else{if(!g){d=this.msg.config_error.no_list_username}}if(d){this.cleanErrors();this.showError(d)}else{this.triggerGetLists(f)
}}))},toggleInputs:function(d){var b=!(jQuery(d.target).attr("checked"));var c=jQuery(d.target).attr("data-toggle-class");
var e=this.getConfigJQ();var a=e.find("."+c);if(a){a.find("input, select, button").each(function(g,f){if(f.type&&f.type!="checkbox"){if(b){jQuery(f).attr("disabled","disabled")
}else{jQuery(f).removeAttr("disabled")}}})}},triggerGetLists:function(){var f=this.getConfigJQ();this.readSettings();
...
e.hide();if(this.settings.widget&&this.settings.widget_type=="list"&&g&&g!=f.find('input[name="widget_list_username"]').attr("data-default-value")){d.hide();
a.css("display","inline");this.getLists(g,d,a,e,b)}else{d.attr("disabled","disabled");a.hide();d.show()
}},getLists:function(h,e,b,f,c){var d=new Date();if(d.getTime()<=this.listsRetrievedTime+30000&&this.listsRetrievedUsername==h){g();
return}function g(){b.hide();f.hide();e.show()}function a(){e.hide();b.hide();f.show()}this.cleanErrors();
jQuery.jsonp({url:"http://api.twitter.com/1/"+h+"/lists.json",callbackParameter:"callback",success:this.toHandler(function(m){var o=false;
if(m&&m.lists){var j=m.lists;if(j.length>0){e.removeAttr("disabled");var k="";var n="";for(var l=0;l<j.length;
l++){if(l in j){n=j[l].slug==c?'selected="selected"':"";k+='<option value="'+j[l].slug+'" '+n+" >"+j[l].name+"</option>"
}}e.html(k);o=true;this.listsRetrievedTime=d.getTime();this.listsRetrievedUsername=h}if(!o){this.cleanErrors();
this.showError(this.msg.config_error.no_lists);k+='<option value="">'+this.msg.config_error.no_lists+"</option>";
e.html(k);e.attr("disabled","disabled");g()}else{g()}}else{this.cleanErrors();this.showError(this.msg.config_error.no_lists);
g();e.attr("disabled","disabled")}}),error:this.toHandler(function(){this.cleanErrors();this.showError(this.msg.config_error.list_jsonp_failed);
a()})})},showError:function(a){this.errorContainer.css("display","block").append(a+"<br/>")},cleanErrors:function(){this.errorContainer.css("display","none").html(" ")
},onDelete:function(){if(this.widget){this.widget.stop();this.widget.removeEvents();this.widget=null}}});
...
TwitterModule.msg.main_error={jsonp_failed:"Unable to load element. Please try again later"};var SocialBookmark=apsinth.ApsinthModule.extend({CONFIG_MIN_WIDTH:300,initialize:function(c,b,a,d){this._super(c,b,a,d)
...
}else{a.find("a.remote-social-networks-link").click(function(b){b.preventDefault()})}},replaceLink:function(c,d){var e=jQuery(d);
var a=e.attr("data-location");if(a){var f=document.getElementsByTagName("title")[0].innerHTML;var b=document.location;
a=unescape(a);f=encodeURIComponent(f);b=encodeURIComponent(b);a=a.replace("{{title}}",f);a=a.replace("{{url}}",b);
e.attr("href",a)}},_setMinWidthStyles:function(){var a=document.getElementById("modul_"+this.moduleId);
...
var Form=apsinth.ApsinthModule.extend({CONFIG_MIN_WIDTH:520,fields:new FormModule.FieldArray(),fieldGroups:{},fieldConfig:{},deletableEventsSet:false,maxEmails:3,maxDeletableFields:5,initialize:function(c,b,a,d){this._super(c,b,a,d)
...
},getSettings:function(){var c=this.getConfigJQ();var b={email:c.find("input[name=email]").val(),captcha:c.find("input[name=captcha]").is(":checked"),headline:c.find("textarea[name=headline]").val(),fields:[]};
...
}}return b},validateConfig:function(){var d=true;var e=this.getConfigJQ();var c=this.getSettings();this.clearError();
if(!c.email){this.showError(FormModule.msg.error.email_required);d=false}else{if(c.email.split(",").length>this.maxEmails){this.showError(FormModule.msg.error.max_email);
d=false}}var b=0;for(var a in this.fields){if(this.fields[a].getId&&!this.fields[a].deleted&&this.fields[a].getEnabled()&&this.fields[a].getName().length==0){b++;
this.fields[a].getHtml().find(".name-input").addClass("invalid")}}if(b){this.showError(FormModule.msg.error.empty_enabled_fields);
d=false}return d},getConfigData:function(){var b=this.getConfigJQ();var a=this.getSettings();return a
},onConfigSaved:function(b,a){this._super(b,a);if(b.fields){this.initFields(b.fields)}},clearError:function(){var a=this.getConfigJQ();
a.find(".x-error-msg").css("display","none").html(" ");a.find(".invalid").removeClass("invalid")},showError:function(a){var b=this.getConfigJQ();
var c=jQuery("<span/>");c.text(a);b.find(".x-error-msg").css("display","block").append(c).append("<br/>")
},initFields:function(a){this.setFieldData(a);this.renderFields()},setFieldData:function(a){this.fieldConfig=a
},renderFields:function(){jQuery("#form-fields-"+this.moduleId).html(" ");this.fields=new FormModule.FieldArray();
this.fieldGroups={};jQuery.each(this.fieldConfig,this.toHandler(this.renderFieldGroup));this.refreshDeletableButtons();
this.setDeletableEvents()},renderFieldGroup:function(c,a){var e;var g;switch(parseInt(c)){case FormModule.Field.typeText:e="field-text";
g=FormModule.msg.config.fields;break;case FormModule.Field.typeCheckbox:e="field-checkbox";g=FormModule.msg.config.checkboxes;
break;case FormModule.Field.typeTextarea:e="field-textarea";g=FormModule.msg.config.textareas;break}var b=jQuery('<div class="form-field-container"/>');
var f=jQuery('<strong class=""/>');b.addClass(e);b.addClass("field-type-"+c);f.text(g+":");b.append(f);
...
e.push(d.getHtml())}}if(e.length==1){d=e.pop();jQuery(d).find(".field-button-plus").show();jQuery(d).find(".field-button-minus").hide()
}else{if(e.length>1){jQuery.each(e,function(h,g){g.find(".field-button-plus").hide();g.find(".field-button-minus").show()
});if(e.length<this.maxDeletableFields){jQuery(e.pop()).find(".field-button-plus").show()}}}}this._initDefaultInputValues()
},setDeletableEvents:function(){if(!this.deletableEventsSet){this.deletableEventsSet=true;jQuery(document).unbind(".formAddRemove"+this.instance_id);
this.getConfigJQ().find(".field-button-plus").live("click.formAddRemove"+this.instance_id,this.toHandler(this.addFieldHandler));
this.getConfigJQ().find(".field-button-minus").live("click.formAddRemove"+this.instance_id,this.toHandler(this.removeFieldHandler))
...
apsinth.version='branches/release-sprint31-6970';apsinth.debug=false;
