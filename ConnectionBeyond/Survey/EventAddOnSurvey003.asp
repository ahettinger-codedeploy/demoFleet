...
Page = "Survey"
SurveyNumber = 844
SurveyFileName = "EventAddOnSurvey.asp"

'Knowledge For Living (9/2/2010)
'===============================================
'Event Add On Survey
...
VIPEventCode = "293223"
Dim VIPEventList(2)
VIPEventList(1) = "293229"
VIPEventList(2) = "297681"

CBEventCode = "292331"
Dim CBEventList(2)
CBEventList(1) = "293226"
CBEventList(2) = "293227"
...
<!-- VIP Package Events Begin -->

<%

SQLVIPCount = "SELECT Count(LineNumber) AS TotalTix FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber INNER JOIN Event (NOLOCK) ON Seat.EventCode = Event.EventCode WHERE OrderLine.OrderNumber = " & Session("OrderNumber") & " AND Event.EventCode = " & VIPEventCode & ""
Set rsVIPCount = OBJdbConnection.Execute(SQLVIPCount)
NumVIPs = rsVIPCount("TotalTix")
rsVIPCount.Close
Set rsVIPCount = nothing

If CInt(NumVIPs) => 1 Then
...
<tr bgcolor="<%=TableCategoryBGColor%>">
    <td>
        ...
        <b>Fort Mifflin Haunt Fest 2010<br />VIP Package</b></FONT>
        <BR>
        <FONT FACE="<%= FontFace %>" COLOR="#ddb3b2" SIZE="2">This package has two events with multiple performance times:</FONT><br />
        <FONT FACE="<%= FontFace %>" COLOR="#ddb3b2" SIZE="2">Please select a performance time for each event:</FONT><br />
    </td>
</tr>
<tr bgcolor="<%=TableDataBGColor%>">
    <td> 
        <FONT FACE="<%= FontFace %>" COLOR="<%=TableDataFontColor%>" SIZE="2"><B>VIP Meet and Greet with SJPR Founder Susan Bove<br />And Medium Marisa Liza Pell</B></FONT><br />       
       <br />
        <SELECT NAME="EventCode" STYLE="width: 300px">
        <OPTION VALUE="0">-- Please Make A Selection --</OPTION>
        <OPTION VALUE="293229">Saturday, October 23, 2010 at 2:00 PM</OPTION>
        <OPTION VALUE="297681">Saturday, October 23, 2010 at 2:30 PM</OPTION>                       
        </SELECT>
        <INPUT TYPE="hidden" NAME="Quantity" VALUE=<%=NumVIPs%>>
...
<tr bgcolor="<%=TableDataBGColor%>">
    <td> 
        <FONT FACE="<%= FontFace %>" COLOR="<%=TableDataFontColor%>" SIZE="2"><B>Connection Beyond Gallery<br />with Medium Marisa Liza Pell</B></FONT><br /><br />       
        <br />
        <SELECT NAME="EventCode" STYLE="width: 300px">
        <OPTION VALUE="0">-- Please Make A Selection --</OPTION>
        <OPTION VALUE="293226">Saturday, October 23, 2010 at 11:30 AM</OPTION>
        <OPTION VALUE="293227">Saturday, October 23, 2010 at 3:30 PM</OPTION>                       
        </SELECT>
        <INPUT TYPE="hidden" NAME="Quantity" VALUE=<%=NumVIPs%>>
        <INPUT TYPE="hidden" NAME="SectionCode" VALUE="GA">
        <INPUT TYPE="hidden" NAME="SeatTypeCode" VALUE="16">
...
<!-- VIP Package Events End -->
<!-- CB Package Events Begin -->

<%

SQLCBCount = "SELECT Count(LineNumber) AS TotalTix FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber INNER JOIN Event (NOLOCK) ON Seat.EventCode = Event.EventCode WHERE OrderLine.OrderNumber = " & Session("OrderNumber") & " AND Event.EventCode = " & CBEventCode & ""
Set rsCBCount = OBJdbConnection.Execute(SQLCBCount)
NumCBs = rsCBCount("TotalTix")
rsCBCount.Close
Set rsCBCount = nothing

If CInt(NumCBs) => 1 Then
...
<tr bgcolor="<%=TableCategoryBGColor%>">
    <td>
        <FONT FACE="<%= FontFace %>" COLOR="<%=TableCategoryFontColor%>" SIZE="4">
        <b>Fort Mifflin Haunt Fest 2010<br />Connection Beyond Package</b></FONT>
        <BR>
        <FONT FACE="<%= FontFace %>" COLOR="#ddb3b2" SIZE="2">This package has an event with two performance times:</FONT><br />
        <FONT FACE="<%= FontFace %>" COLOR="#ddb3b2" SIZE="2">Please select a performance time:</FONT><br />
    </td>
</tr>
<tr bgcolor="<%=TableDataBGColor%>">
    <td> 
        ...
        <INPUT TYPE="hidden" NAME="Quantity" VALUE=<%=NumCBs%>>
        <INPUT TYPE="hidden" NAME="SectionCode" VALUE="GA">
        <INPUT TYPE="hidden" NAME="SeatTypeCode" VALUE="16">
    </td>
</tr>
...
<!-- CB Package Events End -->


<!-- A La Carte Events Begin -->

<tr bgcolor="<%=TableCategoryBGColor%>">
    <td>
        <FONT FACE="<%= FontFace %>" COLOR="<%=TableCategoryFontColor%>" SIZE="4">
        <b>Optional Events</b></FONT>
        <BR>
        <FONT FACE="<%= FontFace %>" COLOR="#ddb3b2" SIZE="2">Would you like to add any of these events to your order?<br />To purchase enter ticket quantities below.
    </td>
</tr>
...
<tr bgcolor="<%=TableDataBGColor%>">
    <td align="center">
        ...
        <table width="40%" border="0" CELLPADDING="0" CELLSPACING="3">
            <tr>    
                <td ALIGN="center"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><U>Ticket Price</U></FONT></td>
                <td ALIGN="center"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><U>Ticket Quantity</U></FONT></td>
            </tr>
            <tr>
                <td ALIGN="center"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=FormatCurrency(OfferPrice,2)%></FONT></td>
                <td ALIGN="center"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><INPUT TYPE="text" NAME="Quantity" maxlength="3" size="3"></td>
            </tr>
            <tr>
                <td colspan = "2">
