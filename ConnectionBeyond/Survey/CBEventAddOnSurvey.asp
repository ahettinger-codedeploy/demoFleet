...
Page = "Survey"
SurveyNumber = 838
SurveyFileName = "CBEventAddOnSurvey.asp"


'Knowledge for Living (8/30/2010)
'====================================
'Event Add On Survey

'If a customer has a ticket to:
'EventCode: 292331 - Connection Beyond Package

'they are offered two choices:

'(1) EventCode: 293226 - Connection Beyond Gallery with Medium Marisa Liza Pell: 11:30AM Show  
'(2) EventCode: 293227  Connection Beyond Gallery with Medium Marisa Liza Pell: 3:30PM Show

'ratio of tickets in offer is 1-to-1
'offer event tickets are not discounted
'offer event tickets incurr no change in per ticket service fees
...
Sub AddOnSurvey
    ...
    SQLTicketCount = "SELECT Count(LineNumber) AS TotalTix FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber INNER JOIN Event (NOLOCK) ON Seat.EventCode = Event.EventCode WHERE OrderLine.OrderNumber = " & Session("OrderNumber") & " AND Event.EventCode = " & RequiredEvent(1) & ""
    ...
    If Not rsCountAvail.EOF Then	
    	SQLEventList = "SELECT EventCode FROM Event (NOLOCK) WHERE Event.EventCode = " & OfferEvent(i) & " "
        Set rsEventList= OBJdbConnection.Execute(SQLEventList) 
        EventOfferList = EventOfferList & rsEventList("EventCode") & ","  
        rsEventList.Close
   ...
   EventOfferList = Left( EventOfferList,((Len( EventOfferList))-1))
...
<tr>
    <td>  
        ...
        <TR>
            ...
            <%
            SQLRequiredActInfo = "SELECT Act.Act, Act.ShortAct, Act.Producer, Act.Comments as ActComments, Event.EventDate, Venue.Venue, Venue.Address_1, Venue.Address_2, Venue.City, Venue.State, Venue.Zip_Code FROM Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode INNER JOIN Venue (NOLOCK) ON Event.VenueCode = Venue.VenueCode WHERE Event.EventCode IN(" & RequiredEvent(1) & ")"
            Set rsRequiredActInfo = OBJdbConnection.Execute(SQLRequiredActInfo) 
            RequiredAct = rsRequiredActInfo("Act")
            RequiredShortAct = rsRequiredActInfo("ShortAct")
            RequiredProducer = rsRequiredActInfo("Producer")
            rsRequiredActInfo.Close   
            Set rsRequiredActInfo = Nothing 
            
            SQLActInfo = "SELECT Act.Act, Act.Comments as ActComments, Event.EventDate, Venue.Venue, Venue.Address_1, Venue.Address_2, Venue.City, Venue.State, Venue.Zip_Code FROM Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode INNER JOIN Venue (NOLOCK) ON Event.VenueCode = Venue.VenueCode WHERE Event.EventCode IN(" & EventOfferList & ")"
            Set rsActInfo = OBJdbConnection.Execute(SQLActInfo) 

                %>
                <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><B><%=RequiredProducer%></B></FONT><br />
                <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><B><%=RequiredShortAct%></B></FONT><br /><br />
                <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2">This package includes the following event:</FONT><br /><br /><br />
                <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><B><%=rsActInfo("Act")%></B></FONT><br />
                <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=FormatDateTime(rsActInfo("EventDate"),vbLongDate)%></FONT><br />
                <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=rsActInfo("Venue")%></FONT><br />
                ...
                <br />
                <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2">This event has two performance times.<BR /><B>Please choose one:</b></FONT><br />
                <br /><br />
                <% 
            rsActInfo.Close   
            ...
            If Not rsPackages.EOF Then
                ...
                %>
                <fieldset style="width:600px; border:2px solid #006699">
                <legend style="color:#000;font-size:13px;font-weight:bold">Option <%=OptionCount%></legend>
                <br />
                <table width="100%" border="0" CELLPADDING="5" CELLSPACING="0">
                <tr>
                    <td ALIGN="left"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><B><U>Event Name</U></B></FONT></td>    
                    <td ALIGN="left"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><B><U>Time</U></B></FONT></td>
                </tr>
                <tr>
                    <td ALIGN="left"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=rsPackages("Act")%></FONT></td>
