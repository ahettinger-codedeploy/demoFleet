...
Case Else
    SQLQuery = "SELECT DISTINCT EventDate, Act, Event.EventCode, SeatCount, SaleEndDate, Venue, City, State, Event.Capacity FROM Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode INNER JOIN Venue (NOLOCK) ON Event.VenueCode = Venue.VenueCode INNER JOIN OrganizationVenue (NOLOCK) ON Event.VenueCode = OrganizationVenue.VenueCode INNER JOIN OrganizationAct (NOLOCK) ON Event.ActCode = OrganizationAct.ActCode LEFT JOIN (SELECT Seat.EventCode, COUNT(ItemNumber) AS SeatCount FROM Seat (NOLOCK) INNER JOIN Event (NOLOCK) on Seat.EventCode = Event.EventCode WHERE StatusCode  = 'A' GROUP BY Seat.EventCode) TicketCount ON Event.EventCode = TicketCount.EventCode WHERE OrganizationVenue.OrganizationNumber = 1947 AND OrganizationAct.OrganizationNumber = 1947 ORDER BY Event.EventDate"
End Select
...
If Not rsEvents.EOF Then
	...
	If IncludeVenueLocation Then
	  ...
	  Response.Write "<TR BGCOLOR=" & TableCategoryBGColor & "><TD>&nbsp;&nbsp;</TD><TD ALIGN=left><FONT FACE=" & FontFace & " COLOR=" & TableCategoryFontColor & " SIZE=2><B>Date/Time</B></FONT></TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=left><FONT FACE=" & FontFace & " COLOR=" & TableCategoryFontColor & " SIZE=2><B>Event</B></FONT></TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=left><FONT FACE=" & FontFace & " COLOR=" & TableCategoryFontColor & " SIZE=2><B>Venue/Location</B></FONT></TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=left WIDTH=100><FONT FACE=" & FontFace & " COLOR=" & TableCategoryFontColor & " SIZE=2><B>Tickets</B></FONT></TD></TR>" & vbCrLf
	  Response.Write "<TR BGCOLOR=" & TableCategoryBGColor & "><TD COLSPAN=8 HEIGHT=2><IMG HEIGHT=1 SRC=""/images/clear.gif"" ALT=""""></TD></TR>" & vbCrLf
	...
	Do While Not rsEvents.EOF
		...
		'SSR 04/08/09 - Display Seat Status
		If IsNull(rsEvents("SeatCount")) Then
			EventStatus = "<FONT FACE=" & FontFace & " COLOR="& SoldOutColor & " SIZE=1><B>SOLD OUT</B></FONT>"
		ElseIf rsEvents("SaleEndDate") <= Now() Then 'Event Status is off sale
			EventStatus = "<FONT FACE=" & FontFace & " COLOR="& SoldOutColor & " SIZE=1><B>SOLD OUT</B></FONT>"
		ElseIf rsEvents("SeatCount")< AlmostSoldOut Then
			EventStatus = "<FONT FACE=" & FontFace & " COLOR=" & TableCategoryBGColor & " SIZE=1><B>Only " & rsEvents("SeatCount") & " left!<B></FONT>"
		Else
			EventStatus = "<FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & " SIZE=1><A HREF=/Event.asp?Event=" & rsEvents("EventCode") & ">Purchase Tix</A>&nbsp;</FONT>"
		End If
