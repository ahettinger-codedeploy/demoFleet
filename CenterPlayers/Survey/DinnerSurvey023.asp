...
'SSR 12/10/2012 - Updated survey. Survey will now populate survey with all events which meet the following parameters: sub-category "Dinner" / Event OnSale / Event Date is in the future

'SLM 5/16/12 - Added additional restaurant option

'SSR 3/29/2013 - Updated survey. Survey will now not only dynamically list all dinner option events, but it will also dynamically list all possible dinner options.
...
SQLEventList = "SELECT Event.EventCode, Act.Act, Act.ActCode FROM Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode Inner Join OrganizationAct (nolock) on organizationact.actcode = act.actcode WHERE OrganizationAct.OrganizationNumber = " & Session("OrganizationNumber") & " And Act.SubCategoryCode IN ( " & SubCategoryList & ") AND Event.OnSale = 1 AND Event.SaleStartDate > '" & Now() &  "' Order by Act"
Set rsEventList = OBJdbConnection.Execute(SQLEventList)

	If Not rsEventList.EOF Then 
			   
		Do Until rsEventList.EOF
		
			SQLSeatType = "SELECT Price.SeatTypeCode, SeatType.SeatType, Price.Price FROM Price (NOLOCK) INNER JOIN SeatType (NOLOCK) ON Price.SeatTypeCode = SeatType.SeatTypeCode WHERE Price.EventCode = " & rsEventList("EventCode") & " AND SectionCode = '" & OfferSectionCode & "'"
			Set rsSeatType = OBJdbConnection.Execute(SQLSeatType)
					
				%>
				
				<TR><TD ALIGN="right"><INPUT TYPE="radio" NAME="EventCode" VALUE="<%= rsEventList("EventCode") %>"></TD><TD><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%= rsEventList("Act") %></FONT></TD><TD ALIGN="right"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=rsSeatType("SeatType")%> - <%=FormatCurrency(rsSeatType("Price"), 2)%></FONT></TD></TR>
