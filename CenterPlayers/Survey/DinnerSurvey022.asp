...
SQLEventList = "SELECT Event.EventCode, Act.Act, Act.ActCode FROM Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode Inner Join OrganizationAct (nolock) on organizationact.actcode = act.actcode WHERE OrganizationAct.OrganizationNumber = 395 and Act.SubCategoryCode = 37 and Event.OnSale = 1 and Now() > Event.Eventdate order by eventcode"
Set rsEventList = OBJdbConnection.Execute(SQLEventList)
	...
	If Not rsEventList.EOF Then 
		...
		Select Case rsEventList("ActCode")
			Case 3318
			ThisPrice = "$30.00"
			Case 9393
			ThisPrice = "$35.00"
		...
		<TR><TD ALIGN="right"><INPUT TYPE="radio" NAME="EventCode" VALUE="<%= rsEventList("EventCode") %>"></TD><TD><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%= rsEventList("Act") %></FONT></TD><TD ALIGN="right"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=ThisPrice%></FONT></TD></TR>
