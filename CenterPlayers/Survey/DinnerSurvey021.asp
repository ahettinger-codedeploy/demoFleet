...
SQLEventList = "SELECT Event.EventCode, Act.Act FROM Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode Inner Join OrganizationAct (nolock) on organizationact.actcode = act.actcode WHERE OrganizationAct.OrganizationNumber = 395 and act.SubCategoryCode = 37 and OnSale = 1 and eventdate > '12/03/2012' order by eventcode"
Set rsEventList = OBJdbConnection.Execute(SQLEventList)
	...
	If Not rsEventList.EOF Then 
		...
		<TR><TD ALIGN="right"><INPUT TYPE="radio" NAME="EventCode" VALUE="<%= rsEventList("EventCode") %>"></TD><TD><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%= rsEventList("Act") %></FONT></TD><TD ALIGN="right"><FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2">$30.00</FONT></TD></TR>
