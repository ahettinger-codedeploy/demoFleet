...
SQLQuery = "SELECT DATENAME ( mm,EventDate ) AS MonthName, MONTH(Event.EventDate) as MonthNumber, Act.ActCode, Act.Act,  Event.EventCode FROM Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode INNER JOIN OrganizationAct (NOLOCK) ON Event.ActCode = OrganizationAct.ActCode INNER JOIN OrganizationVenue (NOLOCK) ON Event.VenueCode = OrganizationVenue.VenueCode " & SQLWhere & " AND Event.OnSale = 1 AND Event.SaleStartDate <= '" & Now() &  "' ORDER BY MONTH(Event.EventDate),Act.Act"
    ...
    Do While Not rsEvents.EOF
		...
		If  LastActCode <> rsEvents("ActCode") Then
		    ...
		    IF LastMonth <> rsEvents("MonthNumber") Then
		        Response.Write "<TR ><TD ALIGN=CENTER COLSPAN=7><BR><H3>" & rsEvents("MonthName") & "</H3></TD></TR>"
