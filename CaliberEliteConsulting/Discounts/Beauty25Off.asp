...
If AllowedSectionOK = "Y" Then 'It's okay to proceed.
		...
		If Now() > StartDate And Now() < ExpirationDate Then
			...
			'Discount for GA Tickets
			If SectionCode = "GENAD" Then
								NewPrice = 60
			End If
			
			
	'Discount for VIP Tickets
	If SectionCode = "VIP" Then

			SQLEventCount = "SELECT COUNT(Seat.EventCode) AS SeatCount FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber WHERE OrderLine.OrderNumber = " & Clean(Request("OrderNumber")) & " AND Orderline.SeatTypeCode = 155 AND Seat.EventCode = " & Clean(Request("EventCode"))
		...
		If Not rsEventCount.EOF Then
				'If rsEventCount("SeatCount") >= Quantity Then 'Apply the discount to this order
					...
					SQLDiscCount = "SELECT COUNT(OrderLine.LineNumber) AS LineCount FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber AND OrderLine.ItemType = 'Seat' INNER JOIN Event(NOLOCK) ON Seat.EventCode = Event.EventCode WHERE OrderLine.OrderNumber = " & OrderNumber & " AND Orderline.SeatTypeCode = 155 AND Event.EventCode = " & EventCode & " AND OrderLine.DiscountTypeNumber = " & DiscountTypeNumber 
					Set rsDiscCount = OBJdbConnection.Execute(SQLDiscCount)

					If Int(rsDiscCount("LineCount") / Quantity) < Int(rsEventCount("SeatCount") / Quantity) Then 'Apply the discount to this item
					  '25% off $70 Couple Ticket
						NewPrice = 52.50				
					Else			
						'25% Off $100 Single Ticket
						NewPrice = 75.00
					End If
