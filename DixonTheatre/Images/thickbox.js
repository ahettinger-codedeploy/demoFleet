...
var tb_pathToImage = "/wp-content/plugins/wpng-calendar/images/loadingAnimation.gif";
...
function tb_show_inner(caption, html, tb_params) {//function called when user programmatically needs to reference innerHTML
	...
	try {
		...
		$j('#TB_load').show();//show loader
			...
			var params = tb_parseQuery( tb_params );
			...
			$j("#TB_ajaxContent").html(html); // Set the html of the content
			tb_position();
			$j("#TB_load").remove();
			tb_init("#TB_ajaxContent a.thickbox");
			//$("#TB_window").css({display:"block"});
