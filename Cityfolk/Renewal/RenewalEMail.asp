...
EMailOrganization = "Cityfolk"
EMailSubject = "Cityfolk 2011/2012 Season Renewal" 
EMailRenewalURL = "http://Cityfolk.tix.com/renew.asp"
EMailReplyTo = "cityfolk@cityfolk.org"
EMailPhone = "(937) 496-3863"
...
If EventList <> "" Then
	...
	If Not rsTickets.EOF Then 'There are some events.  List them
		...
		Response.Write "<FONT FACE=""verdana,arial,helvetica"" COLOR=""#000000"" SIZE=""2"">Select Events for renewal e-mail generation and click 'Send E-Mails' below.</FONT><BR><BR>" & vbCrLf
...
Do Until rsOrderLine.EOF
	...
	If rsOrderLine("OrderNumber") <> LastOrderNumber Then 'It's different.  Print it.
		If TicketCount > 0 Then 'It's not the first one, print the footer.
			...
			EMailMessage = EMailMessage & "<TR><TD WIDTH=""20"">&nbsp;</TD><TD COLSPAN=""" & NumColumns - 1 & """>&nbsp;</TD><TD><HR></TD></TR>" & vbCrLf			
			If LastShipFee <> 0 And LastOrderTypeNumber <> 5 Then
				EMailMessage = EMailMessage & "<TR><TD WIDTH=""20"">&nbsp;</TD><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=Calibri,Charcoal,Arial>Processing Fee</TD><TD ALIGN=""right""><FONT FACE=Calibri,Charcoal,Arial>" & FormatCurrency(LastShipFee,2) & "</TD></TR>" & vbCrLf
				EMailMessage = EMailMessage & "<TR><TD WIDTH=""20"">&nbsp;</TD><TD COLSPAN=""" & NumColumns - 1 & """>&nbsp;</TD><TD><HR></TD></TR>" & vbCrLf
			End If
			...
			EMailMessage = EMailMessage & "<TR><TD WIDTH=""20"">&nbsp;</TD><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=Calibri,Charcoal,Arial>Order Total</TD><TD ALIGN=""right""><FONT FACE=Calibri,Charcoal,Arial>" & FormatCurrency(LastTotal,2) & "</TD></TR>" & vbCrLf
			If LastTender <> 0 Then
				EMailMessage = EMailMessage & "<TR><TD WIDTH=""20"">&nbsp;</TD><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=Calibri,Charcoal,Arial>Less Payments</TD><TD ALIGN=""right""><FONT FACE=Calibri,Charcoal,Arial>" & FormatCurrency(LastTender,2) & "</TD></TR>" & vbCrLf
				EMailMessage = EMailMessage & "<TR><TD WIDTH=""20"">&nbsp;</TD><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=Calibri,Charcoal,Arial>Balance Due</TD><TD ALIGN=""right""><FONT FACE=Calibri,Charcoal,Arial>" & FormatCurrency(LastTotal - LastTender,2) & "</TD></TR>" & vbCrLf
			End If
		...
		If Not rsCustInfo.EOF Then
			...
			EMailMessage = EMailMessage & "<CENTER><HR>" & vbCrLf
			EMailMessage = EMailMessage & "<IMG SRC=""http://www.tix.com/PrivateLabel/CityFolk/Renewal/Images/New CF logo no tagline bw.jpg"" WIDTH=500 HEIGHT=160><BR>" & vbCrLf
			EMailMessage = EMailMessage & "<TABLE CELLPADDING=""3"" WIDTH=""750""><TR><TD><FONT FACE=Calibri,Charcoal,Arial><BR><B>Order Number:&nbsp;" & rsOrderLine("OrderNumber") & "</B><BR>" & vbCrLf
	...
	If LastEventCode <> rsOrderLine("EventCode") Or LastOrderNumber <> rsOrderLine("OrderNumber") Then
		If TicketCount > 0 Then 'It's not the first one, close the table.
			EMailMessage = EMailMessage & "<BR></TD></TR></TABLE>" & vbCrLf
		End If
		
		EMailMessage = EMailMessage & "<TABLE WIDTH=""750""><TR><TD><B><FONT FACE=Calibri,Charcoal,Arial>" & rsOrderLine("Act") & " at " & rsOrderLine("Venue")
		...
		ColumnHeading = ColumnHeading + "<TD WIDTH=125 ALIGN=right><FONT FACE=Calibri,Charcoal,Arial><B><U>Amount</U></B></TD></TR>" & vbCrLf
