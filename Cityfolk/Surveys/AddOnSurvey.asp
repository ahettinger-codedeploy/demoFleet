...
<BR>
<BR>
<FONT FACE="<%= FontFace %>" COLOR="<%= HeadingFontColor %>" SIZE="2"><H3>Would you like to purchase a ticket to the Wrap Up Party Benefit?</H3></FONT>
...
<%
    ...
    SQLEventCode = "SELECT Count(OrderLine.LineNumber) AS TotalTix FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber WHERE OrderLine.OrderNumber = " & Session("OrderNumber") & " AND Seat.EventCode IN(188612,188613,188614)"
    Set rsEventCode = OBJdbConnection.Execute(SQLEventCode)
    If Not rsEventCode.EOF Then
        NumTickets = rsEventCode("TotalTix")
        EventCodeList = "198164"
    Else
        Call Continue
    End If
    rsEventCode.Close
...
%>
    ...
    <FORM ACTION="<%= SurveyFileName %>" METHOD="post" NAME="Survey">
        ...
        Quantity: <INPUT TYPE="text" NAME="Quantity" VALUE="<%=NumTickets%>" maxlength="3" size="3" /><br />        
