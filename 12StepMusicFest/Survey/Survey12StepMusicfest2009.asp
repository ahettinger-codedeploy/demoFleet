...
SQLVIPCampCount = "SELECT COUNT(ItemNumber) AS VIPCampCount FROM OrderLine (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber")	& " AND SeatTypeCode IN (2434, 3230)"
Set rsVIPCampCount = OBJdbConnection.Execute(SQLVIPCampCount)

'Count number of 4-Day Passes
SQLVIPPassCount = "SELECT COUNT(ItemNumber) AS VIPPassCount FROM OrderLine (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber")	& " AND SeatTypeCode = 2756"
Set rsVIPPassCount = OBJdbConnection.Execute(SQLVIPPassCount)
	...
	If rsVIPCampCount("VIPCampCount") > rsVIPPassCount("VIPPassCount") Then 'Need to purchase more VIP Pass Tickets.  Go to warning page
		...
		Message = "You are purchasing a 3 Night RV/Camping site.  You MUST purchase a 4-Day ticket."
...
SQLCampCount = "SELECT COUNT(ItemNumber) AS CampCount FROM OrderLine (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber")	& " AND SeatTypeCode IN (2435, 3228)"
Set rsCampCount = OBJdbConnection.Execute(SQLCampCount)

'Count number of 3-Day Passes
SQLPassCount = "SELECT COUNT(ItemNumber) AS PassCount FROM OrderLine (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber")	& " AND SeatTypeCode = 1736"
Set rsPassCount = OBJdbConnection.Execute(SQLPassCount)
	...
	If rsCampCount("CampCount") > rsPassCount("PassCount") Then 'Need to purchase more 3-Day passes.  Go to warning page
		...
		Message = "You are purchasing a 2 Night RV/Camping site.  You MUST purchase a 3-Day ticket"
