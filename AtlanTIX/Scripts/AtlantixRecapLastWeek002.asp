...
SQLEvents = "SELECT Event.Comments AS Producer, Act.Act AS Production, SUM(OrderLine.Price) AS FaceValue, SUM(OrderLine.Discount) AS Discount, COUNT(OrderLine.ItemNumber) AS TicketCount FROM Act INNER JOIN Event ON Act.ActCode = Event.ActCode INNER JOIN OrganizationVenue ON Event.VenueCode = OrganizationVenue.VenueCode INNER JOIN OrganizationAct ON Event.ActCode = OrganizationAct.ActCode INNER JOIN Seat ON Event.EventCode = Seat.EventCode INNER JOIN OrderLine ON Seat.ItemNumber = OrderLine.ItemNumber WHERE EventDate >= '" & StartDate & "' AND EventDate < '" & EndDate + 1 & "' AND OrganizationVenue.OrganizationNumber = 10 AND OrganizationVenue.Owner = 1 AND OrganizationAct.OrganizationNumber = 10 AND OrderLine.StatusCode = 'S' AND OrderLine.ItemType IN ('Seat', 'SubFixedEvent', 'SubSeat') GROUP BY Event.Comments, Act.Act ORDER BY Event.Comments, Act.Act"
