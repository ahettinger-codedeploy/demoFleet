...
<HTML>
<HEAD>
<TITLE>Tix - AtlanTIX Statement</TITLE>
...
Response.Write "<INPUT TYPE=""radio"" NAME=""DateRangeType"" VALUE=""Event"" CHECKED>&nbsp;Event Date Range&nbsp;&nbsp;&nbsp;<INPUT TYPE=""radio"" NAME=""DateRangeType"" VALUE=""Transaction"">&nbsp;Transaction Date Range<br /><br />" & vbCrLf
...
'Response.Write "<OPTION VALUE=" & Year(Date) + 1 & ">" & Year(Date) + 1 & "</OPTION>" & vbCrLf
For x = 2005 to Year(Date) + 1
    YearSelected = "" 
    If x = Year(Date) Then
        YearSelected = " SELECTED"
    End If
    Response.Write "<OPTION" & YearSelected & " VALUE=" & x & ">" & x & "</OPTION>" & vbCrLf    
Next
...
If Clean(Request("DateRangeType")) = "Transaction" Then
    SQLProducer = "SELECT Event.Comments AS Producer FROM Event (NOLOCK) INNER JOIN Seat (NOLOCK) ON Event.EventCode = Seat.EventCode INNER JOIN OrderLine (NOLOCK) ON Seat.ItemNumber = OrderLine.ItemNumber INNER JOIN OrganizationVenue (NOLOCK) ON Event.VenueCode = OrganizationVenue.VenueCode INNER JOIN OrganizationAct (NOLOCK) ON Event.ActCode = OrganizationAct.ActCode WHERE OrganizationVenue.OrganizationNumber = " & Session("OrganizationNumber") & " AND OrganizationAct.OrganizationNumber = " & Session("OrganizationNumber") & " AND OrderLine.StatusCode = 'S' AND OrderLine.StatusDate >= '" & ReportStartDate & "' AND OrderLine.StatusDate < '" & ReportEndDate + 1 & "' AND OrderLine.ItemType IN ('Seat', 'SubSeat', 'SubFixedEvent') GROUP BY Event.Comments ORDER BY Event.Comments"
Else
...
Do Until rsProducer.EOF
	...
	Response.Write "<TABLE CELLPADDING=""3""><TR BGCOLOR=""#008400""><TD ALIGN=""center""><FONT FACE='verdana,arial,helvetica' COLOR=""#FFFFFF"" SIZE=2><B>Show Title</B></FONT></TD><TD ALIGN=""center""><FONT FACE='verdana,arial,helvetica' COLOR=""#FFFFFF"" SIZE=2><B>Show Date/Time</B></FONT></TD><TD ALIGN=center><FONT FACE='verdana,arial,helvetica' COLOR=""#FFFFFF"" SIZE=2><B>" & FullHalf & " Price</B></FONT></TD><TD ALIGN=center><FONT FACE='verdana,arial,helvetica' COLOR=""#FFFFFF"" SIZE=2><B>Quantity</B></FONT></TD><TD ALIGN=center><FONT FACE='verdana,arial,helvetica' COLOR=""#FFFFFF"" SIZE=2><B>Sub Total</B></FONT></TD><TD ALIGN=center><FONT FACE='verdana,arial,helvetica' COLOR=""#FFFFFF"" SIZE=2><B>Discount</B></FONT></TD><TD ALIGN=center><FONT FACE='verdana,arial,helvetica' COLOR=""#FFFFFF"" SIZE=2><B>Total</B></FONT></TD></TR>" & vbCrLf

    'REE 3/1/9 - Added Transaction Date option
    If Clean(Request("DateRangeType")) = "Transaction" Then
	    SQLShows = "SELECT Act, EventDate, Price, COUNT(OrderLine.ItemNumber) AS TicketCount, SUM(Price) AS TicketTotal, SUM(Discount) AS DiscountTotal FROM Act (NOLOCK) INNER JOIN Event (NOLOCK) ON Act.ActCode = Event.ActCode INNER JOIN Seat (NOLOCK) ON Event.EventCode = Seat.EventCode INNER JOIN OrderLine (NOLOCK) ON Seat.ItemNumber = OrderLine.ItemNumber INNER JOIN OrganizationAct (NOLOCK) ON Event.ActCode = OrganizationAct.ActCode INNER JOIN OrganizationVenue (NOLOCK) ON Event.VenueCode = OrganizationVenue.VenueCode WHERE Event.Comments = '" & CleanSingleQuote(rsProducer("Producer")) & "' AND OrderLine.StatusCode = 'S' AND OrderLine.StatusDate >= '" & ReportStartDate & "' AND OrderLine.StatusDate < '" & ReportEndDate + 1 & "' AND OrganizationVenue.OrganizationNumber = " & Session("OrganizationNumber") & " AND OrganizationAct.OrganizationNumber = " & Session("OrganizationNumber") & " AND OrderLine.ItemType IN ('Seat', 'SubFixedEvent', 'SubSeat') GROUP BY Act, EventDate, Price ORDER BY Act, EventDate, Price"
    Else
	...
	Do Until rsShows.EOF
		...
		Response.Write "<TR BGCOLOR=""#DDDDDD""><TD><FONT FACE=""verdana,arial,helvetica"" SIZE=""2"">" & rsShows("Act") & "</FONT></TD><TD><FONT FACE=""verdana,arial,helvetica"" SIZE=""2"">" & DateAndTimeFormat(rsShows("EventDate")) & "</FONT></TD><TD ALIGN=right><FONT FACE='verdana,arial,helvetica' SIZE=2>" & FormatNumber(rsShows("Price"),2) & "</FONT></TD><TD ALIGN=right><FONT FACE='verdana,arial,helvetica' SIZE=2>" & rsShows("TicketCount") & "</FONT></TD><TD ALIGN=right><FONT FACE='verdana,arial,helvetica' SIZE=2>" & FormatNumber(rsShows("TicketTotal"), 2) & "</FONT></TD><TD ALIGN=right><FONT FACE='verdana,arial,helvetica' SIZE=2>" & FormatNumber(rsShows("DiscountTotal"), 2) & "</FONT></TD><TD ALIGN=right><FONT FACE='verdana,arial,helvetica' SIZE=2>" & FormatNumber(rsShows("TicketTotal") - rsShows("DiscountTotal"), 2) & "</FONT></TD></TR>" & vbCrLf
