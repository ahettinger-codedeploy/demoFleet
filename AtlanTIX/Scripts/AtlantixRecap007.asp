...
<HTML>
<HEAD>
<TITLE>www.TIX.com - AtlanTIX Recap</TITLE>
...
'Calculate Tix Fees
SQLTixFees = "SELECT OrderHeader.OrderNumber, OrderHeader.OrderTypeNumber, OrderLine.Price, OrderLine.Discount, Event.EventCode, OrderLine.ItemType FROM Event (NOLOCK) INNER JOIN OrganizationVenue (NOLOCK) ON Event.VenueCode = OrganizationVenue.VenueCode INNER JOIN OrganizationAct (NOLOCK) ON Event.ActCode = OrganizationAct.ActCode INNER JOIN Seat (NOLOCK) ON Event.EventCode = Seat.EventCode INNER JOIN OrderLine (NOLOCK) ON Seat.ItemNumber = OrderLine.ItemNumber INNER JOIN OrderHeader ON OrderLine.OrderNumber = OrderHeader.OrderNumber WHERE EventDate >= '" & ReportStartDate & "' AND EventDate < '" & ReportEndDate + 1 & "' AND OrganizationVenue.OrganizationNumber = " & Session("OrganizationNumber") & " AND OrganizationVenue.Owner = 1 AND OrganizationAct.OrganizationNumber = " & Session("OrganizationNumber") & " AND OrderHeader.StatusCode = 'S' AND OrderLine.ItemType IN ('Seat', 'SubFixedEvent', 'SubSeat')"
...
Response.Write "Thank you for choosing www.TIX.com!</FONT><BR><BR>" & vbCrLf
