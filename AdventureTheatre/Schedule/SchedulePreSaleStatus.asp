...
If Clean(Request("MemberID")) <> "" Then
	...
	SQLLogin = "SELECT MemberID, MemberPassword, MemberType FROM Members (NOLOCK) WHERE OrganizationNumber = '" & Clean(Request("OrganizationNumber")) & "' AND MemberID = '" & MemberID & "' AND MemberPassword = '" & MemberID & "'"
	...
	If Not rsLogin.EOF Then
		...
		Session("MemberOrganizationNumber") = Clean(Request("OrganizationNumber"))
	Else
...
If FormName = "Logout" Then
    ...
    Response.Redirect("/SchedulePreSale.asp" & QueryString)
End If
%>
...
If Session("MemberID") = "" Then
	Response.Write "<TABLE CELLSPACING=""1"" CELLPADDING=""1"" HEIGHT=10 BORDER=""1"" BGCOLOR=""" & DataBGColor & """><TR><TD ALIGN=""center""><FORM ACTION=SchedulePreSale.asp" & QueryString & " METHOD=""post"" id=form1 name=form1><TABLE CELLPADDING=3 CELLSPACING=3 BORDER=0><TR><TD ALIGN=""right""><FONT FACE=" & FontFace & " COLOR=" & FontColor & " SIZE=2>Enter Pre-Sale Code (optional):&nbsp;<INPUT TYPE=""text"" NAME=""MemberID"" SIZE=""10""></FONT>"
	If Message <> "" Then
		Response.Write "<BR><FONT FACE=" & FontFace & " COLOR=RED SIZE=1><B>" & Message & "&nbsp;&nbsp;&nbsp;</B></FONT>"
	End If
	Response.Write "</TD><TD ALIGN=""left"" VALIGN=""top""><INPUT TYPE=""submit"" VALUE=""Enter"" id=1 name=1></TD></TR>" & vbCrLf
	If Session("MemberID") <> "" Then
		Response.Write "<TR><TD ALIGN=""center"" COLSPAN=""2""><FONT FACE=" & FontFace & " COLOR=" & FontColor & " SIZE=1>You are currently logged in.  Appropriate pre-sale events will appear below.</FONT></TD></TR>" & vbCrLf
	Else
		Response.Write "<TR><TD ALIGN=""center"" COLSPAN=""2""><FONT FACE=" & FontFace & " COLOR=" & FontColor & " SIZE=1>Upon entering a valid pre-sale code, the pre-sale events will appear below.</FONT></TD></TR>" & vbCrLf
	End If
	Response.Write "</TABLE></TD></TR></TABLE></FORM>" & vbCrLf
    
Else 'Member currently logged in
    Response.Write "<TABLE CELLSPACING=""1"" CELLPADDING=""4"" HEIGHT=10 BORDER=""1"" BGCOLOR=""" & DataBGColor & """><TR>" & vbCrLf
    Response.Write "<TD ALIGN=""center"" VALIGN=""top""><FORM ACTION=SchedulePreSale.asp" & QueryString & " METHOD=""post"" Name=Logout><INPUT TYPE=hidden NAME=FormName VALUE=Logout>"
	Response.Write "<FONT FACE=" & FontFace & " COLOR=" & FontColor & " SIZE=1>You are currently logged in as Member ID: <B>" & Session("MemberID") & "</B>.<BR>Appropriate pre-sale events will appear below.<BR><INPUT TYPE=""submit"" VALUE=""Logout""<BR></FONT></FORM></TD></TR>" & vbCrLf
...
'REE 9/2/8 - Removed vbShortDate function to use Now() only.  Start times were being ignored.
SQLQuery = "SELECT DISTINCT EventDate, Event.EventCode, Act, City, State, Venue From Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode INNER JOIN Category (NOLOCK) ON Act.CategoryCode = Category.CategoryCode INNER JOIN SubCategory (NOLOCK) ON Act.SubCategoryCode = SubCategory.SubCategoryCode INNER JOIN Venue (NOLOCK) ON Event.VenueCode = Venue.VenueCode INNER JOIN OrganizationAct (NOLOCK) ON Event.ActCode = OrganizationAct.ActCode INNER JOIN OrganizationVenue (NOLOCK) ON Event.VenueCode = OrganizationVenue.VenueCode LEFT JOIN MemberSaleStartDate (NOLOCK) ON Event.EventCode = MemberSaleStartDate.EventCode " & SQLWhere & "AND Event.OnSale = 1 AND (MemberSaleStartDate.OrganizationNumber = '" & Clean(Request("OrganizationNumber")) & "' AND MemberSaleStartDate.MemberType = '" & Session("MemberType") & "' AND MemberSaleStartDate.MemberSaleStartDate <= '" & Now() & "' AND IsNull(MemberSaleStartDate.MemberSaleEndDate, DATEADD(DAY, 1, GETDATE())) > '" & Now() & "' OR Event.SaleStartDate <= '" & Now() & "')"
Select Case Request("SortOrder")
Case "Act"
	SQLQuery = "SELECT DISTINCT EventDate, SaleEndDate, EventCode, Act, City, State, Venue From Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode INNER JOIN Category (NOLOCK) ON Act.CategoryCode = Category.CategoryCode INNER JOIN SubCategory (NOLOCK) ON Act.SubCategoryCode = SubCategory.SubCategoryCode INNER JOIN Venue (NOLOCK) ON Event.VenueCode = Venue.VenueCode INNER JOIN OrganizationAct (NOLOCK) ON Event.ActCode = OrganizationAct.ActCode INNER JOIN OrganizationVenue (NOLOCK) ON Event.VenueCode = OrganizationVenue.VenueCode " & SQLWhere & "AND Event.OnSale = 1 AND Event.SaleStartDate <= '" & Now() &  "' ORDER BY Act.Act, Event.EventDate"
Case Else
	SQLQuery = "SELECT DISTINCT EventDate, SaleEndDate, EventCode, Act, City, State, Venue From Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode INNER JOIN Category (NOLOCK) ON Act.CategoryCode = Category.CategoryCode INNER JOIN SubCategory (NOLOCK) ON Act.SubCategoryCode = SubCategory.SubCategoryCode INNER JOIN Venue (NOLOCK) ON Event.VenueCode = Venue.VenueCode INNER JOIN OrganizationAct (NOLOCK) ON Event.ActCode = OrganizationAct.ActCode INNER JOIN OrganizationVenue (NOLOCK) ON Event.VenueCode = OrganizationVenue.VenueCode " & SQLWhere & "AND Event.OnSale = 1 AND Event.SaleStartDate <= '" & Now() &  "' ORDER BY Event.EventDate, Act.Act"
End Select
...
If Not rsEvents.EOF Then
	...
	Do While Not rsEvents.EOF
	  ...
	  'SSR 11/27/07 - Update Event Status
		
		'Count the number of unsold seats
		SQLSoldOut = "SELECT ItemNumber FROM Seat (NOLOCK) WHERE EventCode = " & rsEvents("EventCode") & " AND StatusCode = 'A'"
		Set rsSoldOut = OBJdbConnection2.Execute(SQLSoldOut)
		
			If rsSoldOut.EOF Then 'Event Status is sold out.
				EventStatus = "<FONT FACE=" & FontFace & " COLOR=#F15749 SIZE=1><B>SOLD OUT</B></FONT>"
			Else If rsEvents("SaleEndDate") <= Now() Then 'Event Status is off sale
				EventStatus = "<FONT FACE=" & FontFace & " COLOR=" & TableCategoryBGColor & " SIZE=1>Not Available</FONT>"
			Else 'Event Status is on sale
				EventStatus = "<FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & " SIZE=1><A HREF=/Event.asp?Event=" & rsEvents("EventCode") & " id=""TixInfoLink_" & rsEvents("EventCode") & """ onmouseover=""ttDelay=setTimeout('eventTooltip(\'/AJAX/EventInfo.asp?EventCode=" & rsEvents("EventCode") & "\', this)', 200);"" onmouseout=""clearTimeout(ttDelay);hideTooltip();"">Tix & Info</A>&nbsp;</FONT>"
			End If
		...
		If IncludeVenueLocation Then
		  ...
		  Response.Write "<TR BGCOLOR=" & TableDataBGColor & "><TD>&nbsp;&nbsp;</TD><TD ALIGN=left NOWRAP><FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & " SIZE=1>" & DisplayDate & "</FONT></TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=left><FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & " SIZE=1><A HREF=/Event.asp?Event=" & rsEvents("EventCode") & " id=""EventLink_" & rsEvents("EventCode") & """ onmouseover=""ttDelay=setTimeout('eventTooltip(\'/AJAX/EventInfo.asp?EventCode=" & rsEvents("EventCode") & "\', this)', 200);"" onmouseout=""clearTimeout(ttDelay);hideTooltip();""><B>" & rsEvents("Act") & "</B></A></FONT></TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=left><FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & " SIZE=1>" & rsEvents("Venue") & "<BR>" & rsEvents("City") & ", " & rsEvents("State") & "</FONT></TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=center>"& EventStatus & "</TD></TR>" & vbCrLf
		  Response.Write "<TR BGCOLOR=" & TableDataBGColor & "><TD COLSPAN=8 HEIGHT=6><IMG HEIGHT=1 SRC=""/images/clear.gif"" ALT=""""></TD></TR>" & vbCrLf
		  Response.Write "<TR BGCOLOR=" & TableCategoryBGColor & "><TD COLSPAN=8 HEIGHT=1><IMG HEIGHT=1 SRC=""/images/clear.gif"" ALT=""""></TD></TR>" & vbCrLf
		Else
		  Response.Write "<TR BGCOLOR=" & TableDataBGColor & "><TD COLSPAN=6 HEIGHT=6><IMG HEIGHT=1 SRC=""/images/clear.gif"" ALT=""""></TD></TR>" & vbCrLf
		  Response.Write "<TR BGCOLOR=" & TableDataBGColor & "><TD>&nbsp;&nbsp;</TD><TD ALIGN=left NOWRAP><FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & " SIZE=1>" & DisplayDate & "</FONT></TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=left><FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & " SIZE=1><A HREF=/Event.asp?Event=" & rsEvents("EventCode") & " id=""EventLink_" & rsEvents("EventCode") & """ onmouseover=""ttDelay=setTimeout('eventTooltip(\'/AJAX/EventInfo.asp?EventCode=" & rsEvents("EventCode") & "\', this)', 200);"" onmouseout=""clearTimeout(ttDelay);hideTooltip();""><B>" & rsEvents("Act") & "</B></A></FONT></TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=center>"& EventStatus & "</TD></TR>" & vbCrLf
		  Response.Write "<TR BGCOLOR=" & TableDataBGColor & "><TD COLSPAN=6 HEIGHT=6><IMG HEIGHT=1 SRC=""/images/clear.gif"" ALT=""""></TD></TR>" & vbCrLf
		  Response.Write "<TR BGCOLOR=" & TableCategoryBGColor & "><TD COLSPAN=6 HEIGHT=1><IMG HEIGHT=1 SRC=""/images/clear.gif"" ALT=""""></TD></TR>" & vbCrLf
		End If	
		events="Y"
