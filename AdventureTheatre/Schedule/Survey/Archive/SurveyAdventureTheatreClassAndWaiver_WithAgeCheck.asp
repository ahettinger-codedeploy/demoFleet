    ...
    For i = 1 To AvailOfferCount 
            ...
            Select Case j
             ...
             Case 4
                 ...
                 'Format Answer into a DOB
                 
                 DOB = Replace(Clean(Request("Answer"&j&"_"&i)),", ","/")  
                ...
                SQLInsertDetail = "INSERT OrderLineDetail (OrderNumber, LineNumber, FieldName, FieldValue) VALUES(" & Session("OrderNumber") & ", " & i & ", 'Answer" & j & "', '" & DOB & "'  )"            
             ...
             Case 5   
                ...
                'Get Age Min and Age Max for this event
                
                SQLEvent = "SELECT Event.EventCode, Event.EventDate, Act.Act, Venue.Venue FROM OrderLine WITH (NOLOCK) INNER JOIN Seat WITH (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber INNER JOIN Event WITH (NOLOCK) ON Seat.EventCode = Event.EventCode INNER JOIN Act WITH (NOLOCK) ON Event.ActCode = Act.ActCode INNER JOIN Venue WITH (ROWLOCK) ON Event.VenueCode = Venue.VenueCode WHERE OrderLine.OrderNumber = " & Session("OrderNumber") & " AND OrderLine.LineNumber = " & i & " GROUP BY Event.EventCode, Event.EventDate, Act.Act, Venue.Venue ORDER BY Event.EventDate, Act.Act, Venue.Venue"
                Set rsEvent = OBJdbConnection.Execute(SQLEvent)   

                    SQLTicketText1 = "SELECT OptionValue AS TicketText1 FROM EventOptions (NOLOCK) WHERE EventCode = " & rsEvent("EventCode") & " AND OptionName = 'TicketText1'"
                    Set rsTicketText1 = OBJdbConnection.Execute(SQLTicketText1)
                    	
	                    If Not rsTicketText1.EOF Then
		                    TicketText1 = rsTicketText1("TicketText1")
	                    End If
                    	
                    rsTicketText1.Close
                    Set rsTicketText1 = nothing
                    
                   SQLTicketText2 = "SELECT OptionValue AS TicketText2 FROM EventOptions (NOLOCK) WHERE EventCode = " & rsEvent("EventCode") & " AND OptionName = 'TicketText2'"
                    Set rsTicketText2 = OBJdbConnection.Execute(SQLTicketText2)
                    	
	                    If Not rsTicketText2.EOF Then
		                    TicketText2 = rsTicketText2("TicketText2")
	                    End If
                    	
                    rsTicketText2.Close
                    Set rsTicketText2 = nothing 
                    
                rsEvent.Close
                Set rsEvent = nothing
                                
                If ChildAge < TicketText1 Then 
                    ChildAge = "AgeMinError"
                End If
                
                If ChildAge > TicketText2 Then 
                    ChildAge = "AgeMaxError"
                End If                  
...
Message1 = "Please answer all the questions on this form"
Message2 = "Child is too young"
...
Set rsEvent = OBJdbConnection.Execute(SQLEvent)
        ...
        Set rsOrderLineDetail = OBJdbConnection.Execute(SQLOrderLineDetail)  
            ...
            If rsOrderLineDetail("FieldValue") = "AgeMinError" Then        
                FormComplete = "False"
                Message = Message2       
            End If
            
            If rsOrderLineDetail("FieldValue") = "AgeMaxError" Then        
                FormComplete = "False"
                Message = Message3       
            End If
            ...
            If rsOrderLineDetail("FieldValue") = "0" Then        
                FormComplete = "False"
                Message = Message1       
            End If
