...
If FormName = "Logout" Then
    ...
    Response.Redirect("/Schedule.asp" & QueryString)
End If
...
If Session("MemberID") = "" Then 'No member logged in
    If PreSaleActive Then 'Pre-sale event exists and is active, so display login box
        Response.Write "<TABLE CELLSPACING=""1"" CELLPADDING=""1"" HEIGHT=10 BORDER=""1"" BGCOLOR=""" & DataBGColor & """><TR>" & vbCrLf 
        Response.Write "<TD ALIGN=""center""><FORM ACTION=Schedule.asp" & QueryString & " METHOD=""post"" id=form1 name=form1><TABLE CELLPADDING=3 CELLSPACING=3 BORDER=0><TR><TD ALIGN=""right""><FONT FACE=" & FontFace & " COLOR=" & FontColor & " SIZE=2>Enter Pre-Sale Code (optional):&nbsp;<INPUT TYPE=""text"" NAME=""MemberID"" SIZE=""10""></FONT>"
        If Message <> "" Then
        ...
        Response.Write "<TR><TD ALIGN=""center"" COLSPAN=""2""><FONT FACE=" & FontFace & " COLOR=" & FontColor & " SIZE=1>Upon entering a valid pre-sale code, the pre-sale events will appear below.<BR>This is not a discount code</FONT></TD></TR>" & vbCrLf
...
Else 'Member currently logged in
    ...
    Response.Write "<TD ALIGN=""center"" VALIGN=""top""><FONT FACE=" & FontFace & " COLOR=" & FontColor & " SIZE=1>You are currently logged in as Member ID: <B>" & Session("MemberID") & "</B><BR>Appropriate pre-sale events will appear below.<BR><BR><INPUT TYPE=""submit"" VALUE=""Logout""></FONT></TD></TR>" & vbCrLf
