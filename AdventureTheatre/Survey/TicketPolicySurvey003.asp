...
MemberEventCode = 186501  'Eventcode for which discount is valid
...
If Session("OrderTypeNumber") = 1 Then
			If Clean(Request("Price"))= 22 Then
				NewSurcharge = 3.70
			Else
				NewSurcharge = 3.00
			End If
...
If CInt(NumberFreeTickets) > 0 Then 'It is ok to give further discounts
				...
				SQLLineNo = "SELECT TOP(" & NumberFreeTickets & ") LineNumber FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber WHERE OrderLine.OrderNumber = " & Session("OrderNumber") & " AND Seat.EventCode = " & MemberEventCode & " ORDER BY LineNumber"
...
Response.Write "<CENTER><TABLE><TR><TD><FONT FACE=" & FontFace & " COLOR=" & FontColor & " SIZE=""2"">Your order qualifies for discount pricing on " & NumberFreeTickets & " " & tix1& ".<BR><BR><CENTER>" & vbCrLf
