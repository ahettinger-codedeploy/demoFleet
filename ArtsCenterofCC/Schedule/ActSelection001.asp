...
SQLQuery = "SELECT DATENAME(yyyy,EventDate) AS YearName, DATENAME ( mm,EventDate ) AS MonthName, DATENAME ( dd,EventDate ) AS DayName, MONTH(Event.EventDate) as MonthNumber, Act.ActCode, Act.Act,  Event.EventCode FROM Event (NOLOCK) INNER JOIN Act (NOLOCK) ON Event.ActCode = Act.ActCode INNER JOIN OrganizationAct (NOLOCK) ON Event.ActCode = OrganizationAct.ActCode INNER JOIN OrganizationVenue (NOLOCK) ON Event.VenueCode = OrganizationVenue.VenueCode " & SQLWhere & " AND Event.EventCode NOT IN (" &  SubscriptionEvents & ") AND Event.OnSale = 1 AND Event.SaleStartDate <= '" & Now() &  "' ORDER BY YEAR(Event.EventDate), MONTH(Event.EventDate), Act.Act,"
...
If Not rsEvents.EOF Then
	...
	Response.Write "<TR BGCOLOR=" & TableCategoryBGColor & "><TD>&nbsp;&nbsp;</TD><TD ALIGN=center><FONT FACE=" & FontFace & " COLOR=" & TableCategoryFontColor & " SIZE=2><B>Event</B></FONT></TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=center><FONT FACE=" & FontFace & " COLOR=" & TableCategoryFontColor & " SIZE=2><B>Date(s)</B></FONT></TD><TD>&nbsp;&nbsp;</TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=center><FONT FACE=" & FontFace & " COLOR=" & TableCategoryFontColor & " SIZE=2><B>Tickets</B></FONT></TD></TR>" & vbCrLf
	Response.Write "<TR BGCOLOR=" & TableCategoryBGColor & "><TD COLSPAN=7 HEIGHT=2><IMG HEIGHT=1 SRC=""/images/clear.gif""></TD></TR>" & vbCrLf
	...
	Do While Not rsEvents.EOF
		...
		If LastActCode <> rsEvents("ActCode") Then
			...
			If rsDates("StartDate") <> rsDates("EndDate") Then 'Display date range
				Response.Write "<TR BGCOLOR=" & TableDataBGColor & "><TD>&nbsp;&nbsp;</TD><TD><FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & " SIZE=1><A HREF=" & Link & " id=""ProductionInfoLink_" & rsEvents("ActCode") & """ onmouseover=""eventTooltip('/AJAX/ProductionInfo.asp?ActCode=" & rsEvents("ActCode") & "', this);"" onmouseout=""hideTooltip();"">" & rsEvents("Act") & "</A></FONT></TD><TD>&nbsp;&nbsp;</TD><TD ALIGN=center NOWRAP><FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & " SIZE=1>" & MonthName(Month(rsDates("StartDate")), true) & " " & Day(rsDates("StartDate")) & " ~ " & MonthName(Month(rsDates("EndDate")), true) & " " & Day(rsDates("EndDate")) & "</FONT></TD><TD>&nbsp;&nbsp;</TD><TD>&nbsp;&nbsp;</TD><TD><FONT FACE=" & FontFace & " COLOR=" & TableDataFontColor & " SIZE=1><A HREF=" & Link & " id=""TixInfoLink_" & rsEvents("ActCode") & """ onmouseover=""eventTooltip('/AJAX/ProductionInfo.asp?ActCode=" & rsEvents("ActCode") & "', this);"" onmouseout=""hideTooltip();"">Tix & Info</A></FONT></TD></TR>" & vbCrLf
			Else 'Display Date
				DisplayDate = ""
