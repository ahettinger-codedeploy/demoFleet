...
If Not rsSeat.EOF Then
	...
	If Clean(Request("IncludeSurvey")) = "on" Then
		SurveyResults = "<TD ALIGN=center><FONT FACE=verdana,arial,helvetica SIZE=2 COLOR=#FFFFFF><B>Survey Answers</B></FONT></TD>"
	...
	Do Until rsSeat.EOF
		...
		If CustomerNo <> rsSeat("CustomerNumber") Then
	        ...
	        If SurveyResults <> "" Then
	            ...
	            SQLSurveyAnswers = "SELECT Question, Answer FROM SurveyAnswers (NOLOCK) INNER JOIN OrderHeader (NOLOCK) ON SurveyAnswers.OrderNumber = OrderHeader.OrderNumber WHERE SurveyAnswers.OrderNumber = " & rsSeat("OrderNumber") & " ORDER BY AnswerNumber"
	            Set rsSurveyAnswers = OBJdbConnection2.Execute(SQLSurveyAnswers)
	            Do While Not rsSurveyAnswers.EOF
	                SurveyAnswers = SurveyAnswers & rsSurveyAnswers("Question") & " - " & rsSurveyAnswers("Answer") & "<BR>" & vbCrLf
