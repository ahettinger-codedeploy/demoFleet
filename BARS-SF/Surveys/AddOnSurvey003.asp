...
'Bay Area Rainbow Symphony (6/01/2010)
'=====================================

'Add On Event Survey

'Purchase 1 ticket to this event:
'EventCode  EventName
'250681     BARS: Milhaud, Gershwin, Revueltas, & Copland w/ Ruth Butterfield-Winter 


'Receive 1 ticket option to this event:
'EventCode  EventName
'271676     BARS VIP Wine Reception 

'SeatType: VIP Reception (819)
'Offer Price: $10.00

'=====================================

RequiredEvent = 250681
RequiredSeatType = "3,6,2117,2040"
...
<%
    ...
    If EventCodeSelected = RequiredEvent Then   
        EventCodeList = OfferEvent
    Else 'Invalid EventCode
        Call Continue    
    End If
         
    'Determine how many tickets in this package
    SQLTicketCount = "SELECT COUNT(LineNumber) AS TotalTickets FROM OrderLine (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber") & " "
...
<TABLE CELLPADDING=10 CELLSPACING=5 WIDTH="600" border=0>
    ...
    <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=rsPackages("EventDate")%></FONT></br><br />
    
    <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2">Ticket Price: $10.00</FONT><br /><br />
    
    <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2">Ticket Quantity: <%=NumTickets%></FONT>
    ...
    <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><INPUT type="submit" value="Add to Cart" /></FONT>
    </FORM>
	...
	Select Case ReturnCode
		Case 0	'Stored Procedure executed properly.  Continue.
			...
			For i = 1 To OfferQuantity
				...
				SQLUpdateSeatType = "UPDATE OrderLine WITH (ROWLOCK) SET SeatTypeCode = " & Clean(Request("SeatTypeCode")) & ", Price = " & Price & ", Surcharge = 0, Discount = 0 WHERE OrderNumber = " & Session("OrderNumber") & " AND ItemNumber IN (SELECT TOP 1 ItemNumber FROM OrderLine (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber") & " AND SeatTypeCode IS NULL AND ItemType IN ('Seat', 'SubFixedEvent') ORDER BY LineNumber DESC)"
...
SQLMemberDonation = "SELECT OrderLine.ItemNumber FROM OrderLine (NOLOCK) WHERE OrderLine.ItemNumber IN (2886,2887,2888,2889,2890) AND OrderLine.OrderNumber = " & CleanNumeric(Session("OrderNumber"))
Set rsMemberDonation = OBJdbConnection2.Execute(SQLMemberDonation)
...
If DonationFound Then
	 ...
	 Response.Redirect("/GenericSurvey.asp?SurveyNo=791")
