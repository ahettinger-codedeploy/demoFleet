...
OfferTitle = "BARS VIP Wine Reception"
...
<%
    ...
    SQLTicketCount = "SELECT COUNT(LineNumber) AS TotalTickets FROM OrderLine (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber") & " AND OrderLine.SeatTypeCode IN (" & RequiredSeatType & ")"
...
<TABLE CELLPADDING=10 CELLSPACING=5 WIDTH="600" border=0>
    ...
    <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><%=rsPackages("Act")%></FONT></br><br />
    ...
    <FONT FACE="<%= FontFace %>" COLOR="<%= FontColor %>" SIZE="2"><INPUT type="submit" value="Yes" /></FONT>
    </FORM>
