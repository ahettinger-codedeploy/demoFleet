<%
'CHANGE LOG
'TTT - 9/2/11 - Added check if re-open order then bypass hidden add-on events
%>
...
Page = "Survey"
SurveyNumber = 1034
SurveyFileName = "HiddenAddOnSurvey.asp"
...
'check if order is reopened
SQLOpen = "SELECT OrderNumber FROM OrderHeader (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber") & " AND StatusCode = 'O'"
Set rsOpen = OBJdbConnection.Execute(SQLOpen)
If Not rsOpen.EOF Then 'order is re-opened
    rsOpen.Close
    Set rsOpen = Nothing
    Call Continue
End If
...
'check if required events are on order
SQLReqEvents = "SELECT Orderline.SeatTypeCode, Seat.SectionCode, COUNT(*) AS TotalTix FROM OrderLine (NOLOCK) INNER JOIN Seat (NOLOCK) ON OrderLine.ItemNumber = Seat.ItemNumber WHERE Orderline.OrderNumber = " & Session("OrderNumber") & " AND Seat.EventCode IN (351761,351762,351764) AND OrderLine.ItemType IN ('Seat', 'SubFixedEvent') GROUP BY Orderline.SeatTypeCode, Seat.SectionCode"
Set rsReqEvents = OBJdbConnection.Execute(SQLReqEvents)

    If Not rsReqEvents.EOF Then
        SeatTypeCode = rsReqEvents("SeatTypeCode")
        SectionCode =  rsReqEvents("SectionCode")
        TotalTix = rsReqEvents("TotalTix")
    End If
    
rsReqEvents.Close
Set rsReqEvents = Nothing

If CInt(TotalTix) > 0 Then
    Call AddToCart("353597",SectionCode,TotalTix,SeatTypeCode)
Else
    Call Continue
End If
	...
	Select Case ReturnCode
		Case 0	'Stored Procedure executed properly.  Continue.
			...
			For i = 1 To OfferQuantity
				...
				'SQLPrice = "SELECT Price, Surcharge FROM Price (NOLOCK) WHERE EventCode = " & EventCode & " AND SectionCode = '" & SectionCode & "' AND SeatTypeCode = " & CleanNumeric(SeatTypeCode)
				...
				SQLUpdateSeatType = "UPDATE OrderLine WITH (ROWLOCK) SET SeatTypeCode = " & CleanNumeric(SeatTypeCode) & ", Price = 0, Surcharge = 0, Discount = 0 WHERE OrderNumber = " & Session("OrderNumber") & " AND ItemNumber IN (SELECT TOP 1 ItemNumber FROM OrderLine (NOLOCK) WHERE OrderNumber = " & Session("OrderNumber") & " AND SeatTypeCode IS NULL AND ItemType IN ('Seat', 'SubFixedEvent') ORDER BY LineNumber DESC)"
...
Sub Continue
    Response.Redirect("/Clients/Bars-SF/Surveys/AudienceSurvey.asp")
