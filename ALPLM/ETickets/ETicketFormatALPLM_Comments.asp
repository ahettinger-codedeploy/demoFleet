...
'SSR 3/24/15 - Updated format.  If event comments are found print thsoe and suppress event date.
...
If Not rsOrderNum.EOF Then
	...
	If Not rsTix.EOF Then
		...
		Do Until rsTix.EOF
			...
			SQLEventComments = "SELECT Event.Comments FROM Event(NOLOCK) WHERE EventCode = " & rsTix("EventCode") & ""
			Set rsEventComments = OBJdbConnection.Execute(SQLEventComments)
			
				If IsNull(rsEventComments("Comments")) Then
				...
				Else
					Response.Write "<BR><FONT FACE=""Arial,Helvetica"" SIZE=""3"">" & rsEventComments("Comments") & "</FONT><BR><BR>" & vbCrLf
				End If
			
			rsEventComments.Close
			Set rsEventComments = nothing
