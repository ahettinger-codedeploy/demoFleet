...
EMailOrganization = "Cabrillo Playhouse"
EMailReplyTo = "Cabrillo_Theatre@hotmail.com"
EMailSubject = "Cabrillo Playhouse Season Ticket Renewal" 
EMailRenewalURL = "http://cabrilloplayhouse.tix.com/renew.aspx"
EMailPhoneNumber = "(949) 492-0465"
EMailFontFace = "Calibri, Arial, Helvetica"
BeginOrderNumber = 0

DIM SHOWCHILDEVENTS 
SHOWCHILDEVENTS = TRUE


'TestEMailAddress = "sergio@tix.com"
'TestEMailAddress = "silvia@tix.com"
'TestEMailAddress = "support@tix.com"
	...
	If rsOrderLine("OrderNumber") <> LastOrderNumber Then 'It's different.  Print it.
		If TicketCount > 0 Then 'It's not the first one, print the footer.
			...
			If LastTender <> 0 Then
				EMailMessage = EMailMessage & "<TR><TD WIDTH=""50"">&nbsp;</TD><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=C" & EMailFontFace & ">Less Payments</TD><TD ALIGN=""right""><FONT FACE=" & EMailFontFace & ">" & FormatCurrency(LastTender,2) & "</TD></TR>" & vbCrLf
...
<%		
	...
	If LastEventCode <> rsOrderLine("EventCode") Or LastOrderNumber <> rsOrderLine("OrderNumber") Then
        ...
        End if
		EMailMessage = EMailMessage & "<TABLE border=""0""><TR><TD WIDTH=10>&nbsp;</TD><TD><FONT FACE=" & EMailFontFace & "><B>" & rsOrderLine("Act") & " at " & rsOrderLine("Venue")
		...
		EMailMessage = EMailMessage & "</B><BR>Delivery Method: " & rsOrderLine("ShipType") & "</FONT></TD></TR>" & vbCrLf
		
		'-----------------------------------------
				
			'Child Events
			
			If SHOWCHILDEVENTS Then
				...
				If Not rsEventChild.EOF Then
					...
					ChildEvents = ChildEvents & "<TD colspan=""2""><BR><FONT FACE=" & EMailFontFace & ">This series includes:</FONT><BR></td>" & vbCrLf
					ChildEvents = ChildEvents & "</tr>" & vbCrLf
					ChildEvents = ChildEvents & "<TR>" & vbCrLf
					ChildEvents = ChildEvents & "<TD><FONT FACE=" & EMailFontFace & "><B>Production</B></FONT></td>" & vbCrLf
					ChildEvents = ChildEvents & "<TD><FONT FACE=" & EMailFontFace & "><B>Date</B></FONT></td>" & vbCrLf
					ChildEvents = ChildEvents & "</tr>" & vbCrLf
				
						Do While Not rsEventChild.EOF

							ChildEvents = ChildEvents & "<tr>" & vbCrLf							
							ChildEvents = ChildEvents & "<td WIDTH=""40%"">" & vbCrLf
							ChildEvents = ChildEvents & "<FONT FACE=" & EMailFontFace & ">" & rsEventChild("Act") & "</FONT>" & vbCrLf	
							ChildEvents = ChildEvents & "</td>" & vbCrLf	
							ChildEvents = ChildEvents & "<td WIDTH=""60%"" NOWRAP>" & vbCrLf
							ChildEvents = ChildEvents & "<FONT FACE=" & EMailFontFace & ">" & FormatDateTime(rsEventChild("EventDate"), vbLongDate) & " at " & Left(FormatDateTime(rsEventChild("EventDate"),vbLongTime),Len(FormatDateTime(rsEventChild("EventDate"),vbLongTime))-6) & Right(FormatDateTime(rsEventChild("EventDate"),vbLongTime),3) & "</FONT>" & vbCrLf
							ChildEvents = ChildEvents & "</td>" & vbCrLf
							ChildEvents = ChildEvents & "</tr>" & vbCrLf
					...
					ChildEvents = ChildEvents & "</TABLE><BR>" & vbCrLf
