...
ClientDirectory = "/Clients/ArmedForcesBowl/Renewal"

TemplateFileName = "2016_ArmedForcesBowl.asp"

LogoFileName = ""

DIM RenewalURL
RenewalURL = "http://armedforcesbowl.tix.com/renew.aspx?"
...
DIM EMailSubject
EMailSubject = "REMINDER Renew your bowl tickets NOW!" 
...
html.wf-active body div.page-wrapper div.justify-center.content-bg div#content-wrapper div#content-inner-wrapper div.page-content div#navbar div.pull-right a.btn.btn-default
{
color: #333333;
transition: all 0.35s ease-in-out 0s;
}

html.wf-active body div.page-wrapper div.justify-center.content-bg div#content-wrapper div#content-inner-wrapper div.page-content div#navbar div.pull-right a:hover.btn.btn-default
{
background-color: #CECECE;
transition: all 0.35s ease-in-out 0s;
}

html.wf-active body div.page-wrapper div.justify-center.content-bg div#content-wrapper div#content-inner-wrapper div.page-content div#navbar div.active.pull-right a.btn.btn-default
{
color: #0E9F0E;
font-weight:bold;
background-color: #FFFFFF;
...
<div class="panel panel-default" id="panel-parent">
			...
			If EventList <> "" Then
				...
				If Not rsTickets.EOF Then 'There are some events.  List them
					...
					Response.Write "<TABLE class=""table"">" & vbCrLf
...
Do Until rsOrderLine.EOF
	...
	'This is not the last order.
		...
		If TicketCount > 0 Then
			...
			DetailTender = DetailTender & "<TR><TD COLSPAN=""" & NumColumns - 1 & """>&nbsp;</TD><TD><HR></TD></TR>" & vbCrLf			
			If LastShipFee <> 0 And LastOrderTypeNumber <> 5 Then
				DetailTender = DetailTender & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Processing Fee</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastShipFee,2) & "</TD></TR>" & vbCrLf
				DetailTender = DetailTender & "<TR><TD COLSPAN=""" & NumColumns - 1 & """>&nbsp;</TD><TD><HR></TD></TR>" & vbCrLf
			End If
			...
			DetailTender = DetailTender & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Order Total</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTotal,2) & "</TD></TR>" & vbCrLf
			If LastTender <> 0 Then
				DetailTender = DetailTender & "<TR><TD WIDTH=""50"">&nbsp;</TD><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=C" & MsgFontFace & ">Less Payments</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTender,2) & "</TD></TR>" & vbCrLf
				DetailTender = DetailTender & "<TR><TD WIDTH=""50"">&nbsp;</TD><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Balance Due</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTotal - LastTender,2) & "</TD></TR>" & vbCrLf
			End If
			DetailTender = DetailTender & "</TABLE>" & vbCrLf
			DetailTender = DetailTender & "</CENTER><BR>" & vbCrLf
                  
                  'EMailMessage = EMailMessage &  DetailTender
		...
		If Not rsCustInfo.EOF Then
			...
			Set rsItemNumber = OBJdbConnection.Execute(SQLItemNumber)
				...
				EasyRenewalURL = RenewalURL & "o=" & RenewalOrder & "&c=" & RenewalCode & ""
                        ...
                        If rsCustInfo("ShipFirstName") = "*" Then
                              PatronFirstName =  UCase(rsCustInfo("ShipLastName")) 
                        Else
                              PatronFirstName = UCase(rsCustInfo("ShipFirstName"))
                        End If
	...
	If LastEventCode <> rsOrderLine("EventCode") Or LastOrderNumber <> rsOrderLine("OrderNumber") Then
		...
		ProductionDetail = ProductionDetail & "<TABLE WIDTH=""650"" cellpadding=""0"" cellspacing=""0""  border=""0"">" & vbCrLf
		ProductionDetail = ProductionDetail & "<TR><TD>" & vbCrLf
		ProductionDetail = ProductionDetail & "<FONT FACE=""" & MsgFontFace & """><B>" & rsOrderLine("Act") & "<br>" & vbCrLf
		ProductionDetail = ProductionDetail & "at " & rsOrderLine("Venue") & "<br>" & vbCrLf
		...
		If rsDateSuppress.EOF Then
			ProductionDetail = ProductionDetail & " on " & FormatDateTime(rsOrderLine("EventDate"),vbShortDate)
		End If
		...
		If rsTimeSuppress.EOF Then
			ProductionDetail = ProductionDetail & " at " & Left(FormatDateTime(rsOrderLine("EventDate"),vbLongTime),Len(FormatDateTime(rsOrderLine("EventDate"),vbLongTime))-6) & Right(FormatDateTime(rsOrderLine("EventDate"),vbLongTime),3)
		End If
		rsTimeSuppress.Close
		Set rsTimeSuppress = nothing

		ProductionDetail = ProductionDetail & "</B><BR>Delivery Method: " & rsOrderLine("ShipType") & "</TD></TR>" & vbCrLf
		ProductionDetail = ProductionDetail & "</TABLE>" & vbCrLf
		
		ProductionDetail = ProductionDetail & "<BR>" & vbCrLf
            
            
            'EMailMessage = EMailMessage & ProductionDetail
		...
		'-----------------------------------------
			...
			If SHOWCHILD Then
				...
				If Not rsEventChild.EOF Then
					...
					ChildEvents = ChildEvents & "<FONT FACE=" & MsgFontFace & " SIZE=2><B>This subscription includes:</B></FONT>" & vbCrLf
					...
					ChildEvents = ChildEvents & "</tr>" & vbCrLf
						...
						Do While Not rsEventChild.EOF
							...
							ChildEvents = ChildEvents & "<tr>" & vbCrLf	
							ChildEvents = ChildEvents & "<td ALIGN=""LEFT"" WIDTH=""65%"">" & vbCrLf
							ChildEvents = ChildEvents & "<FONT FACE=" & MsgFontFace & " SIZE=""2"">" & rsEventChild("Act") & "</FONT>" & vbCrLf	
							ChildEvents = ChildEvents & "</td>" & vbCrLf	
							ChildEvents = ChildEvents & "<td ALIGN=""RIGHT""  WIDTH=""35%"" NOWRAP>" & vbCrLf
							ChildEvents = ChildEvents & "<FONT FACE=" & MsgFontFace & " SIZE=""2"">" & WeekDayName(WeekDay(rsEventChild("EventDate")),1) & "&nbsp;" & FormatDateTime(rsEventChild("EventDate"),vbShortDate) & " at " & Left(FormatDateTime(rsEventChild("EventDate"),vbLongTime),Len(FormatDateTime(rsEventChild("EventDate"),vbLongTime))-6) & Right(FormatDateTime(rsEventChild("EventDate"),vbLongTime),3) & "</FONT>" & vbCrLf
							ChildEvents = ChildEvents & "</td>" & vbCrLf
							ChildEvents = ChildEvents & "</tr>" & vbCrLf
...
If TicketCount > 0 Then	'Print the last footer
	...
	'EMailMessage = TenderDetail & EMailMessageBody
	
	If TestEMailAddress = "" Then
      
                  TenderDetail = ""
	
			TenderDetail = TenderDetail  & "<TR><TD COLSPAN=""" & NumColumns - 1 & """>&nbsp;</TD><TD><HR></TD></TR>" & vbCrLf			
			
			If LastShipFee <> 0 And LastOrderTypeNumber <> 5 Then
				TenderDetail = TenderDetail & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Processing Fee</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastShipFee,2) & "</TD></TR>" & vbCrLf
				TenderDetail = TenderDetail & "<TR><TD COLSPAN=""" & NumColumns - 1 & """>&nbsp;</TD><TD><HR></TD></TR>" & vbCrLf
			End If
			...
			TenderDetail = TenderDetail & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Order Total</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTotal,2) & "</TD></TR>" & vbCrLf
			
			If LastTender <> 0 Then
				TenderDetail = TenderDetail & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Less Payments</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTender,2) & "</TD></TR>" & vbCrLf
				TenderDetail = TenderDetail & "<TR><TD ALIGN=""right"" COLSPAN=""" & NumColumns - 1 & """><FONT FACE=" & MsgFontFace & ">Balance Due</TD><TD ALIGN=""right""><FONT FACE=" & MsgFontFace & ">" & FormatCurrency(LastTotal - LastTender,2) & "</TD></TR>" & vbCrLf
			End If
						
			TenderDetail = TenderDetail & "</TABLE><BR>" & vbCrLf
                  
                  'EMailMessage = EMailMessage & TenderDetail
...
strNewText = ""
	...
	Else
		...
		strNewText = "Template File Not Found: " & TemplateFilePath & ""
...
PUBLIC FUNCTION replaceVar(strText)
		...
		strNewText = Replace(strNewText, "[EasyRenewalURL]", EasyRenewalURL)
            strNewText = Replace(strNewText, "[PatronFirstName]", PatronFirstName)
